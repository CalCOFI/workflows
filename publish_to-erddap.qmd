...
# The dataset-catalog page (plan 2026-09-05 D-4/D-5(4)) — built from the key
...
# creator_name/_type/_email/_url + institution: I-13's WS-P2 fix (D-5(4)) —
...
# organization (never invented) — the same fallback chain WS-E1's build_eml()
...
# re-serializes it as the literal 6-character string "—" — verified on
...
for (i in seq_len(nrow(cfg))) {
  ...
  xml <- erddap_duckdb_dataset_xml(
    ...
    # OMITTED, never written as the literal string "NA" — an ERDDAP global
...
xml_all <- paste0(
  ...
  # the release date, never the wall clock: a config that changes only its header on
  # every render is a change to git, to the server and to ERDDAP's reload for nothing
...
cat(glue("3. spliced {length(ids$all)} generated + {length(ids$kept_outside)} hand-maintained; ",
         "added {length(ids$added)}, changed {length(ids$changed)}, retiring {length(ids$retired)}\n"))
...
# only a dataset whose definition changed (or is new) is flagged: a reload of all 34
# on every render is server work for nothing, and the verification that follows it
# is a 30 s wait for nothing
to_flag <- union(ids$added, ids$changed)
if (length(to_flag) || length(ids$retired)) {
  erddap_flag(SSH_HOST, ERDDAP_FLAG, to_flag, ids$retired)
  cat(glue("4. flagged {length(to_flag)} for reload\n"))
  # ERDDAP reloads asynchronously, so give it a moment before asking.
  Sys.sleep(30)
} else cat("4. no dataset definition changed — nothing flagged\n")
...
```{r}
#| label: deploy-status
# which release erddap.calcofi.io serves, for publish_status.qmd: this run's when it
# deployed (and verified — the stopifnot above stops the render otherwise), else the
# last run's, carried forward, so a dry run reports the server as behind
status_path <- file.path(dir_erddap, "deploy_status.json")
prev <- if (file.exists(status_path)) jsonlite::fromJSON(status_path, simplifyVector = FALSE) else list()
st <- if (DEPLOY) list(
  release = RELEASE, deployed_release = RELEASE,
  deployed_utc = format(Sys.time(), "%Y-%m-%dT%H:%M:%SZ", tz = "UTC"),
  added = as.list(ids$added), changed = as.list(ids$changed), retired = as.list(ids$retired)) else
  modifyList(prev, list(release = RELEASE))
writeLines(jsonlite::toJSON(st, auto_unbox = TRUE, pretty = TRUE, null = "null"), status_path)
cat(glue("erddap.calcofi.io serves {st$deployed_release %||% 'an unknown release'}; the release is {RELEASE}\n"))
```
