...
# One flag for the whole post-release chain: CALCOFI_DEPLOY=true also drives the
# server-side consumer sync from test_release.qmd. CALCOFI_ERDDAP_DEPLOY remains
# for deploying ERDDAP alone (it is the slowest leg — a ~1.6 GB server-side
# parquet pull — so it is worth being able to run it on its own).
DEPLOY      <- identical(Sys.getenv("CALCOFI_ERDDAP_DEPLOY"), "true") ||
               identical(Sys.getenv("CALCOFI_DEPLOY"), "true")
...
cat(glue("deploy: {DEPLOY} (set CALCOFI_DEPLOY=true, or CALCOFI_ERDDAP_DEPLOY=true for ERDDAP alone, to push to {ERDDAP_URL})\n"))
```
