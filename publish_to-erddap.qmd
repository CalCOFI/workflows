...
# calcofi_ctd-cast, ucsd_sio_mesopelagic-fish — produces SQL DuckDB parses as a
...
1. **Sync the release parquet** to the identity-mounted path the views name. The
   ...
   ```bash
   # rsync is NOT installed on the server, and the release is already on GCS,
   # so pull it server-side rather than pushing 2 GB from a laptop. Its gcloud
   # needs the alpha track; gsutil is simpler:
   ssh calcofi "mkdir -p {ERDDAP_ROOT}/datasets/release/{RELEASE} \\
     && gsutil -m cp -r gs://calcofi-db/ducklake/releases/{RELEASE}/parquet \\
          {ERDDAP_ROOT}/datasets/release/{RELEASE}/ \\
     && gsutil -m cp -r gs://calcofi-db/ingest/calcofi_mets {ERDDAP_ROOT}/datasets/ingest/" 
   ```
...
2. **Build the view database on the server.** It cannot be built here: DuckDB
   ...
   ```bash
   scp data/erddap/build_erddap_db.R calcofi:{ERDDAP_ROOT}/
...
5. **Verify** — a `.das` request is the cheapest end-to-end check:
   ...
   and check `/share/erddap/data/logs/log.txt` for `WARNING: Bad line(s)`.
