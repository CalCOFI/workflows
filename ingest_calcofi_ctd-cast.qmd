...
# archives this notebook has to fetch from calcofi.org are 25-125 MB and the
# server delivers ~0.4 MB/s; R's default `timeout` of 60 s aborts every one of
# them at 15-25 MB (run of 2026-08-24: seven consecutive truncations, then the
# render failed on the archives that never arrived). One hour per file.
...
fp <- input_fingerprint(
  ...
  files = c(
    ...
    file.path(dir_meta, "flag_overrides.csv"),
  ...
  #
  # ...and the archive's SIZE on disk: a provider re-publishing a corrected zip under
  # the same name (20-2607SH_CTDPrelim.zip, 2026-09-14: station numbers >= 100 had
  # been cut to three digits) changes nothing else, so without it the fix would read
  # as "inputs unchanged" and never be ingested (CalCOFI/workflows#104)
  values = c(sort(paste(d_zips$source, d_zips$file_zip,
                        file.size(file.path(dir_dl, d_zips$file_zip)))),
...
# restore from checkpoint DB if available (skips expensive read+bind+filter) — but
# only a checkpoint built from THESE archives: it holds ctd_raw, i.e. the files as
# read, so a replaced archive must not be answered from the old copy of it
fp_zips <- input_fingerprint(
  files  = file.path(dir_meta, "flds_redefine.csv"),
  values = sort(paste(d_zips$file_zip, file.size(file.path(dir_dl, d_zips$file_zip)))))
checkpoint_fp_path <- paste0(db_checkpoint, ".inputs.json")
fp_ckpt <- read_input_fingerprint(checkpoint_fp_path)
if (file_exists(db_checkpoint) &&
    (is.null(fp_ckpt) || !identical(fp_ckpt$hash, fp_zips$hash))) {
  file_delete(db_checkpoint)
  say("Checkpoint discarded: built from a different set of archives")
}
...
# DuckDB's memory limit from what is AVAILABLE, not half of total: this notebook
# shares the machine with R's own ~20 GB of scan frames and whatever else runs (the
# 2026-09-23 run crawled at 3-9x under swap with a fixed half-of-RAM limit). 40 % of
# available, between 2 GB and half of total; DuckDB spills to temp_directory beyond it.
# BUILD_MEMORY_GB overrides it (as libs/duckdb_budget.R does in the sibling repos): on
# 2026-09-24 availability was 5.8 GB at start, so this rule gave DuckDB 2 GB while R itself
# held ~25 GB of scan frames that nothing released (fixed in [rename_fields]).
mem_sys <- ps::ps_system_memory()
mem_gb  <- max(2, floor(min(mem_sys$total / 2, mem_sys$avail * 0.4) / 1024^3))
mem_env <- suppressWarnings(as.numeric(Sys.getenv("BUILD_MEMORY_GB", NA)))
if (!is.na(mem_env) && mem_env > 0) mem_gb <- mem_env
say(glue("DuckDB memory_limit {mem_gb} GB (available {round(mem_sys$avail / 1024^3, 1)} of ",
         "{round(mem_sys$total / 1024^3)} GB; BUILD_MEMORY_GB {if (is.na(mem_env)) 'unset' else mem_env})"))
# every connection this notebook opens to the wrangling DB gets the same budget. The
# checkpoint step closes and reopens `con`, and until 2026-09-24 the reopened connection
# silently ran on DuckDB's default (80 % of RAM) instead of this limit: the restarted
# render's R process peaked at 22 GB in [ctd_measurement] with a 6 GB limit "set".
# preserve_insertion_order = false lets the large INSERT ... SELECTs of the pivot stream
# instead of buffering to keep row order, which nothing downstream relies on (every
# table is re-sorted at write_parquet_outputs()).
duck_budget <- function(con) {
...
download_and_unzip <- function(url, file_zip, dest_dir, ext_dir, zip_type,
  ...
  # an archive REPLACED under the same name (see above) keeps its member count, so
  # the count below would bless the old extraction: key it on the archive's size and
  # mtime instead, recorded beside the extraction
  stamp_file <- file.path(dir_unzip, ".archive_stamp")
  stamp_now  <- paste(file.size(dest_file), as.numeric(file.mtime(dest_file)))
  stale <- dir_exists(dir_unzip) && if (file_exists(stamp_file))
    !identical(read_lines(stamp_file), stamp_now) else
    file.mtime(dest_file) > file.mtime(dir_unzip)
  if (stale) {
    say(glue("Archive changed since it was extracted — re-extracting {file_zip}"))
    dir_delete(dir_unzip)
  }
  ...
  write_lines(stamp_now, stamp_file)
}
...
# A provider may re-publish a corrected archive under the same name — the 2607
# preliminary file was, on 2026-09-14, after station numbers >= 100 had shipped cut
# to three digits (CalCOFI/workflows#104). The Shared Drive stays the authoritative
# copy (it primes dir_dl above), so this REPORTS a calcofi.org archive whose size
# differs from the local copy rather than replacing it: replacing it here would swap
# an input without the per-series diff a data fix needs. Only archives this notebook
# reads, only an HTTP 200 (calcofi.org lists some archives that 404), and a failed
# HEAD is simply not reported. Fix a hit by updating the Shared Drive copy.
head_size <- function(url) tryCatch({
  r <- curl::curl_fetch_memory(url, handle = curl::new_handle(
    nobody = TRUE, timeout = 30, followlocation = TRUE))
  if (r$status_code != 200) return(NA_real_)
  as.numeric(curl::parse_headers_list(r$headers)[["content-length"]] %||% NA)
}, error = function(e) NA_real_)
d_zip_drift <- d_zips_read |>
  filter(!is.na(url), zip_type %in% c("final", "preliminary"),
         file_exists(file.path(dir_dl, file_zip))) |>
  mutate(local_bytes  = file.size(file.path(dir_dl, file_zip)),
         remote_bytes = map_dbl(url, head_size)) |>
  filter(!is.na(remote_bytes), remote_bytes != local_bytes) |>
  select(file_zip, zip_type, local_bytes, remote_bytes, url)
if (nrow(d_zip_drift) > 0) {
  say(glue("{nrow(d_zip_drift)} read archive(s) differ from calcofi.org — update the ",
           "Shared Drive copy and re-render: {paste(d_zip_drift$file_zip, collapse = ', ')}"))
  dt(d_zip_drift, caption = paste(
    "Read archives whose calcofi.org copy differs in size from the local (Shared Drive)",
    "copy: a re-published file the ingest has NOT taken"), fname = "ctd_zip_drift")
} else say("every read archive matches its calcofi.org size (where calcofi.org answers)")
```
...
d_csv <- d_csv |>
  ...
  mutate(
    data = map2(path, seq_along(path), \(path, idx) {
      ...
      # a flag column the provider left empty throughout a file reads as logical
      # (all NA). Make every flag column numeric so the files bind to one type and
      # COALESCE(provider flag, sensor flag) below never mixes BOOLEAN with DOUBLE.
      #
      # ...and DROP a 2026-layout flag column the file leaves wholly empty. bind_rows
      # broadcasts every column to every scan of the archive, so the 27 new *Q columns
      # of ONE file (the corrected 2607 preliminary, every code empty) became ~30M
      # doubles each and took R past its 24 GB vector limit in
      # [pts_distance_from_lnst] (run of 2026-09-23). An all-empty column carries no
      # flag, and [meas_col_registry] adds any flag column still missing as NULL
      # before the COALESCE, so dropping it loses nothing.
      q_new <- c("temp_ave_q", "salt1_corr_q", "salt2_corr_q", "salt_ave_corr_q",
                 "ox1_cruise_corr_q", "ox1_sta_corr_q", "ox2_cruise_corr_q",
                 "ox2_sta_corr_q", "ox_ave_sta_corr_q", "ox1u_mq", "ox1u_m_cruise_corr_q",
                 "ox1u_m_sta_corr_q", "ox2u_mq", "ox2u_m_cruise_corr_q", "ox2u_m_sta_corr_q",
                 "ox_aveu_m_sta_corr_q", "est_chl_cruise_corr_q", "est_chl_sta_corr_q",
                 "est_no3_cruise_corr_q", "est_no3_sta_corr_q", "batq", "po_t1q", "po_t2q",
                 "dyn_ht_q", "svaq", "ox_sat1q", "ox_sat2q")
      q_empty <- intersect(q_new, names(data))[
        vapply(intersect(q_new, names(data)), \(c) all(is.na(data[[c]])), logical(1))]
      data |>
        mutate(across(where(is.logical) & matches("q$"), as.numeric)) |>
        # the layout marker survives the drop (the cross-check needs it)
        mutate(.src_qc_2026 = "salt_ave_corr_q" %in% names(data)) |>
        select(-any_of(q_empty))
    }),
    nrows = map_int(data, ~ if (is.null(.x)) 0 else nrow(.x)),
    # the file layout: the CTD team's 2026 processing software writes a quality
    # code for every corrected, averaged and derived series (SaltAve_CorrQ,
    # OxAve_StaCorrQ, EstChl_StaCorrQ, …; CalCOFI/workflows#105), the legacy
    # layout flags only the sensors. The cross-check in [combine_sensor_pairs]
    # reads it.
    src_format = map_chr(data, ~ if (isTRUE(.x$.src_qc_2026[1])) "qc_2026" else "legacy"),
    data = map(data, ~ select(.x, -.src_qc_2026))
  )

say(glue("file layouts: {paste(names(table(d_csv$src_format)), table(d_csv$src_format), sep = ' = ', collapse = ', ')}"))
write_rds(d_csv |> transmute(`_source_file` = path_unzip, src_format),
          glue("{dir_tmp}/d_src_format.rds"))

d_csv |>
  arrange(cruise_key, file_csv) |>
  select(cruise_key, file_csv, nrows, src_format) |>
  dt(
...
# --- station numbers cut to three digits: repaired by the cast's own position ---
# The CTD team's preliminary files for 2507SR, 2511SR, 2601RL, 2604SH and 2607SH
# wrote station numbers >= 100 with three digits: 100/110/120 read as 000/010/020
# (`Sta` "000.0", `Sta_ID` "090.0 000.0"). The distance filter then dropped every
# such cast as a position error, so those cruises lost their offshore stations in
# every release through v2026.09.11 (CalCOFI/workflows#104). The provider re-issued
# 2607 on 2026-09-14, and the ONLY difference is `Sta`/`Sta_ID` = station + 100; the
# other four are not re-issued yet (questions.csv Q39). So repair exactly that, and
# only where the cast's recorded GPS position says so: an inshore number (< 30)
# whose cast lies within CTD_TRUNC_KM of station + 100 and farther than that from
# the station as written. Every repair is listed below; [filter_pts] still stops the
# render if a truncated station survives unrepaired.
CTD_TRUNC_KM <- 10   # the distance filter's own cutoff (max_dist_dec_lnst_km)
sta_num  <- suppressWarnings(as.numeric(d_bind$sta))
line_num <- suppressWarnings(as.numeric(d_bind$line))
i_cand <- which(!is.na(sta_num) & !is.na(line_num) & sta_num < 30 & line_num > 0 &
                  !is.na(d_bind$longitude) & !is.na(d_bind$latitude) &
                  d_bind$longitude > -180 & d_bind$latitude > -90)
d_trunc_fix <- tibble(
  .i = i_cand, src = d_bind$`_source_file`[i_cand], cast_id = d_bind$cast_id[i_cand],
  line = line_num[i_cand], sta = sta_num[i_cand],
  lon = d_bind$longitude[i_cand], lat = d_bind$latitude[i_cand]) |>
  group_by(src, cast_id, line, sta) |>
  summarize(lon = median(lon), lat = median(lat), n_scans = n(), .groups = "drop")
if (nrow(d_trunc_fix) > 0) {
  nominal <- function(line, sta) sf_project(
    from = "+proj=calcofi", to = "+proj=longlat +datum=WGS84", pts = cbind(line, sta))
  ...
  xy0   <- nominal(d_trunc_fix$line, d_trunc_fix$sta)
  xy100 <- nominal(d_trunc_fix$line, d_trunc_fix$sta + 100)
  d_trunc_fix <- d_trunc_fix |>
    mutate(km_as_written = hav_km(lon, lat, xy0[, 1], xy0[, 2]),
           km_plus_100   = hav_km(lon, lat, xy100[, 1], xy100[, 2])) |>
    filter(km_plus_100 <= CTD_TRUNC_KM, km_as_written > CTD_TRUNC_KM)
}
if (nrow(d_trunc_fix) > 0) {
  key_fix <- paste(d_trunc_fix$src, d_trunc_fix$cast_id, d_trunc_fix$line, d_trunc_fix$sta)
  i_fix   <- i_cand[paste(d_bind$`_source_file`[i_cand], d_bind$cast_id[i_cand],
                          line_num[i_cand], sta_num[i_cand]) %in% key_fix]
  sta_new <- sta_num[i_fix] + 100
  d_bind$sta[i_fix] <- if (is.character(d_bind$sta)) sprintf("%05.1f", sta_new) else sta_new
  d_bind$sta_id[i_fix] <- sprintf("%05.1f %05.1f", line_num[i_fix], sta_new)
  say(glue("station numbers repaired (+100, confirmed by position): ",
           "{nrow(d_trunc_fix)} casts, {format(length(i_fix), big.mark = ',')} scans, ",
           "files: {paste(unique(basename(d_trunc_fix$src)), collapse = ', ')}"))
  write_rds(d_trunc_fix, glue("{dir_tmp}/d_station_truncation_repaired.rds"))
  d_trunc_fix |>
    mutate(file = basename(src), sta_repaired = sta + 100) |>
    select(file, cast_id, line, sta_as_written = sta, sta_repaired, n_scans,
           km_as_written, km_plus_100) |>
    dt(caption = paste(
      "Station numbers cut to three digits by the source, repaired to station + 100",
      "where the cast's recorded position confirms it (CalCOFI/workflows#104)"),
      fname = "ctd_station_truncation_repaired")
} else say("no truncated station numbers found")
rm(sta_num, line_num, i_cand)
...
# A scan with no position AND no CalCOFI line/station to derive one from cannot
# be placed, so it cannot enter `sample`/`obs` (both key on a position). Until
# 2026-08-24 this was a bare hard stop, which held for years because every such
# scan had a line/station. Then calcofi.org's 19-9604JD and 19-9608NH FinalQC
# archives (fetched for the first time that day; JRW's FinalDB copies omit them)
# brought 9,225 scans from the "RATHBURN CORE STN" canyon site: lat/lon -99 and
# the station NAME in the line/sta columns. Those are not CalCOFI grid casts
# (questions.csv Q, "Rathburn"). Report them by file and drop them; the assertion
# below still holds for everything placeable, and the drop is bounded so an
# archive that loses its positions wholesale fails loudly rather than vanishing.
CTD_UNPLACED_MAX_PCT <- 0.2   # 9,225 of 9,135,263 = 0.10% at 2026-08-24; only ever DOWN

d_unplaced <- d_bind |>
  filter(is.na(longitude) | is.na(latitude))
if (nrow(d_unplaced) > 0) {
  d_unplaced |>
    count(cruise_key, `_source_file`, line, sta, name = "n_scans") |>
    arrange(desc(n_scans)) |>
    dt(
      caption = glue(
        "{format(nrow(d_unplaced), big.mark = ',')} scans DROPPED: no position and ",
        "no line/station to derive one from"),
      fname = "ctd_scans_unplaced")
}
stopifnot(
  "unplaceable scans exceed the ratchet: a source lost its positions; investigate before dropping" =
    100 * nrow(d_unplaced) / nrow(d_bind) <= CTD_UNPLACED_MAX_PCT)
d_bind <- d_bind |>
  filter(!is.na(longitude), !is.na(latitude))
...
# only the columns dist_one_cruise() reads: group_split() COPIES what it splits, and
# splitting the whole ~110-column frame doubled R's peak (2026-09-23: 24 GB limit)
d_dist <- d_bind |>
  select(.row_id, cruise_key, longitude, latitude, lon_lnst, lat_lnst) |>
...
# A station number written with only three digits is not "far from its station",
# it is a different station: 100/110/120 shipped as 000.0/010.0/020.0 in five
# preliminary files (CalCOFI/workflows#104), and the distance filter above dropped
# every such cast as a position error — quietly, one row in a table of cruises.
# [fill_lonlat] repairs the ones the cast's position confirms; this is the backstop:
# any dropped scan that still carries the signature (an inshore number < 30 whose
# cast sits within the cutoff of station + 100) stops the render.
d_trunc <- pts |>
  filter(!is_dist_dec_lnst_within_max, !is.na(sta), !is.na(line),
         suppressWarnings(as.numeric(sta)) < 30) |>
  distinct(cruise_key, `_source_file`, line, sta, longitude, latitude)
if (nrow(d_trunc) > 0) {
  xy100 <- sf_project(
    from = "+proj=calcofi", to = "+proj=longlat +datum=WGS84",
    pts  = cbind(as.numeric(d_trunc$line), as.numeric(d_trunc$sta) + 100))
  hav_km <- function(lon1, lat1, lon2, lat2) {
    r <- pi / 180
    a <- sin((lat2 - lat1) * r / 2)^2 +
      cos(lat1 * r) * cos(lat2 * r) * sin((lon2 - lon1) * r / 2)^2
  ...
  d_trunc$dist_sta_plus100_km <- hav_km(d_trunc$longitude, d_trunc$latitude,
                                        xy100[, 1], xy100[, 2])
  d_trunc <- filter(d_trunc, dist_sta_plus100_km <= max_dist_dec_lnst_km)
}
if (nrow(d_trunc) > 0)
  stop(glue(
    "{nrow(distinct(d_trunc, `_source_file`, line, sta))} station(s) look TRUNCATED to three ",
    "digits (the cast sits at station + 100): ",
    paste(unique(glue("{basename(d_trunc$`_source_file`)} {d_trunc$line}/{d_trunc$sta}")),
          collapse = "; "),
    ". Ask the CTD team for a corrected file; do not widen the distance filter."))
...
mapView(bb_cr) +
  mapView(
    # 20 per cruise, not 100: at 100 the widget reached 11.7 MB on ONE line, and pandoc
    # sat on it for 45+ minutes at full CPU (2026-09-24) after every R chunk had finished
    pts_filt |>
      group_by(cruise_key) |>
      slice_sample(n = 20) |>
      ungroup() |>
...
# which layout each source file is in, for the provider-average cross-check
dbWriteTable(con, "ctd_src_format", read_rds(glue("{dir_tmp}/d_src_format.rds")),
             overwrite = TRUE)

# The scans live in DuckDB from here on. Release R's copies: d_bind, pts, pts_filt and
# d_filt are each a ~30M-row frame, and keeping them made the R process 35 GB by
# [add_spatial] on 2026-09-24, with DuckDB's own 2 GB on top, until the machine ran out
# of swap. Nothing after this point reads them.
rm(list = intersect(c("d_bind", "pts", "pts_filt", "d_filt", "d_csv"), ls()))
invisible(gc())
...
if (!file_exists(db_checkpoint) || overwrite) {
  ...
  write_input_fingerprint(checkpoint_fp_path, fp_zips)
  con <- get_duckdb_con(db_path)
  duck_budget(con)
  load_duckdb_extension(con, "spatial")
  load_duckdb_extension(con, "icu")
...
# --- every series names the PROVIDER'S OWN flag column (CalCOFI/workflows#105) ---
# The 2026 processing software flags each corrected, averaged and derived series
# (SaltAve_CorrQ, EstChl_StaCorrQ, …) and the registry now points each series at that
# column (libs/build_ctd_measurement_registry.R § 4). A legacy-layout file has no such
# column, so fill it from the sensor flag the series inherited before 2026-09-23:
# COALESCE(provider flag, sensor flag) — the provider's word wherever it has one, and
# never weaker than the old rule where it does not. An average has no sensor to fall
# back on (NA): it is rebuilt from its pair in [combine_sensor_pairs] and keeps the
# provider's flag on it only as the provider's own verdict.
qual_fallback <- c(
  temp_ave_q            = NA,        salt_ave_corr_q       = NA,
  salt1_corr_q          = "salt1q",  salt2_corr_q          = "salt2q",
  ox1_sta_corr_q        = "ox1q",    ox2_sta_corr_q        = "ox2q",
  ox1_cruise_corr_q     = "ox1q",    ox2_cruise_corr_q     = "ox2q",
  ox_ave_sta_corr_q     = NA,
  ox1u_mq               = "ox1q",    ox2u_mq               = "ox2q",
  ox1u_m_sta_corr_q     = "ox1q",    ox2u_m_sta_corr_q     = "ox2q",
  ox1u_m_cruise_corr_q  = "ox1q",    ox2u_m_cruise_corr_q  = "ox2q",
  ox_aveu_m_sta_corr_q  = NA,
  est_chl_sta_corr_q    = "fluor_q", est_chl_cruise_corr_q = "fluor_q",
  est_no3_sta_corr_q    = "isusq",   est_no3_cruise_corr_q = "isusq",
  batq                  = NA,        dyn_ht_q              = NA,
  svaq                  = NA,
  po_t1q                = "temp1q",  po_t2q                = "temp2q",
  ox_sat1q              = "ox1q",    ox_sat2q              = "ox2q")
stopifnot(
  "every provider flag column the registry names has a fallback rule here" =
    all(setdiff(na.omit(d_meas_ctd$`_qual_column`),
                c("", "pr_q", "temp1q", "temp2q", "salt1q", "salt2q", "ox1q", "ox2q",
                  "fluor_q", "isusq", "sig_theta_ts1q", "sig_theta_ts2q", "trans_q",
                  "p_hq", "sparq", "parq")) %in% names(qual_fallback)))
for (q in names(qual_fallback))
  dbExecute(con, glue("ALTER TABLE ctd_raw ADD COLUMN IF NOT EXISTS {q} DOUBLE"))
q_fill <- qual_fallback[!is.na(qual_fallback)]
dbExecute(con, glue("UPDATE ctd_raw SET {paste0(names(q_fill), ' = COALESCE(TRY_CAST(', names(q_fill), ' AS DOUBLE), TRY_CAST(', q_fill, ' AS DOUBLE))', collapse = ', ')}"))
cat(glue("provider flag columns: {length(qual_fallback)} ({length(q_fill)} filled from their sensor flag where the file predates them)\n"))
meas_cols <- d_meas_ctd$`_source_column` |> na.omit() |> as.character()
meas_cols <- meas_cols[meas_cols != ""]
...
# --- the two-sensor averages are NOT repaired here -----------------------------
# The source's pre-computed averages (TempAve, SaltAve_Corr, OxAve_StaCorr,
# OxAveuM_StaCorr) are rebuilt from their two sensors by the PROVIDER'S FLAGS in
# § Combine the Sensor Pairs by Their Flags, after the CTD team's accepted flags are
# applied — Rasmus Swalethorp's rule (2026-09-09), calcofi4db::combine_sensor_pair().
# Until 2026-09-11 this spot "repaired" them by the registry's physical bounds alone,
# which let a sensor the source flags 8 (questionable) or 9 (bad) into the average
# whenever its value was physically possible (1,425 temperature and 6,001 salinity
# averages in v2026.09.10) and never saw the accepted flags at all. A physical bound
# is a sentinel guard, not a quality test.
...
for (i in seq_len(nrow(d_meas_ctd))) {
  ...
  # a series the ingest BUILDS (oxygen_ml_l_ave_cruise_corr, from its sensor pair in
  # [combine_sensor_pairs]) has no source column: nothing to pivot
  if (is.na(src_col) || src_col == "") next
...
# --- declare the PAR sensors' physical bounds ---------------------------------
# `par` and `spar` were two of the nine CTD types with no bound, and v2026.09.06
# published what that leaves in at the release grain: `spar` from -3.07e17 to
# 2.01e16 µE/m²/s (31,623 values on ten cruises, 1995–2008 — 6% of the series) and
# `par` with 34,233 values at exactly 9999 — a fill, not a reading — plus 1,065 more
# above 5,000 and eight below -100 (24 cruises, 1994–2025). Photosynthetically
# available radiation at the sea surface cannot exceed the ~2,500 µE/m²/s the sun
# delivers at the top of the atmosphere, and a dark reading sits within a few units
# of zero, so -100…5,000 catches only the impossible (Ben, 2026-09-10: a certain
# bug, not a question). Idempotent, as the bottle's § Declare and Enforce Physical
# Bounds is.
ctd_bounds <- tibble::tribble(
  ~measurement_type, ~valid_min, ~valid_max,
  "par",                   -100,       5000,
  "spar",                  -100,       5000)
d_meas_type <- calcofi4db::declare_measurement_bounds(
  ctd_bounds, here("metadata/measurement_type.csv"))

bounds_pre <- check_measurement_bounds(
  con, "ctd_measurement", mt = d_meas_type)

# report BEFORE deleting: `undeclared` is a finding too, and the seven CTD types
# with no bound are the ones still to take to the provider
...
# the guard must leave nothing behind: a violation here means the delete and the
# check disagree on a bound
bounds_post <- check_measurement_bounds(
  con, "ctd_measurement", mt = d_meas_type, include_undeclared = FALSE)
stopifnot(
  "ctd_measurement must hold no value outside a declared bound" =
    sum(bounds_post$status == "out_of_range") == 0)
...
## Apply Accepted QC Flags

The CTD team's PostgreSQL QA/QC database (`server/postgis/init/40_ctd.sql`, schema
`ctd`) is a *working* store beside this release, never a dependency this
pipeline connects to live — the bridge is a nightly snapshot,
`gs://calcofi-db/qc/ctd/flag_accepted.parquet`
(`server/scripts/pg_flag_snapshot.R`, cron `pg_flag_snapshot.sh`), of every
`ctd.flag` row a curator has accepted. This chunk downloads that snapshot,
joins each accepted flag to the scan it names, and overwrites
`ctd_measurement.measurement_qual` for the match.

**Join key.** `ctd.file.archive` is the zip filename and `.path` is the member
path inside it (`libs/pg_ctd.R::pg_ctd_discover_files()`:
`archive = paste0(dir_unzip, ".zip")`, `path = rel`) — byte-identical to this
notebook's own `` `_source_file` `` (the `d_csv` chunk's `path_unzip`), so
`archive` reconstructs from it as the text before the first `/` plus `.zip`.
Within one file, `ctd.scan.cast_id` / `.depth` are the *verbatim* source
`Cast_ID` / `Depth` columns — `flds_redefine.csv` renames those to this
notebook's `cast_key` / `depth_m`, so `(archive, _source_file, cast_key,
depth_m)` is the same identity as `(archive, path, cast_id, depth)` in the
snapshot. `ctd.flag.variable` is a `ctd.scan` **column name** (e.g.
`salt1_corr`), not a `measurement_type` — mapped through `` `_source_column` ``
on `d_meas_ctd` (built above in "Measurement Column Registry").

**Two flag shapes.** A scan-level flag carries `depth`/`cast_id` (from the
snapshot's `LEFT JOIN ctd.scan`); a file/cast-level flag (`ctd.flag.scan_id IS
NULL`, the whole file or cast is being marked) does not, and applies to every
scan in that file — for `variable = '*'` that means every measurement type,
not one.

```{r}
#| label: apply_accepted_flags

flag_url   <- "https://storage.googleapis.com/calcofi-db/qc/ctd/flag_accepted.parquet"
flag_cache_dir <- here("data/cache/ctd-cast")
dir_create(flag_cache_dir)
flag_cache_path <- file.path(flag_cache_dir, "flag_accepted.parquet")

# download-first, skip when unchanged: a HEAD request's Content-Length is
# compared to the cached file's size before spending the GET. A size collision
# with genuinely different content is not a concern here — the snapshot is
# COPY'd fresh each night (server/scripts/pg_flag_snapshot.R), never edited in
# place, so "same size" only happens when the file legitimately did not change.
flag_head <- tryCatch(httr::HEAD(flag_url), error = function(e) NULL)
remote_size <- if (!is.null(flag_head) && httr::status_code(flag_head) == 200)
  suppressWarnings(as.numeric(httr::headers(flag_head)[["content-length"]])) else NA_real_
remote_last_modified <- if (!is.null(flag_head))
  httr::headers(flag_head)[["last-modified"]] else NA_character_

local_size <- if (file_exists(flag_cache_path)) as.numeric(file_size(flag_cache_path)) else NA_real_

need_download <- is.na(local_size) ||
  (!is.na(remote_size) && local_size != remote_size)

if (need_download) {
  stopifnot("could not reach flag_accepted.parquet and no local cache exists" =
              !is.null(flag_head) && httr::status_code(flag_head) == 200)
  utils::download.file(flag_url, flag_cache_path, mode = "wb", quiet = TRUE)
  say(glue("Downloaded flag_accepted.parquet ({round(file_size(flag_cache_path) / 1024, 1)} KB, ",
           "last modified {remote_last_modified %||% 'unknown'})"))
} else {
  say(glue("flag_accepted.parquet unchanged ({round(local_size / 1024, 1)} KB) — using cache"))
}

d_flags <- dbGetQuery(con, glue("SELECT * FROM read_parquet('{flag_cache_path}')"))
n_flags <- nrow(d_flags)

if (n_flags == 0) {

  say(glue("0 accepted flags in snapshot (last modified {remote_last_modified %||% 'unknown'})"))

} else {

  # variable (a ctd.scan column name, e.g. "salt1_corr") -> measurement_type,
  # via the same crosswalk the pivot above used (d_meas_ctd$`_source_column`)
  d_var_xwalk <- d_meas_ctd |>
    select(variable = `_source_column`, measurement_type) |>
    filter(!is.na(variable), variable != "")
  dbWriteTable(con, "_flag_var_xwalk", d_var_xwalk, overwrite = TRUE)
  dbWriteTable(con, "_flags_accepted", d_flags, overwrite = TRUE)

  # the same (archive, _source_file, cast_key, depth_m) identity as ctd.file /
  # ctd.scan (see the section prose above), one row per physical scan
  dbExecute(con, "
    CREATE OR REPLACE TABLE _ctd_raw_keys AS
    SELECT DISTINCT ctd_cast_uuid, depth_m, cast_key, \"_source_file\",
           regexp_extract(\"_source_file\", '^([^/]+)', 1) || '.zip' AS archive
    FROM ctd_raw")
  stopifnot("(archive, _source_file, cast_key, depth_m) is not a unique scan key in ctd_raw" =
    dbGetQuery(con, "
      SELECT COUNT(*) - COUNT(DISTINCT archive || '|' || \"_source_file\" || '|' || cast_key || '|' || depth_m) AS n
      FROM _ctd_raw_keys")$n == 0)

  # scan-level flags: exact (archive, path, cast_id, depth) match
  dbExecute(con, "
    CREATE OR REPLACE TABLE _flags_scan_matched AS
    SELECT f.flag_id, f.qual_code, f.variable, k.ctd_cast_uuid, k.depth_m
    FROM _flags_accepted f
    JOIN _ctd_raw_keys k
      ON k.archive = f.archive AND k.\"_source_file\" = f.path
     AND k.cast_key = f.cast_id AND k.depth_m = f.depth
    WHERE f.depth IS NOT NULL")

  # file/cast-level flags (no scan_id in the ledger — every scan in the file)
  dbExecute(con, "
    CREATE OR REPLACE TABLE _flags_file_matched AS
    SELECT f.flag_id, f.qual_code, f.variable, k.ctd_cast_uuid, k.depth_m
    FROM _flags_accepted f
    JOIN _ctd_raw_keys k
      ON k.archive = f.archive AND k.\"_source_file\" = f.path
    WHERE f.depth IS NULL")

  n_scan_flags <- dbGetQuery(con, "SELECT COUNT(DISTINCT flag_id) AS n FROM _flags_accepted WHERE depth IS NOT NULL")$n
  n_file_flags <- n_flags - n_scan_flags
  n_scan_matched_flags <- dbGetQuery(con, "SELECT COUNT(DISTINCT flag_id) AS n FROM _flags_scan_matched")$n
  n_file_matched_flags <- dbGetQuery(con, "SELECT COUNT(DISTINCT flag_id) AS n FROM _flags_file_matched")$n
  n_unmatched <- (n_scan_flags - n_scan_matched_flags) + (n_file_flags - n_file_matched_flags)

  stopifnot(
    "accepted flag(s) in the snapshot find no scan in ctd_raw — a broken (archive, path, cast_id, depth) bridge" =
      n_unmatched == 0)

  # apply: named-variable rows go through the crosswalk; '*' applies to every
  # measurement_type recorded for that (ctd_cast_uuid, depth_m)
  dbExecute(con, "
    UPDATE ctd_measurement m
    SET measurement_qual = CAST(x.qual_code AS VARCHAR)
    FROM (
      SELECT fm.ctd_cast_uuid, fm.depth_m, w.measurement_type, fm.qual_code
      FROM (SELECT * FROM _flags_scan_matched UNION ALL SELECT * FROM _flags_file_matched) fm
      JOIN _flag_var_xwalk w ON w.variable = fm.variable
      WHERE fm.variable != '*'
      UNION ALL
      SELECT fm.ctd_cast_uuid, fm.depth_m, m2.measurement_type, fm.qual_code
      FROM (SELECT * FROM _flags_scan_matched UNION ALL SELECT * FROM _flags_file_matched) fm
      JOIN ctd_measurement m2
        ON m2.ctd_cast_uuid = fm.ctd_cast_uuid AND m2.depth_m = fm.depth_m
      WHERE fm.variable = '*'
    ) x
    WHERE m.ctd_cast_uuid = x.ctd_cast_uuid AND m.depth_m = x.depth_m
      AND m.measurement_type = x.measurement_type")

  n_scans_affected <- dbGetQuery(con, "
    SELECT COUNT(DISTINCT ctd_cast_uuid || '|' || depth_m) AS n
    FROM (SELECT * FROM _flags_scan_matched UNION ALL SELECT * FROM _flags_file_matched)")$n

  say(glue("accepted CTD QC flags applied: {format(n_flags, big.mark = ',')} flag(s) · ",
           "{format(n_scans_affected, big.mark = ',')} scan(s) affected · 0 unmatched"))

  n_flags_applied <- n_flags  # exported into metadata.json below, for release_database.qmd's n_flags_pending
  dbExecute(con, "DROP TABLE _flag_var_xwalk")
  dbExecute(con, "DROP TABLE _flags_accepted")
  dbExecute(con, "DROP TABLE _ctd_raw_keys")
  dbExecute(con, "DROP TABLE _flags_scan_matched")
  dbExecute(con, "DROP TABLE _flags_file_matched")
}
```

::: {.callout-note}
This chunk is shipping **unrun** this round (2026-09-03): the snapshot is a
600-byte header-only parquet with zero accepted rows (last modified
2026-08-19), so the CTD team has not yet accepted a flag through the ledger.
It first has an effect at the next CTD ingest render after the team accepts
one. `RELEASES.md` gets an entry only once `n_flags_applied` > 0 — a
zero-effect chunk shipping is not itself a release-content change.
:::

## Apply Flag Overrides

The accepted-flag ledger above holds flags a CTD curator has entered against individual scans in
the team's PostgreSQL database. Some flags are known before anyone enters them, most often because
the provider's own cruise notes say a sensor failed. `metadata/calcofi/ctd-cast/flag_overrides.csv`
is where those live: one committed row per (cruise, cast range, direction, series), each with the
flag, the reason, the source it rests on and the question that keeps it provisional until the
provider flags at source. It is applied **after** the provider's flags and the accepted ledger and
**before** any average, so an overridden sensor can never enter `temperature_ave` or anything built
on it: the climatology, the transect shards, `measurements.json`, the derived dataset.

The first entry (2026-09-24, questions.csv Q37) is cruise 2026-07-3322 casts 1–6. The provider's
notes say the secondary temperature sensor was faulty there, and the data agree: sensor 2 sits a
median 11 °C from sensor 1, up to 42 °C, against 0.002 °C on casts 7–72. Every series the provider
builds from that temperature is flagged with it: conductivity-derived salinity, sigma-theta,
potential temperature, and the sensor-2 oxygens, since oxygen concentration and saturation use the
sensor's own temperature and salinity. The corrected forms of those series are included so the rule
still holds when a bottle-merged file for the cruise arrives.

```{r}
#| label: apply_flag_overrides

path_overrides <- file.path(dir_meta, "flag_overrides.csv")
d_overrides <- read_csv(path_overrides, show_col_types = FALSE, na = "",
                        col_types = cols(cast_min = "i", cast_max = "i", qual_code = "c",
                                         .default = "c"))
stopifnot(
  "flag_overrides.csv: every row needs a reason, a source and a question" =
    all(nzchar(d_overrides$reason) & nzchar(d_overrides$source) & nzchar(d_overrides$question)),
  "flag_overrides.csv: cast_dir is D, U or *" = all(d_overrides$cast_dir %in% c("D", "U", "*")),
  "flag_overrides.csv: measurement_type must be a registered CTD type" =
    all(d_overrides$measurement_type %in% d_meas_ctd$measurement_type))

dbWriteTable(con, "_flag_overrides", d_overrides, overwrite = TRUE)
dbExecute(con, "
  CREATE OR REPLACE TEMP TABLE _override_hits AS
  SELECT o.cruise_key, o.cast_min, o.cast_max, o.cast_dir, o.measurement_type, o.qual_code,
         c.cast_key, m.ctd_cast_uuid, m.depth_m
  FROM _flag_overrides o
  JOIN ctd_cast c
    ON c.cruise_key = o.cruise_key
   AND (o.cast_dir = '*' OR c.cast_dir = o.cast_dir)
   AND TRY_CAST(regexp_extract(c.cast_key, '_([0-9]{3})', 1) AS INTEGER)
         BETWEEN o.cast_min AND o.cast_max
  JOIN ctd_measurement m
    ON m.ctd_cast_uuid = c.ctd_cast_uuid AND m.measurement_type = o.measurement_type")
dbExecute(con, "
  UPDATE ctd_measurement m SET measurement_qual = h.qual_code
  FROM _override_hits h
  WHERE m.ctd_cast_uuid = h.ctd_cast_uuid AND m.depth_m = h.depth_m
    AND m.measurement_type = h.measurement_type")

d_override_report <- d_overrides |>
  left_join(dbGetQuery(con, "
    SELECT cruise_key, cast_min, cast_max, cast_dir, measurement_type,
           COUNT(*) AS n_flagged, COUNT(DISTINCT cast_key) AS n_casts
    FROM _override_hits GROUP BY ALL"),
    by = c("cruise_key", "cast_min", "cast_max", "cast_dir", "measurement_type")) |>
  mutate(across(c(n_flagged, n_casts), \(x) coalesce(as.integer(x), 0L)))
# a series the cruise's file does not carry yet (the corrected forms, before the bottle merge)
# matches nothing, which is fine; an override whose FIRST series matches nothing is a broken
# key, and so is an override row whose cruise has no casts at all in that range
stopifnot("a flag override matches no cast: check cruise_key / cast range / direction" =
  all(d_override_report |> group_by(cruise_key, cast_min, cast_max, cast_dir) |>
        summarize(n = sum(n_flagged), .groups = "drop") |> pull(n) > 0))
dbExecute(con, "DROP TABLE _flag_overrides")
say(glue("flag overrides applied: {nrow(d_overrides)} rule(s), ",
         "{format(sum(d_override_report$n_flagged), big.mark = ',')} value(s) flagged"))
datatable(d_override_report |> select(cruise_key, cast_min, cast_max, cast_dir, measurement_type,
                                      qual_code, n_casts, n_flagged, reason, question),
          rownames = FALSE, options = list(pageLength = 12, scrollX = TRUE),
          caption = "Flag overrides (metadata/calcofi/ctd-cast/flag_overrides.csv) and what each flagged")
```

## Combine the Sensor Pairs by Their Flags

The canonical CTD temperature, salinity and oxygen are the **average of a sensor
pair**, and the source ships that average pre-computed. It is not trustworthy as
shipped: where the second sensor is missing, `SaltAve_Corr` and `TempAve` are
sometimes exactly half of sensor 1 (1998-07-32NM, 2024-01-33UD: salinity ≈ 17 at
1,000 m beside a sensor reading 34), `TempAve` reads 35–40 °C at 1–2 m on
2003-04-31JD beside a sensor reading 12, and none of these carries a flag. So every
average is **rebuilt here from its two sensors**, by the rule Rasmus Swalethorp set
for this project (2026-09-09), implemented and tested as
`calcofi4db::combine_sensor_pair()`:

1. a sensor flagged **8** (questionable) or **9** (bad) is dropped;
2. a **1** (use primary) or **2** (use secondary) selects that sensor alone;
3. otherwise the **mean** of the two, **one alone** when the other is missing, and
   nothing when neither survives.

The flags are each sensor's own (`T1Q`/`T2Q`, `S1Q`/`S2Q`, `Ox1Q`/`Ox2Q`), which the
corrected series inherit from their sensor (`libs/build_ctd_measurement_registry.R`
§ 4), **after** the CTD team's accepted flags (§ above) have been written onto them.
A sensor value removed above as a `-99` sentinel or outside its physical bound is
absent here, which the rule treats like a 9. A physical bound is only that guard: it
never decides that a flagged value is good.

Every average is rebuilt, oxygen included, and an average is written wherever a
surviving sensor exists even if the source shipped none (Ben, 2026-09-11: Rasmus is
the authority on the rule). The source's own average survives only on a scan where the
source shipped **no sensor value at all** — there is nothing to rebuild it from.

```{r}
#| label: combine_sensor_pairs

pair_rule <- tibble::tribble(
  ~ave,                          ~s1,                          ~s2,                          ~raw1,              ~raw2,
  "temperature_ave",             "temperature_1",              "temperature_2",              "temp1",            "temp2",
  "salinity_ave_corr",           "salinity_1_corr",            "salinity_2_corr",            "salt1_corr",       "salt2_corr",
  "oxygen_ml_l_ave_sta_corr",    "oxygen_ml_l_1_sta_corr",     "oxygen_ml_l_2_sta_corr",     "ox1_sta_corr",     "ox2_sta_corr",
  # the source ships no cruise-corrected average; built here (CalCOFI/workflows#106)
  "oxygen_ml_l_ave_cruise_corr", "oxygen_ml_l_1_cruise_corr",  "oxygen_ml_l_2_cruise_corr",  "ox1_cruise_corr",  "ox2_cruise_corr",
  "oxygen_umol_kg_ave_sta_corr", "oxygen_umol_kg_1_sta_corr",  "oxygen_umol_kg_2_sta_corr",  "ox1u_m_sta_corr",  "ox2u_m_sta_corr")

# a sensor series with no flag column would make the rule silently flag-blind
qual_cols <- setNames(d_meas_ctd$`_qual_column`, d_meas_ctd$measurement_type)
stopifnot("every paired sensor series must carry its sensor's flag column" =
            all(!is.na(qual_cols[c(pair_rule$s1, pair_rule$s2)]) &
                  qual_cols[c(pair_rule$s1, pair_rule$s2)] != ""))

combined_sql <- calcofi4db::combine_sensor_pair_sql("p.v1", "p.v2", "p.q1", "p.q2")
q89 <- function(q) glue("regexp_replace(CAST({q} AS VARCHAR), '\\.0+$', '') IN ('8', '9')")
# the same md5 key assign_deterministic_uuids_md5() uses, so a rebuilt row keeps its id
uuid_sql <- "substr(_h, 1, 8) || '-' || substr(_h, 9, 4) || '-' || substr(_h, 13, 4) || '-' ||
             substr(_h, 17, 4) || '-' || substr(_h, 21, 12)"

# the file layout per source file ([rename_fields]); a checkpoint from before
# 2026-09-23 has no such table, which the cross-check reports as 'unknown'
src_format_tbl <- if ("ctd_src_format" %in% dbListTables(con)) "ctd_src_format" else
  "(SELECT CAST(NULL AS VARCHAR) AS _source_file, CAST(NULL AS VARCHAR) AS src_format)"

# the provider's own average, beside ours: agreement per layout. The rule is ours
# (Rasmus, 2026-09-09) and it stands; this table is how a disagreement reaches the
# CTD team (Ben Gire, Kelsey Vogel) as a QC finding instead of being a silent choice.
pair_xcheck <- list()

pair_report <- pmap_dfr(pair_rule, function(ave, s1, s2, raw1, raw2) {
  dbExecute(con, glue("
    CREATE OR REPLACE TEMP TABLE _pair AS
    WITH m AS (
      SELECT ctd_cast_uuid, depth_m,
        any_value(cruise_key) AS cruise_key,
        max(measurement_value) FILTER (WHERE measurement_type = '{s1}')  AS v1,
        max(measurement_qual)  FILTER (WHERE measurement_type = '{s1}')  AS q1,
        max(measurement_value) FILTER (WHERE measurement_type = '{s2}')  AS v2,
        max(measurement_qual)  FILTER (WHERE measurement_type = '{s2}')  AS q2,
        max(measurement_value) FILTER (WHERE measurement_type = '{ave}') AS v_src,
        max(measurement_qual)  FILTER (WHERE measurement_type = '{ave}') AS q_src
      FROM ctd_measurement
      WHERE measurement_type IN ('{s1}', '{s2}', '{ave}')
      GROUP BY ctd_cast_uuid, depth_m),
    r AS (
      -- did the SOURCE ship any sensor value on this scan (before sentinels and bounds)?
      SELECT x.ctd_cast_uuid, x.depth_m,
        bool_or({raw1} IS NOT NULL OR {raw2} IS NOT NULL) AS src_has_sensor,
        any_value(coalesce(f.src_format, 'unknown'))     AS src_format
      FROM ctd_raw x LEFT JOIN {src_format_tbl} f ON f._source_file = x._source_file
      GROUP BY x.ctd_cast_uuid, x.depth_m)
    SELECT m.*, coalesce(r.src_has_sensor, FALSE) AS src_has_sensor,
           coalesce(r.src_format, 'unknown') AS src_format
    FROM m LEFT JOIN r USING (ctd_cast_uuid, depth_m)"))

  dbExecute(con, glue("
    CREATE OR REPLACE TEMP TABLE _pair_new AS
    SELECT p.*,
      CASE WHEN p.src_has_sensor THEN {combined_sql} ELSE p.v_src END AS v_new
    FROM _pair p"))

  r <- dbGetQuery(con, glue("
    SELECT
      COUNT(*) FILTER (WHERE v_src IS NOT NULL)                                   AS n_shipped,
      COUNT(*) FILTER (WHERE v_new IS NOT NULL)                                   AS n_written,
      COUNT(*) FILTER (WHERE v_src IS NOT NULL AND v_new IS NOT NULL
                         AND abs(v_src - v_new) > 1e-4)                           AS n_changed,
      COUNT(*) FILTER (WHERE v_src IS NULL AND v_new IS NOT NULL)                 AS n_filled,
      COUNT(*) FILTER (WHERE v_src IS NOT NULL AND v_new IS NULL)                 AS n_removed,
      COUNT(*) FILTER (WHERE ({q89('q1')} AND v1 IS NOT NULL)
                          OR ({q89('q2')} AND v2 IS NOT NULL))                    AS n_flag_excluded,
      COUNT(*) FILTER (WHERE NOT src_has_sensor AND v_src IS NOT NULL)            AS n_src_only
    FROM _pair_new"))

  pair_xcheck[[ave]] <<- dbGetQuery(con, glue("
    SELECT '{ave}' AS measure, src_format,
      COUNT(*) FILTER (WHERE v_src IS NOT NULL AND v_new IS NOT NULL)             AS n_both,
      COUNT(*) FILTER (WHERE v_src IS NOT NULL AND v_new IS NOT NULL
                         AND abs(v_src - v_new) > 1e-4)                           AS n_disagree,
      max(abs(v_src - v_new)) FILTER (WHERE v_src IS NOT NULL AND v_new IS NOT NULL) AS max_abs_diff,
      COUNT(*) FILTER (WHERE {q89('q_src')})                                      AS n_provider_flag_89,
      COUNT(*) FILTER (WHERE {q89('q_src')} AND v_new IS NOT NULL)                AS n_provider_89_kept_flagged,
      COUNT(*) FILTER (WHERE v_src IS NOT NULL AND v_new IS NULL)                 AS n_provider_only
    FROM _pair_new GROUP BY src_format ORDER BY src_format"))

  dbExecute(con, glue("DELETE FROM ctd_measurement WHERE measurement_type = '{ave}'"))
  dbExecute(con, glue("
    INSERT INTO ctd_measurement BY NAME
    SELECT {uuid_sql} AS ctd_measurement_uuid,
           ctd_cast_uuid, depth_m, '{ave}' AS measurement_type,
           v_new AS measurement_value,
           -- the provider's verdict on ITS average (SaltAve_CorrQ, OxAve_StaCorrQ, …)
           -- rides on ours: a value it flags 8/9 stays flagged for every consumer
           -- that applies qual_ok, even though the pair rule rebuilt it
           q_src AS measurement_qual, cruise_key
    FROM (
      SELECT *, md5(concat_ws('|', COALESCE(CAST(ctd_cast_uuid AS VARCHAR), ''),
                                   COALESCE(CAST(depth_m AS VARCHAR), ''),
                                   '{ave}')) AS _h
      FROM _pair_new WHERE v_new IS NOT NULL)"))

  mutate(r, measure = ave, .before = 1)
})
dbExecute(con, "DROP TABLE IF EXISTS _pair")
dbExecute(con, "DROP TABLE IF EXISTS _pair_new")

say(glue("sensor pairs combined by their flags: ",
         "{format(sum(pair_report$n_written), big.mark = ',')} averages written, ",
         "{format(sum(pair_report$n_changed), big.mark = ',')} changed from the source's, ",
         "{format(sum(pair_report$n_filled), big.mark = ',')} filled where it shipped none, ",
         "{format(sum(pair_report$n_flag_excluded), big.mark = ',')} with a flagged sensor left out"))
datatable(pair_report, caption = "Averages rebuilt from their sensor pairs by the flag rule",
          rownames = FALSE)
pair_xcheck <- bind_rows(pair_xcheck)
write_csv(pair_xcheck, glue("{dir_tmp}/ctd_pair_xcheck.csv"), na = "")
datatable(pair_xcheck, rownames = FALSE, caption = paste(
  "Cross-check: our flag-rule average against the provider's own (SaltAve_Corr +",
  "SaltAve_CorrQ, OxAve_StaCorr + OxAve_StaCorrQ, …) per file layout. n_disagree > 0 is",
  "a QC finding for the CTD team, never a silent choice; ours is what is written."))

# the rebuilt averages must still sit inside their own bounds, and each scan has one
bounds_pairs <- check_measurement_bounds(
  con, "ctd_measurement", mt = d_meas_type, include_undeclared = FALSE)
stopifnot(
  "a rebuilt average fell outside its declared bound" =
    sum(bounds_pairs$status == "out_of_range" &
          bounds_pairs$measurement_type %in% pair_rule$ave) == 0,
  "one rebuilt average per scan" =
    dbGetQuery(con, glue("
      SELECT COUNT(*) - COUNT(DISTINCT ctd_cast_uuid || '|' || depth_m || '|' || measurement_type) AS n
      FROM ctd_measurement WHERE measurement_type IN ({paste0(\"'\", pair_rule$ave, \"'\", collapse = ', ')})"))$n == 0)

# and the rule itself, asserted on what was written: where a sensor is flagged 8/9 the
# average is the OTHER sensor (or absent) — never a mean that includes the flagged one
n_flag_leak <- sum(pmap_int(pair_rule, function(ave, s1, s2, raw1, raw2) {
  dbGetQuery(con, glue("
    WITH p AS (
      SELECT ctd_cast_uuid, depth_m,
        max(measurement_value) FILTER (WHERE measurement_type = '{s1}')  AS v1,
        ...
        max(measurement_value) FILTER (WHERE measurement_type = '{s2}')  AS v2,
        max(measurement_qual)  FILTER (WHERE measurement_type = '{s2}')  AS q2,
        max(measurement_value) FILTER (WHERE measurement_type = '{ave}') AS va
      FROM ctd_measurement WHERE measurement_type IN ('{s1}', '{s2}', '{ave}')
      GROUP BY ctd_cast_uuid, depth_m)
    SELECT COUNT(*) AS n FROM p
    WHERE va IS NOT NULL AND (
      ({q89('q1')} AND v1 IS NOT NULL AND (v2 IS NULL OR {q89('q2')} OR abs(va - v2) > 1e-9)) OR
      ({q89('q2')} AND v2 IS NOT NULL AND (v1 IS NULL OR {q89('q1')} OR abs(va - v1) > 1e-9)))"))$n
}))
stopifnot("an average includes a sensor its provider flags 8/9" = n_flag_leak == 0)

# and the overrides in particular: on every overridden cast the average is the OTHER sensor
# exactly — for Q37, temperature_ave on 2026-07-3322 casts 1-6 equals temperature_1
n_override_leak <- dbGetQuery(con, glue("
  WITH t AS (
    SELECT h.ctd_cast_uuid, h.depth_m
    FROM _override_hits h WHERE h.measurement_type = 'temperature_2'),
  p AS (
    SELECT m.ctd_cast_uuid, m.depth_m,
      max(measurement_value) FILTER (WHERE measurement_type = 'temperature_1')   AS v1,
      max(measurement_qual)  FILTER (WHERE measurement_type = 'temperature_1')   AS q1,
      max(measurement_value) FILTER (WHERE measurement_type = 'temperature_ave') AS va
    FROM ctd_measurement m JOIN t USING (ctd_cast_uuid, depth_m)
    WHERE measurement_type IN ('temperature_1', 'temperature_ave')
    GROUP BY 1, 2)
  SELECT
    COUNT(*) FILTER (WHERE va IS NOT NULL AND (v1 IS NULL OR {q89('q1')} OR abs(va - v1) > 1e-12)) AS n_leak,
    COUNT(*) FILTER (WHERE va IS NOT NULL) AS n_ave,
    max(abs(va - v1)) AS max_abs_diff
  FROM p"))
say(glue("override check: {n_override_leak$n_ave} temperature_ave values on overridden casts, ",
         "max |temperature_ave - temperature_1| = {n_override_leak$max_abs_diff}"))
stopifnot("an average on an overridden cast includes the overridden sensor" =
            n_override_leak$n_leak == 0)
dbExecute(con, "DROP TABLE IF EXISTS _override_hits")
```
...
# one measurement type at a time: every group is (cruise, site, depth, TYPE), so the
# per-type slices are disjoint and their union is exactly the one-statement result. The
# single GROUP BY over ~280M rows needed a hash table larger than the DuckDB budget
# (6 GB, 2026-09-24: "Out of Memory Error ... 5.5 GiB/5.5 GiB used"); it had only ever
# passed because the post-checkpoint connection ran unlimited.
summ_sql <- function(type) glue("
  SELECT
    c.cruise_key,
    c.site_key,
    m.depth_m,
  ...
  WHERE m.measurement_type = '{type}'
    AND NOT isnan(m.measurement_value)
    AND isfinite(m.measurement_value)
  GROUP BY c.cruise_key, c.site_key, m.depth_m, m.measurement_type")
summ_types <- dbGetQuery(con, "SELECT DISTINCT measurement_type FROM ctd_measurement ORDER BY 1")$measurement_type
dbExecute(con, glue("CREATE OR REPLACE TABLE ctd_summary AS {summ_sql(summ_types[1])}"))
for (type in summ_types[-1])
  dbExecute(con, glue("INSERT INTO ctd_summary {summ_sql(type)}"))
...
Many of the measurement types below are variants of the same property (multiple oxygen units and sensor/correction combinations, dual temperature and salinity sensors, derived estimates). `ctd_thin` keeps the canonical types — the averages, the bottle references, the uncorrected sensors (2026-08-06, so a sensor-only cruise still has salinity and oxygen) and, since 2026-09-10, the bottle-corrected per-sensor series and the chlorophyll / nitrate sensor estimates at both correction levels (Rasmus Swalethorp's transect-plotter rule) — see the `is_canonical` flag in `metadata/measurement_type.csv`, set by `libs/build_ctd_measurement_registry.R`.
...
# n_flags_applied (from "Apply Accepted QC Flags" above) is not a
# build_metadata_json() field, so it is patched onto the written sidecar
# rather than widening that helper's signature for one dataset's one number.
# release_database.qmd's qc_flags_pending chunk reads it to report how many of
# the snapshot's accepted flags this release does NOT yet reflect.
d_metadata_json <- jsonlite::read_json(metadata_path)
d_metadata_json$n_flags_applied <- if (exists("n_flags_applied")) n_flags_applied else 0L
jsonlite::write_json(d_metadata_json, metadata_path, auto_unbox = TRUE, pretty = TRUE, null = "null")
```
