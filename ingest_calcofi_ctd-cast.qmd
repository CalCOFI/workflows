...
# --- recompute TempAve from sensors that are individually valid ---------------
#
# The source ships a PRE-COMPUTED TempAve, and its arithmetic is faithful — what
# it does not do is check whether each sensor is worth averaging. Two failure
# modes, and the second is why validating the average is not enough:
#
#  1. MISSING SENSOR. Only one temperature sensor fitted, so the source averaged
#     the good one with its -99 missing marker:
#       Depth 3.0 | Temp1 20.8951 | Temp2 -99.0    | TempAve -39.0525
#     Exact on 29,596 of 29,596 such rows in 9308NH. Lands far out of range.
#
#  2. FAILED SENSOR. Temp2 reading nonsense but not a sentinel — on 2607SH it
#     ranges -31.95 to 32.64 degC while Temp1 sits at a steady 20.15:
...
#
# So the test belongs on each SENSOR, using the bounds the registry already
# declares for temperature_1 / temperature_2, and the average is rebuilt from
# whichever sensors pass. Recompute rather than discard: every poisoned row has a
# valid Temp1 beside it, so dropping instead would throw away 9308NH's entire
# temperature record (29,661 rows) to fix it.
t_lo <- d_meas_type$valid_min[d_meas_type$measurement_type == "temperature_1"]
t_hi <- d_meas_type$valid_max[d_meas_type$measurement_type == "temperature_1"]
stopifnot("temperature_1 must declare bounds for the TempAve repair" =
            length(t_lo) == 1 && !is.na(t_lo) && !is.na(t_hi))

n_tav_bad <- dbGetQuery(con, glue("
  SELECT COUNT(*) n FROM ctd_raw
  WHERE (temp1 IS NOT NULL AND temp1 NOT BETWEEN {t_lo} AND {t_hi})
     OR (temp2 IS NOT NULL AND temp2 NOT BETWEEN {t_lo} AND {t_hi})
     OR (temp_ave IS NOT NULL AND temp_ave NOT BETWEEN {t_lo} AND {t_hi})"))$n

# BETWEEN yields NULL on a NULL operand, which is falsy in CASE, so each arm
# doubles as its own presence test.
dbExecute(con, glue("
  UPDATE ctd_raw SET temp_ave = CASE
    -- both sensors good: the honest average
    WHEN temp1 BETWEEN {t_lo} AND {t_hi} AND temp2 BETWEEN {t_lo} AND {t_hi}
      THEN (temp1 + temp2) / 2.0
    -- one sensor good: that sensor IS the average
    WHEN temp1 BETWEEN {t_lo} AND {t_hi} THEN temp1
    WHEN temp2 BETWEEN {t_lo} AND {t_hi} THEN temp2
    -- neither usable: missing, not a number
    ELSE NULL
  END
  WHERE (temp1 IS NOT NULL AND temp1 NOT BETWEEN {t_lo} AND {t_hi})
     OR (temp2 IS NOT NULL AND temp2 NOT BETWEEN {t_lo} AND {t_hi})
     OR (temp_ave IS NOT NULL AND temp_ave NOT BETWEEN {t_lo} AND {t_hi})"))

n_tav_after <- dbGetQuery(con, glue("
  SELECT COUNT(*) n FROM ctd_raw
  WHERE temp_ave IS NOT NULL AND temp_ave NOT BETWEEN {t_lo} AND {t_hi}"))$n
say(glue("TempAve recomputed where a sensor was invalid: ",
         "{format(n_tav_bad, big.mark = ',')} row(s); ",
         "{n_tav_after} out-of-range average(s) remain"))
stopifnot("TempAve repair must leave no out-of-range average" = n_tav_after == 0)
...
# about a genuinely wild reading. A declared bound is the general guard, and the
# bounds are deliberately GENEROUS — impossible, not merely unusual — so this
# drops nothing an oceanographer would want to see.
#
# DELETE rather than flag, for the same reason as the sentinel: in long format a
# row IS an assertion that a value was measured. A -10 pH silently corrupts any
# mean, minimum or anomaly a consumer computes, and the anomaly case is exactly
# what this data is being used for.
d_bounds <- d_meas_type |>
  filter(!is.na(valid_min), !is.na(valid_max)) |>
  select(measurement_type, valid_min, valid_max)

dbWriteTable(con, "meas_bounds", as.data.frame(d_bounds), overwrite = TRUE)

oob_tally <- dbGetQuery(con, "
  SELECT m.measurement_type, COUNT(*) AS n,
         MIN(m.measurement_value) AS v_min, MAX(m.measurement_value) AS v_max
  FROM ctd_measurement m JOIN meas_bounds b USING (measurement_type)
  WHERE m.measurement_value < b.valid_min OR m.measurement_value > b.valid_max
  GROUP BY 1 ORDER BY n DESC")

n_oob <- dbExecute(con, "
  DELETE FROM ctd_measurement WHERE rowid IN (
    SELECT m.rowid FROM ctd_measurement m JOIN meas_bounds b USING (measurement_type)
    WHERE m.measurement_value < b.valid_min OR m.measurement_value > b.valid_max)")
dbExecute(con, "DROP TABLE IF EXISTS meas_bounds")

say(glue("out-of-range rows removed: {format(n_oob, big.mark = ',')} ",
         "across {nrow(oob_tally)} type(s), against ",
         "{nrow(d_bounds)} declared bound(s)"))

if (nrow(oob_tally))
  oob_tally |>
    dt(caption = paste(
      "Values outside the registry's declared valid_min/valid_max, removed.",
      "These are IMPOSSIBLE readings, not unusual ones — the bounds are",
      "deliberately generous."),
      fname = "ctd_out_of_range_removed")
