...
# --- repair TempAve, which the SOURCE averaged with its own missing marker ----
#
# The source ships a pre-computed TempAve. Where only one temperature sensor was
# fitted it averaged the good sensor with the -99 missing marker:
#
#   Depth 3.0 | Temp1 20.8951 | Temp2 -99.0 | TempAve -39.0525
#
# and (20.8951 + -99)/2 = -39.0525 exactly — verified on 29,596 of 29,596 such
# rows in 9308NH, the first cruise of the Technical Group archive.
#
# NEITHER existing guard can catch this, and it is worth being precise about why:
# the `= -99` DELETE below is applied to every cruise uniformly, but by this point
# the sentinel has been ARITHMETICALLY ABSORBED — the value is -39.05, not -99.
# The isfinite/isnan guard misses it for the same reason: -39.05 is a perfectly
# finite double. A sentinel that has been averaged is invisible to any test for
# the sentinel.
#
# Recompute rather than discard: every poisoned row in 9308NH has a valid Temp1
# beside it (4.8-22.1 degC), so range-filtering alone would throw away that
# cruise's ENTIRE temperature record — 29,596 rows — while recomputing recovers
# all of it. This is what the column already means; the source simply did not
# handle a missing sensor.
n_tav_bad <- dbGetQuery(con, "
  SELECT COUNT(*) n FROM ctd_raw
  WHERE temp_ave IS NOT NULL AND (temp_ave < -2 OR temp_ave > 40)")$n
...
dbExecute(con, "
  UPDATE ctd_raw SET temp_ave = CASE
    ...
    WHEN temp1 BETWEEN -2 AND 40 AND temp2 BETWEEN -2 AND 40
      THEN (temp1 + temp2) / 2.0
    -- one sensor good: that sensor IS the average
    WHEN temp1 BETWEEN -2 AND 40 THEN temp1
    WHEN temp2 BETWEEN -2 AND 40 THEN temp2
    -- neither usable: missing, not a number
    ELSE NULL
  END
  WHERE temp_ave IS NOT NULL AND (temp_ave < -2 OR temp_ave > 40)")

n_tav_after <- dbGetQuery(con, "
  SELECT COUNT(*) n FROM ctd_raw
  WHERE temp_ave IS NOT NULL AND (temp_ave < -2 OR temp_ave > 40)")$n
say(glue("TempAve repaired: {format(n_tav_bad, big.mark = ',')} out-of-range ",
         "values recomputed from the sensors; {n_tav_after} remain"))
stopifnot("TempAve repair must leave no out-of-range values" = n_tav_after == 0)
