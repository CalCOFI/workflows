...
`metadata/calcofi/ctd-cast/questions.csv` and surfaced here so they travel with
the workflow rather than living in someone's inbox. Two are **blockers** — the
`-99` sentinel and the physically impossible sensor extremes both currently reach
`obs` as if they were measurements.
...
# real table in the wrangling DB at render time, so read it there.
