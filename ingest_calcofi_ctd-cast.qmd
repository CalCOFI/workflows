...
# overwrite: delete wrangling DB + parquet, but keep checkpoint DB + downloads
# overwrite_all: also delete checkpoint DB and RDS intermediates
if (overwrite) {
  ...
  if (overwrite_all) {
    ...
    message("Deleted checkpoint DB and RDS intermediates")
  }
}
...
dedup_key  <- c("cruise_key", "cast_key", "cast_dir", "datetime_utc", "depth_m")
stopifnot(
  "ctd_raw missing expected columns for de-duplication" =
    all(c(dedup_key, "_source_file", "lat_dec", "lon_dec") %in% raw_cols))
...
dbExecute(con, glue("
  ...
  QUALIFY ROW_NUMBER() OVER (
    ...
    ORDER BY length(\"_source_file\"), \"_source_file\", lat_dec, lon_dec) = 1"))
...
# datetime_utc). within that key the only cast-level column that varies is
...
dbExecute(
  ...
  FROM ctd_raw
  QUALIFY ROW_NUMBER() OVER (
    PARTITION BY cruise_key, cast_key, cast_dir, datetime_utc
    ORDER BY lat_dec, lon_dec) = 1
...
parquet_stats <- write_parquet_outputs(
  ...
  sort_by = list(
    ...
    ctd_cast        = "hilbert:lon_dec,lat_dec"),
...
metadata_path <- build_metadata_json(
  ...
  provider             = provider,
  dataset              = dataset,
  workflow_url         = glue("https://calcofi.io/workflows/ingest_{provider}_{dataset}.html")
)
