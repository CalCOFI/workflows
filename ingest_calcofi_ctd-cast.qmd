...
account is in the [QA/QC protocol](https://calcofi.io/workflows/qc_protocol.html).
...
::: {.callout-warning title="`data_stage` does not survive into the release"}
...
This notebook knows which cruises are which — `data_stage` is `final` or
`preliminary` on `ctd_cast`, and the appendix charts it. **The released `sample`
carries no such column**, so a consumer reading the release cannot tell a
final cruise from a preliminary one, and the caveat above cannot reach them. That
is a gap in the core model rather than in this ingest, and it is filed as question
`calcofi_ctd-cast_14`.
:::
...
if ("ctd_cast" %in% DBI::dbListTables(con)) {
  dbGetQuery(con, "
    ...
    datatable(
      caption = paste("Final vs preliminary in this ingest — a distinction the",
                      "released `sample` cannot currently express"),
...
read_csv(here(cc$questions_file), show_col_types = FALSE) |>
  arrange(factor(priority, c("blocker", "high", "normal", "low")), id) |>
  select(id, priority, question, context, status) |>
  datatable(
    caption  = "Questions for the CalCOFI CTD data providers (ranked)",
    rownames = FALSE,
    options  = list(dom = "t", scrollX = TRUE,
                    columnDefs = list(list(width = "70px", targets = 1))))
```
