...
d_zips <- tryCatch(
  {
    d <- read_html(url) |>
      ...
      mutate(
        ...
        # https://calcofi.org/downloads/2026/20-2601RL_CTDPrelim.zip # NOT FOUND
...
# derive ship_key from last 2 chars of cruise_key (YYMMKK format)
...
# filter for CTD measurement types only
d_meas_ctd <- d_meas_type |>
  filter(str_detect(`_source_datasets`, "calcofi_ctd-cast"))

# define cast-level columns (everything that's NOT a measurement or quality flag)
...
dbExecute(con, "ALTER TABLE ctd_measurement ADD COLUMN IF NOT EXISTS cruise_key VARCHAR")
dbExecute(
  con,
...
assign_deterministic_uuids_md5(
  ...
  key_cols = c("cruise_key", "sta_key", "depth_m", "measurement_type")
)
```
...
## Finalize — Ingest to Working DuckLake

Push all parquet tables to the Working DuckLake with provenance tracking.

```{r}
#| label: finalize_ingest

wdl_stats <- finalize_ingest(
  parquet_dir = dir_parquet,
  source_label = glue("{provider}_{dataset}")
)

wdl_stats |>
  dt(caption = "Working DuckLake ingestion stats")
```
...
# read ctd_cast from parquet
dbExecute(
  con_pq,
  glue(
    "
  CREATE VIEW ctd_cast AS
  SELECT * FROM read_parquet('{dir_parquet}/ctd_cast.parquet')"
  )
)
