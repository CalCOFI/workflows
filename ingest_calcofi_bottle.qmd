...
7. Declares the series' physical bounds in the registry and deletes the values outside them
8. Loads `measurement_type` reference table
9. Validates primary keys and foreign key integrity
10. Adds spatial geometry and grid assignments
11. Exports to parquet files (for later integration into Working DuckLake)
...
Every ingest that emits measurements declares the physical bounds of its series in
`metadata/measurement_type.csv` and deletes what breaks them (the `measurement-bounds`
skill): a bound catches the impossible and never polices oceanography, so a value
outside it is a sentinel or a scaling error, not a reading. Until 2026-09-10 the bottle
series declared nothing, and release v2026.09.06 published a 99.00 °C and a 56.87 °C at
50 m (cruise 2020-07-33P4, station 93.3/30 — the same two bottles read 9.50 and
21.06 PSU beside 33.6 in the bottles above and below), six potential densities of
216–251 kg/m³ (2014-11-32NM and 2015-04-32NM, flagged `6`), and one surface oxygen of
−200.2 ml/L / −8,740.6 µmol/kg / −3,605 % saturation (2020-07-33P4, station 90/30):
eleven values on three cruises. None is a DBF-era 9-fill — the 9-family scan finds no
−99 / 999 pattern in any bottle series, and the 1,739 `temperature` values at exactly 9
or 99 are real 9.0 °C readings.

The bounds mirror the CTD's for the same quantities (`temperature_ave` −2…40 °C,
`sigma_theta_1` 15…35 kg/m³). The three oxygen floors sit just below zero rather than
at the CTD's 0: four Winkler titrations in the oxygen minimum on line 83.3 station 45
(550–570 m, 1995–2002) read −0.010 ml/L / −0.435 µmol/kg / −0.10 %, a blank subtraction
of zero rather than a sentinel, and an oceanographer wants to see that the water was
measured anoxic. Salinity declares nothing yet: cruise 2021-05-3322 carries 918 values
of 24.19–27.06 PSU to 568 m across 39 stations with `S_qual` 254–344 — a whole cruise,
not a sentinel, and a floor that caught it would delete provider-flagged data while one
that spared it would miss the 9.50 (question Q13).

So the 9.50 and the 21.06 go with their bottles instead (2026-09-11, found again by the
measurement faces' familiar scale): **a bottle whose temperature is impossible loses its
salinity too**. Those two bottles are the 99.00 °C and 56.87 °C ones; after the bounds
delete their temperature, oxygen and density, the salinity was the only reading either
had left, and a salinity beside an impossible temperature on the same bottle is the same
corrupt record, not a second opinion. The rule is written for the class, and today it
removes exactly those two values.

Three more series gained bounds the same day. **Nitrate**: seven bottles read 56–95
µmol/L (bottle ids 508764–508823, one cruise) beside 0.6–0.97 µmol/L phosphate, 6–10
µmol/L silicate and 4.3–6.1 ml/L oxygen — a nitrate:phosphate ratio near 100 in
oxygenated water, which no ocean holds (the deep North Pacific, the world's most
nitrate-rich water, sits in the 40s at a ratio near 15); they read like a slipped decimal.
The ceiling of 60 µmol/L sits above CalCOFI's real deep tail, which runs continuously to
52 µmol/L, and removes six of the seven (66–95); the seventh, 56.0, is impossible only by
its phosphate, not by its size, so a bound cannot take it without cutting into that tail
and it stays until the provider answers question 14. The floor of −1 keeps the five
blank-subtraction negatives (to −0.4). **Chlorophyll-a** and **phaeopigment** get floors
only — −1 and −5, which keep the rounding negatives of the acid-ratio calculation
(phaeopigment to −3.89; calcofi.org notes chlorophyll-b reads as phaeopigment) and catch
a sentinel — and no ceiling: a red-tide bloom off San Diego reaches tens of µg/L, and the
record's 66 is one.

```{r}
#| label: bounds

# --- declare, then enforce, the bottle's physical bounds ----------------------
# declare_measurement_bounds() is idempotent: a bound equal to the declared one is
# "unchanged", a different one errors unless overwrite = TRUE — a declared bound is
# an agreement with the provider, not a side effect of a re-run.
bottle_bounds <- tibble::tribble(
  ~measurement_type,   ~valid_min, ~valid_max,
  "temperature",             -2,          40,
  "sigma_theta",             15,          35,
  "oxygen_ml_l",           -0.5,          15,
  "oxygen_umol_kg",         -20,         700,
  "oxygen_saturation",       -1,         250,
  "nitrate",                 -1,          60,
  "chlorophyll_a",           -1,          NA,
  "phaeopigment",            -5,          NA)
d_meas_type <- declare_measurement_bounds(bottle_bounds, meas_type_csv)

# report BEFORE deleting: `undeclared` is a finding too — the bottle types with no
# bound are the ones still to take to the provider
bounds_pre <- check_measurement_bounds(con, "bottle_measurement", mt = d_meas_type)
bounds_datatable(bounds_pre)

# a bottle whose temperature is impossible loses its salinity too: collect those
# bottles BEFORE the bounds delete their temperature, then remove the salinity
t_lo <- d_meas_type$valid_min[d_meas_type$measurement_type == "temperature"]
t_hi <- d_meas_type$valid_max[d_meas_type$measurement_type == "temperature"]
n_sal_corrupt <- dbExecute(con, glue("
  DELETE FROM bottle_measurement
  WHERE measurement_type = 'salinity'
    AND bottle_id IN (
      SELECT bottle_id FROM bottle_measurement
      WHERE measurement_type = 'temperature'
        AND measurement_value NOT BETWEEN {t_lo} AND {t_hi})"))
say(glue("salinity removed beside an impossible temperature: {n_sal_corrupt} value(s)"))

oob_tally <- drop_out_of_bounds(con, "bottle_measurement", mt = d_meas_type)
say(glue(
  "out-of-range rows removed: {format(sum(oob_tally$n_bad), big.mark = ',')} ",
...
bounds_post <- check_measurement_bounds(
  con, "bottle_measurement", mt = d_meas_type, include_undeclared = FALSE)
stopifnot(
  "bottle_measurement must hold no value outside a declared bound" =
...
#
# FIXED 2026-09-03 (WS-G, Rasmus item f): the comparison used to be
# `SUBSTR(cruise_key,...) || SUBSTR(cruise_key,...) <> cruise` — a VARCHAR
# built from cruise_key compared directly against `cruise`, a DOUBLE
# (Cruise = YYYYMM.0 in the source). That mixed-type comparison rendered
# `cruise`'s side with a trailing ".0" (e.g. "198809.0") wherever a plain
# CAST(...AS VARCHAR) is what got applied, so the VARCHAR "198809" built from
# cruise_key never matched — the check failed on almost every row regardless
# of whether the month actually agreed, which is why 670 of the 684 rows this
# table used to show were a formatting artifact rather than a real
# disagreement (measured against the rendered output, umbrella plan
# 2026-09-03 "Context"). Casting both sides to INTEGER removes the ambiguity
# and leaves the 14 genuine disagreements (829 casts, 1953-1989) that are the
# real content of this check.
dbGetQuery(con, "
  SELECT cruise, cruise_key, cruise_key_method, COUNT(*) AS n_casts
  FROM casts
  WHERE cruise_key IS NOT NULL
    AND CAST(SUBSTR(cruise_key, 1, 4) || SUBSTR(cruise_key, 6, 2) AS INTEGER) <> CAST(cruise AS INTEGER)
