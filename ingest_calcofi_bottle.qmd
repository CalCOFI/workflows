...
sea state rather than needing one of its own.

```{r}
#| label: emit_core

core <- emit_core_tables(con, "calcofi_bottle")
cat(glue(
...
stopifnot(
  ...
  "obs must be one row per grid-matched bottle measurement" =
    dbGetQuery(con, "SELECT COUNT(*) FROM obs")[[1]] ==
  ...
  "bottom_depth must reach sample_measurement" =
    dbGetQuery(con, "SELECT COUNT(*) FROM sample_measurement WHERE measurement_type='bottom_depth'")[[1]] > 0)
cat(glue("sample: {format(n_cast, big.mark=',')} casts / {format(n_btl, big.mark=',')} bottles"), "\n")

compat <- create_compat_views(con, "calcofi_bottle")
