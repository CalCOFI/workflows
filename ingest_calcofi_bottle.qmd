...
# load unified measurement_type reference (needed for pivot mappings)
...
ship_renames_csv <- here(
  "metadata/calcofi/bottle/ship_renames.csv"
)
...
if (!file_exists(map_casts_png)) {
  ...
  cast_cols <- dbGetQuery(con,
    "SELECT column_name FROM information_schema.columns
     WHERE table_name = 'casts' AND data_type != 'GEOMETRY'")$column_name
  cast_sql <- paste(
...
# exclude tables with GEOMETRY columns from dm (duckdb R driver limitation)
...
cd_cols <- dbGetQuery(con,
  "SELECT column_name FROM information_schema.columns
   WHERE table_name = 'casts_derived' AND data_type != 'GEOMETRY'")$column_name
dbGetQuery(con, glue("SELECT {paste(cd_cols, collapse=', ')} FROM casts_derived LIMIT 100")) |>
  datatable(
...
# exclude tables with GEOMETRY columns (duckdb R driver limitation)
geom_tbls_wr <- DBI::dbGetQuery(con,
  "SELECT DISTINCT table_name FROM information_schema.columns
   WHERE data_type = 'GEOMETRY'")$table_name
dm_tbls_wr <- setdiff(db_tables(con), geom_tbls_wr)

tryCatch({
  dm_bottle <- dm_from_con(
    con,
    table_names = dm_tbls_wr,
    learn_keys = FALSE
  ) |>
    add_bottle_keys()
  ...
  message(glue("relationships.json skipped: {e$message}"))
})
...
# load ship and cruise from ichthyo workflow
load_prior_tables(
  con = con,
...
casts_cols <- dbGetQuery(con,
  "SELECT column_name FROM information_schema.columns
   WHERE table_name = 'casts'")$column_name
grid_cols <- dbGetQuery(con,
  "SELECT column_name FROM information_schema.columns
