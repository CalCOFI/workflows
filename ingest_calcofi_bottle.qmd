...
parquet_stats <- write_parquet_outputs(
  ...
  tables           = own_tables,
  sort_by          = list(
    bottle_measurement = "measurement_type",
    casts              = "hilbert:longitude,latitude"),
...
# also drop cruise_key_0 — interim rename used to avoid collision during
# the bridge merge; canonical `cruise_key` (YYYY-MM-NODC) is what consumers
# query against (see /query/_queries/browse/cruises.md and CLAUDE.md note
# on identifier standardization)
...
drop_cols   <- c(prov_cols, "cruise_key_0")
export_cols <- setdiff(cols, drop_cols)
DBI::dbExecute(
  con,
  glue(
...
message("Re-exported casts.parquet with ship_key + cruise_key (cruise_key_0 dropped)")

# sync the fixed casts.parquet back to GCS (the earlier sync_to_gcs ran
# before the cruise bridge; without this push, the release would copy the
# stale ingest/calcofi_bottle/casts.parquet that has cruise_key_0 only)
put_gcs_file(
  casts_pqt,
  glue("gs://calcofi-db/ingest/{dir_label}/casts.parquet"))
message("Re-uploaded casts.parquet to GCS ingest/")
