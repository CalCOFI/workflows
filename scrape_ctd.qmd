...
d_csv <- d_csv |>
  ...
  mutate(
    data = map2(path, seq_along(path), \(path, idx) {
      ...
      # TODO: detect repeat header rows and skip them
      # eg line 69 of
      path = '/Users/bbest/Library/CloudStorage/GoogleDrive-ben@ecoquants.com/My Drive/projects/calcofi/data/calcofi.org/ctd-cast/download/20-0711NH_CTDFinalQC/db-csvs/20-0711_CTDBTL_001-067Dno001b.csv'
      idx = 1
...
d_bind <- d_bind |>
  mutate(
    ...
    dtime_utc = if_else(
      str_detect(
        date_time_utc, # eg "23-Jan-1998 17:50:44"  "3-JAN-1998 17:50:44"
    ...
    dtime_utc = if_else(
      str_detect(
        date_time_utc, # eg "01/01/2015 17:12", "1/01/2015 17:12", "1/1/2015 17:12"
...
sum(is.na(d_bind$dtime_utc)) # 7,219

d_bind |> filter(is.na(dtime_utc)) |> select(date_time_utc, source_file)
#   date_time_utc source_file
#   <chr>         <chr>
# 1 Date_Time_UTC 20-0711NH_CTDFinalQC/db-csvs/20-0711_CTDBTL_001-067Dno001b.csv

d_bind |>
  filter(is.na(dtime_utc)) |>
  select(cruise_id, source_file, date_time_utc) |>
  View()
