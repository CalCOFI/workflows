...
calcofi:
  ...
  # is Q01: no licence, no citation, and permission to publish unconfirmed. Flip to
  # `true` — or delete the key, since the flag is opt-out — once Q01 is answered,
...
# whether these files may be published is itself an open blocker (Q01 — no
# licence, no citation, and CDFW commissioned the work), and both buckets are
# world-readable. Uploading is a one-way door, so it is opt-in until Q01 is
...
nullable <- tribble(
  ...
  # taxon: two different situations, deliberately not lumped
  "taxon",                 "ncbi_id",           3L,   "never populated by ANY dataset — the column exists, nothing fills it",
  "taxon",                 "inat_id",           3L,   "never populated by ANY dataset",
  "taxon",                 "itis_id",           3L,   "WoRMS could supply a cross-authority id; not fetched (see Not Yet Done)",
  "taxon",                 "gbif_id",           3L,   "as itis_id",
  "taxon",                 "parent_taxon_key",  3L,   "REAL GAP: no lineage built for measurement_taxon-resolved taxa (see Not Yet Done)")
...
# The lineage gap is asserted rather than merely noted, so it cannot be quietly
# forgotten: this fails the moment lineage IS populated, forcing the note in
# "Not Yet Done" to be removed in the same change.
n_taxon_no_lineage <- dbGetQuery(con, "
  SELECT COUNT(*) FROM taxon WHERE rank IS NULL AND parent_taxon_key IS NULL")[[1]]
stopifnot("taxon lineage gap has changed — update the note in Not Yet Done" =
            n_taxon_no_lineage == 3)
...
cat(glue("KNOWN GAP: all {n_taxon_no_lineage} taxa lack rank + parent_taxon_key, so ",
         "hierarchy rollups (e.g. 'all Decapoda') will not work. NOT specific to this ",
         "ingest — swfsc_cufes, calcofi_phyllosoma and cce-lter_euphausiids are all ",
         "in the same state, because taxa resolved through measurement_taxon.csv get ",
         "no lineage fetch. Only swfsc_ichthyo has it, from its own species table."), "\n")
...
read_csv(here(glue("metadata/{provider}/{dataset}/questions.csv"))) |>
  ...
  # show the number, so "Q01" in the prose above points at something. Ids are
  # assigned in priority order (blocker first) and the table is also sorted by
  # priority, so the ordering stays right even after a priority changes.
  mutate(`#` = str_replace(id, ".*_(\\d+)$", "Q\\1"), .before = 1) |>
  select(`#`, priority, status, question, context, answer) |>
  datatable(caption = "Questions for data providers, in priority order",
...
1. **Answer the one remaining blocker**, Q01 — permission to publish, a licence,
   ...
   (Q11, standardization, is answered — see the Notes sheet above.)
...
3. **Populate the taxon lineage.** All three taxa have `rank` and
   `parent_taxon_key` NULL, so they cannot participate in hierarchy rollups — a
   query for "all Decapoda" will not find the *M. magister* records. This is **not
   specific to this ingest**: every dataset whose taxa resolve through
   `metadata/measurement_taxon.csv` is in the same state (`swfsc_cufes`,
   `calcofi_phyllosoma`, `cce-lter_euphausiids`), because that path carries only
   `worms_id` + `scientific_name` and no classification is fetched. Only
   `swfsc_ichthyo` has lineage, from its own `species` table. The fix belongs in
   `build_taxon_reference()` — fetch rank + classification from WoRMS for
   crosswalk-resolved taxa — and is asserted in "Validate" so it fails here once
   done. Note `ncbi_id`/`inat_id` are populated by no dataset at all, and `family`
   by none including ichthyo.
...
5. **Parse the free-text identification notes** (Q07) into `obs_attribute` stage
   and length bins.
6. Move `measurement_type_new.csv` into `metadata/measurement_type.csv` and drop
