...
redo_if_exists <- TRUE # set to FALSE to skip already processed datasets (for testing/debugging)
...
process_spatial_dataset <- function(
  ...
  # write parquet via DuckDB spatial ----
  ...
  tippecanoe_args <- c(
    ...
    "--simplification=10",
    "--no-tile-size-limit",
    # "--use-attribute-for-id=id", # wierdly drops 'id', so incompatible with mapgl::add_pmtiles_source(promote_id='id')
    "-l",
    dataset_group,
    "--force",
    geojson_tmp
...
results <- d_groups |>
  # filter(dataset_group == "ca_county_boundaries") |> # DEBUG: process one group first
...
# str_subset("ca_county_boundaries") # DEBUG: only upload one file first
