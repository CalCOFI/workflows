...
mapview(cinms_ply) # weird...ply is no longer showing up...
```
...
## TODO: Make google sheet and read into R
...
get_oceano_var_aoi <- function(
  ...
  # test values
  # var = "Bottle_O2_ml_L"; aoi = cinms_ply; date_step = "year"; depth_min = 0; depth_max = 10
  ...
  # d <- eval(as.name(eval(parse(text = glue("var_lookup${var}$data_source")))))
  ...
  d_aoi_summ <- d_aoi %>% 
    ...
    # {if (any(!is.na(.data[[Depthm]]))) filter(Depthm >= depth_min, Depthm < depth_max)} %>% 
    filter(
      Depthm >= depth_min,Depthm < depth_max) %>% 
