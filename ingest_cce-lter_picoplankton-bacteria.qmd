...
calcofi:
  target_name: ingest_cce-lter_picoplankton-bacteria
  ...
  tables_owned:
    - {table: picoplankton_bacteria_bottle}
    - {table: picoplankton_bacteria_measurement}
...
- **Completeness**: Depth, Heterotrophic Bacteria, Synechococcus, and
  ...
  blank in the current export and is dropped. No literal-zero values appear
...
Read from `{dir_data}/cce-lter/picoplankton-bacteria/`. Columns renamed per
`metadata/cce-lter/picoplankton-bacteria/flds_redefine.csv`.

```{r}
#| label: read-source
pico_csv <- path_expand(glue(
  "{dir_data}/cce-lter/picoplankton-bacteria/PicoplanktonandBacteriaAbundance.csv"))
...
pb_types <- tribble(
  ...
  mutate(`_source_column` = case_when(
...
new_types <- pb_types |> filter(!measurement_type %in% d_meas_type$measurement_type)
if (nrow(new_types) > 0) {
...
parquet_stats <- write_parquet_outputs(
  ...
  tables = c("picoplankton_bacteria_bottle", "picoplankton_bacteria_measurement",
             "measurement_type", "dataset"),
...
metadata_path <- build_metadata_json(
  ...
  metadata_derived_csv = here("metadata/cce-lter/picoplankton-bacteria/metadata_derived.csv"),
  output_dir = dir_parquet, tables = c("picoplankton_bacteria_bottle", "picoplankton_bacteria_measurement"),
...
- [ ] Core-table projection (`sample`/`obs`/`obs_freq`/`sample_measurement`)
  is not yet wired up — per `design_env-bio-consolidation.md`, that's Phase 3
  (`calcofi4db::append_sample()` / `append_obs()` / `append_obs_freq()` /
  `append_sample_measurement()`, called after `build_sample_reference()`),
  and Phase 2 (materializing the core tables at all) hasn't landed yet
  either. `picoplankton_bacteria_bottle`/`_measurement` are a bottle-shaped
  sample (leaf = `bottle_id`, parent = `cast_id`, same as `calcofi_bottle`)
  with an `obs` headline of the four FCM measurement types — revisit once
  the `append_*` helpers exist.
