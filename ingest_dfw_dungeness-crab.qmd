...
calcofi:
  ...
  # and projected into sample / obs / obs_attribute / sample_measurement + the taxa
  # refs via the generic append_* helpers. The three per-dataset tables the core
  # does not model ride along; dungeness_measurement / dungeness_carapace survive
  # as compat VIEWs over the core and are not written separately.
  tables_owned:
    - {table: sample, shared: true, note: "core event dimension (examined subsamples + sorting-log tows)"}
    - {table: obs, shared: true, note: "core occurrence headline (abundance x 4 resolved taxa)"}
    ...
    - {table: dungeness_sample, note: "source detail with no core home (cruise_orig, position_source, comments)"}
    - {table: dungeness_sorting_log, note: "1949-2009 sorted/unsorted inventory; gear + status columns the core does not model"}
...
The 2,011-row sorting log also becomes `sample` rows (`sample_type = 'tow'`) with
**no** `obs`. That is the whole point of it: it is absence/effort data, and the
core `sample` dimension is where "we looked here and found nothing" becomes
queryable. `pic_zooplankton` contributes `sample` only for the same reason.
...
stopifnot(
  ...
  "obs must carry every source count row" = core_n$obs == nrow(d_meas),
  "obs totals must equal the source totals" =
  ...
  # the sorting log is effort only: it must contribute zero occurrences
  "sorting-log tows must carry no obs" =
    dbGetQuery(con, "SELECT COUNT(*) FROM obs o JOIN sample s USING (sample_key)
                     WHERE s.sample_type='tow'")[[1]] == 0)
...
The two per-dataset tables the core fully models survive as VIEWs over it — detail
survives, bytes don't — so the previews below and any ad-hoc query keep working
against the names this notebook built. The originals are kept as `*_src` for the
ERD above.

Three tables stay real, because the core does not model them: `dungeness_sample`
carries source columns with no core home (`cruise_orig`, `position_source`,
`comments`); `dungeness_sorting_log` carries gear and sorting status (its *rows*
are in `sample`, its columns are not); and `dungeness_specimen` is a taxonomic
determination, not an abundance observation.
...
stopifnot(
  "compat view must expose every obs row"           = n_view == core_n$obs,
...
cat(glue("compat views over core — dungeness_measurement {n_view} rows, ",
         "dungeness_carapace {n_cara} rows"), "\n")
```

## Schema

The **source** shape, documenting the wrangling above. The tables this ingest
publishes are the consolidated core, so `relationships.json` is written from
`core_relationships()` in "Write Outputs".
...
# the CORE is this ingest's published output — release_database.qmd concatenates
# the per-dataset shards. The three per-dataset tables the core does not model
# ride along as extras; the compat VIEWs are deliberately NOT written (they are
# views over the core, so writing them would duplicate the same bytes).
tbls_out <- core_output_tables(
  con, extra = c("dungeness_sample", "dungeness_sorting_log", "dungeness_specimen",
