...
format:
  html:
    # NOTE: the project-wide `mermaid-format: png` this used to override is now
    # disabled in _quarto.yml for the same reason found here first — PNG needs a
    # headless browser and hangs (system Chrome on its profile lock; quarto's
    # bundled Chromium 91.0.4469.0 too old for mermaid 11.x). This explicit svg
    # is therefore redundant but harmless, and documents where the problem was
    # diagnosed before it cost two pipeline runs.
    mermaid-format: svg
---
