...
1. Create a folder for your data: `{provider}/{dataset}/` (e.g., `swfsc/ichthyo/`)
...
|----------|---------|--------|
| `ingest_swfsc_ichthyo.qmd` | Ingest ichthyo tables + taxonomy → Working DuckLake | Active |
| `ingest_calcofi_bottle.qmd` | Ingest bottle/cast tables + cruise bridge → Working DuckLake | Active |
| `ingest_calcofi_ctd-cast.qmd` | Ingest CTD tables → Working DuckLake | Active |
| `release_database.qmd` | Validate, freeze, and upload release from Working DuckLake | Active |
...
- **Top-level folders**: `{provider}/` - the organization providing the data (e.g., `swfsc`)
- **Dataset folders**: `{dataset}/` - specific dataset name (e.g., `ichthyo`)
...
│   ├── swfsc_ichthyo/                     # Flattened {dir_label}
│   │   ├── cruise.parquet
│   │   ├── ship.parquet
│   │   ├── site.parquet
│   │   ├── ...
...
```json
{
  ...
  "files": [
    {
      "Path": "calcofi/bottle/bottle.csv",
      "Size": 45234567,
      "ModTime": "2026-02-02T10:30:00Z",
      "MimeType": "text/csv",
      "gcs_url": "gs://calcofi-files-public/archive/2026-02-02_121557/calcofi/bottle/bottle.csv"
    },
    {
      "Path": "swfsc/ichthyo/larva.csv",
      "Size": 12345678,
      "ModTime": "2026-02-01T08:00:00Z",
      "MimeType": "text/csv",
      "gcs_url": "gs://calcofi-files-public/archive/2026-02-02_121557/swfsc/ichthyo/larva.csv"
    }
  ],
  "total_files": 112457,
...
- Unique identifiers are suffixed with:
  - `*_uuid` for source UUIDs (minted at sea — primary identifiers for SWFSC tables)
  - `*_id` for integer keys (source counters like `cast_id`, or sequential IDs for derived tables)
  - `*_key` for string/text keys (natural keys like `cruise_key`, `ship_key`)
...
### UUID-First Approach (revised 2026-02-24)

**Source UUIDs are the primary unique identifiers** for SWFSC ichthyo tables. These
UUIDs are minted at sea during data collection and remain stable throughout the entire
data lifecycle — even when rows are temporarily removed and later re-included for QA/QC
purposes. Sequential integer IDs would lose this stability because re-sorting or row
additions change the assignment.

**Exceptions:**

- `cruise` uses a natural key (`cruise_key`, YYMMKK) because it has few rows with
  easily identifiable attributes (ship + year-month). `cruise_uuid` is also retained
  from source.
- Bottle tables (`casts`, `bottle`) use source integer counters (`Cst_Cnt`, `Btl_Cnt`)
  as stable keys from the source system.
- Derived/pivoted tables without source UUIDs (ichthyo, lookup, segment,
  bottle_measurement, cast_condition) use sequential integer IDs as convenience
  identifiers.

### Source UUID Keys (SWFSC ichthyo chain)

| Table | Primary Key | Type | Notes |
|-------|-------------|------|-------|
| `site` | `site_uuid` | UUID | Minted at sea |
| `tow` | `tow_uuid` | UUID | Minted at sea |
| `net` | `net_uuid` | UUID | Minted at sea |

### Natural Keys

| Table | Primary Key | Format | Example |
|-------|-------------|--------|---------|
| `cruise` | `cruise_key` | TEXT `YYMMKK` | `'2401NH'` (also retains `cruise_uuid`) |
| `ship` | `ship_key` | TEXT (2-letter) | `'NH'` |
| `species` | `species_id` | INTEGER | `1234` (natural from source) |

### Source Integer Keys (bottle chain)

| Table | Primary Key | Type | Notes |
|-------|-------------|------|-------|
| `casts` | `cast_id` | INTEGER | Source counter (`Cst_Cnt`) — stable |
| `bottle` | `bottle_id` | INTEGER | Source counter (`Btl_Cnt`) — stable |

### Deterministic UUID Keys (derived tables with composite natural key)

| Table | Primary Key | Composite Key |
|-------|-------------|---------------|
| `ichthyo` | `ichthyo_uuid` | UUID v5 from `net_uuid, species_id, life_stage, measurement_type, measurement_value` |

### Sequential Integer Keys (derived tables only)

For derived/pivoted tables without source identifiers, sequential integers provide
a convenience primary key:

| Table | Primary Key | Sort Order for ID Assignment |
|-------|-------------|------------------------------|
| `lookup` | `lookup_id` | `ORDER BY lookup_type, lookup_num` |
| `segment` | `segment_id` | `ORDER BY time_beg` |
| `bottle_measurement` | `bottle_measurement_id` | `ORDER BY bottle_id, measurement_type` |
| `cast_condition` | `cast_condition_id` | `ORDER BY cast_id, condition_type` |
...
cruise.cruise_key (PK)   ←──  cruise.ship_key (FK)
    ↓                         cruise.cruise_uuid (unique, from source)
...
bottle_measurement        ←──  bottle_measurement.bottle_id (FK)
                               bottle_measurement.measurement_type → measurement_type
...
segment.segment_id (PK)  ←──  segment.site_uuid_beg, segment.site_uuid_end
```
...
each ingest notebook produces a `relationships.json` sidecar file that
records primary keys and foreign keys in a machine-readable format.
...
```json
{
  ...
  "provider": "swfsc",
  "dataset": "ichthyo",
  "primary_keys": { "cruise": "cruise_key", "ship": "ship_key" },
  "foreign_keys": [
    { "table": "cruise", "column": "ship_key",
      "ref_table": "ship", "ref_column": "ship_key" }
  ]
}
```

**Per-dataset files** are produced by `build_relationships_json()` in each
ingest notebook (ichthyo, bottle, ctd-cast) and written to
`data/parquet/{provider}_{dataset}/relationships.json`.

**Merged file** is created by `merge_relationships_json()` in
`release_database.qmd`, which unions PKs (last-writer-wins for shared
tables like `measurement_type`), concatenates and deduplicates FKs, and
adds cross-dataset FKs (e.g. `casts.cruise_key → cruise.cruise_key`).
The merged file is uploaded alongside parquet files in frozen releases.

**User access** via `calcofi4r::cc_get_dm()` downloads
`relationships.json` from the release and returns a `dm` object with
PKs/FKs applied, enabling `dm::dm_draw()` schema visualization.

### UUID Column Handling

- **Retain** `cruise_uuid`, `site_uuid`, `tow_uuid`, `net_uuid` in all output tables
  (frozen releases included) — these are the primary unique identifiers
...
FROM larvae
WHERE _source_file LIKE '%swfsc/ichthyo/larva.csv'
...
cc_make_view(con, template, view_name, base_table, column_definitions)
  ...
  # Templates: "casts_extra" (year, month, quarter, julian_day, lat/lon parts, cruise, db_sta_key from site_key)
...
flowchart LR
    subgraph archive["📁 GCS Archive"]
        archive_swfsc["swfsc/<br/>ichthyo"]
        archive_bottle["calcofi/<br/>bottle"]
    end
...
3. **Update `workflows/_targets.R`** ✅
   ...
   - Pipeline: CSV→parquet→DuckDB + manifest-based integration

4. **Cross-dataset integration (new)** ✅
   - **Ship matching**: `calcofi4db/R/ship.R` with `match_ships()`, `fetch_ship_ices()`, `derive_cruise_key_on_casts()`
   - **Cruise bridge**: Derive `cruise_key` (YYMMKK) from bottle casts via ship_key + datetime (now in `ingest_calcofi_bottle.qmd`)
   - **Taxonomy standardization**: `calcofi4db/R/taxonomy.R` with `standardize_species_local()`, `build_taxon_hierarchy()` (now in `ingest_swfsc_ichthyo.qmd`)
   - **Grid canonical source**: grid.parquet only from swfsc (loaded via `load_prior_tables()`)
...
2. **Each ingest notebook finalizes to Working DuckLake** ✅
   - `merge_ichthyo_bottle.qmd` eliminated; integration redistributed to ingest notebooks
   - Each notebook calls `finalize_ingest()` to push tables with provenance
   - `release_database.qmd` creates frozen release from Working DuckLake
...
### Phase 4b: UUID-First PK Revision (2026-02-24)

**Goal**: Revert from sequential `*_id` to source `*_uuid` primary keys per
discussion with Ed Weber. UUIDs minted at sea remain stable through QA/QC
cycles; sequential IDs do not.

1. **Updated SWFSC ingest workflow** ✅
   ...
   - Updated ichthyo FK: `net_uuid` (not `net_id`)
   - Updated segment: `site_uuid_beg`/`site_uuid_end` (not `site_id_*`)
   - Updated dm relationships, FK diagrams, metadata_derived.csv

2. **Updated bottle ingest workflow** ✅
   - Added rationale note: cast_id/bottle_id are stable source counters
   - bottle_measurement_id/cast_condition_id stay (derived tables)

3. **Updated README_PLAN.qmd** ✅
   - Rewrote Primary Key Strategy section
   - Documented UUID-first approach with rationale

4. **Next steps to update DuckLakes**:
   - Re-render `ingest_swfsc_ichthyo.qmd` (rebuilds wrangling DB + parquet + taxonomy + Working DuckLake)
   - Re-render `ingest_calcofi_bottle.qmd` (bottle tables + cruise bridge + Working DuckLake)
   - Re-render `ingest_calcofi_ctd-cast.qmd` (CTD tables + Working DuckLake)
   - Re-render `release_database.qmd` (validate + freeze + upload to GCS)
   - Update `calcofi4r::cc_get_db()` and schema app if needed
