...
format:
  html:
    ...
    # toc-depth: 4
    # toc-expand: 2
    number-sections: false # true  # md to exclude: {.unnumbered}
    code-fold: false # true
    code-tools: false # true
...
## 2. File Versioning Strategy

### 2.1 The Two-World Problem
...
### 2.2 Google Drive: The Human Interface
...
### 2.3 Google Cloud Storage: The Versioned Data Lake
...
### 2.4 Rclone Sync: Capturing Daily Changes
...
### 2.6 Reconstructing Historical State
...
### 2.7 Automation Schedule

```
┌─────────────────────────────────────────────────────────────────┐
│ Daily Sync Schedule (UTC)                                        │
├─────────────────────────────────────────────────────────────────┤
│ 00:00 - rclone sync runs                                        │
│       - Changed files archived with timestamp                    │
│       - Current folder updated to match Drive                    │
│       - Manifest generated                                       │
│                                                                   │
│ 00:15 - targets pipeline triggered (if changes detected)        │
│       - CSV → Parquet transformation                             │
│       - DuckDB database update                                   │
│       - DuckLake catalog update                                  │
│                                                                   │
│ 00:30 - Notification sent (if errors)                           │
└─────────────────────────────────────────────────────────────────┘
```
...
```
┌─────────────────────────────────────────────────────────────────┐
│                    DATA CONTRIBUTOR LAYER                        │
│                                                                   │
│   Google Drive (calcofi/data/)                                   │
│   ├── calcofi.org/bottle-database/*.csv                          │
│   ├── swfsc.noaa.gov/calcofi-db/*.csv                           │
│   └── coastwatch.pfeg.noaa.gov/erdCalCOFIlrvsiz/*.csv           │
└───────────────────────────┬─────────────────────────────────────┘
                            │
                            ▼ rclone sync --backup-dir
┌─────────────────────────────────────────────────────────────────┐
│                    VERSIONED DATA LAKE (GCS)                     │
│   GCP Project: ucsd-sio-calcofi                                  │
│                                                                   │
│   gs://calcofi-files/      # Versioned source files              │
│   ├── current/             # Mirror of Google Drive              │
│   ├── archive/             # Timestamped backups (YYYY-MM-DD)    │
│   └── manifests/           # Version control metadata            │
└───────────────────────────┬─────────────────────────────────────┘
                            │
                            ▼ targets pipeline (CSV → Parquet)
┌─────────────────────────────────────────────────────────────────┐
│                    INTEGRATED DATABASE                           │
│                                                                   │
│   gs://calcofi-db/         # Database storage                    │
│   ├── parquet/             # Transformed Parquet tables          │
│   ├── duckdb/calcofi.duckdb                                      │
│   ├── ducklake/            # DuckLake catalog (time travel)      │
│   └── tiles/               # PMTiles for vector maps             │
└───────────────────────────┬─────────────────────────────────────┘
                            │
                            ▼ publish workflows
┌─────────────────────────────────────────────────────────────────┐
│                    PUBLISHING LAYER                              │
│                                                                   │
│   ├── OBIS (DarwinCore Archive)                                  │
│   ├── EDI (EML + data files)                                     │
│   ├── ERDDAP (NetCDF/CSV)                                        │
│   └── ODIS (schema.org JSON-LD)                                  │
└─────────────────────────────────────────────────────────────────┘
```
...
### 4.2 Function Reuse Across Contexts
...
### 4.3 New Functions to Implement
...
## 5. Workflow Dependency Management with `targets`
...
```
Development Cycle:
1. Explore data    → explore_*.qmd (ad-hoc, not in pipeline)
2. Design ingest   → ingest_*.qmd (develop transformation logic)
3. Add to targets  → _targets.R (formalize as target)
4. Run pipeline    → tar_make() (execute with caching)
5. Publish         → publish_*.qmd (uses targets outputs)

Update Cycle:
1. New data arrives → Google Drive
2. Sync to GCS      → rclone (scheduled or manual)
3. Run pipeline     → tar_make() (only recomputes changed targets)
4. Republish        → Automatic via target dependencies
```
...
raw_bottle ──┬──► pqt_bottle ──┐
             │                 │
raw_cast ────┴──► pqt_cast ────┼──► db_manifest ──► duckdb_file ──► obis_archive
                               │
...
### 6.1 Recommended Changes to `docs/`
...
### 6.2 Where Documentation Should Live
...
## 9. Migration Strategy: PostgreSQL → DuckDB

### 9.1 Current PostgreSQL Dependencies
...
### 9.2 Vector Tiles: pg_tileserv → PMTiles

The current architecture uses `pg_tileserv` to serve vector tiles from PostGIS for Shiny mapping apps. The migration path:

#### Current Flow
```
PostGIS (geometries) → pg_tileserv → Leaflet/Mapbox in Shiny
```

#### Target Flow
```
GeoParquet/GeoJSON → tippecanoe → PMTiles → mapgl::add_pmtiles_source()
```
