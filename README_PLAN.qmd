...
├── workflow.R     # Workflow outputs to GCS (@concept workflow) [NEW]
...
├── validate.R     # FK/lookup validation functions (@concept validate) [NEW]
├── wrangle.R      # Local DuckDB wrangling functions (@concept wrangle) [NEW]
...
write_ingest_outputs(data_info, provider, dataset, gcs_bucket)
  ...
  # 2. Write parquet files to gs://calcofi-db/ingest/{provider}/{dataset}/parquet/
...
2. **`write_ingest_outputs()`**: Writes parquet files + manifest.json to `gs://calcofi-db/ingest/{provider}/{dataset}/`
...
3. Writes parquet files to `gs://calcofi-db/ingest/{provider}/{dataset}/`
...
1. **Migrate metadata to `workflows/metadata/`**
   - Create `workflows/metadata/{provider}/{dataset}/` directories
   - Copy redefinition files from `calcofi4db/inst/ingest/`
   - Update workflows to use `metadata_dir` parameter
...
2. **Create `calcofi4db/R/workflow.R`**
   ...
   - `create_database_from_ingests()`: Combine ingest outputs

3. **Update `workflows/_targets.R`**
   - Define ingest targets using new functions
   - Separate ingest outputs from final database
   - Each ingest writes to `gs://calcofi-db/ingest/{provider}/{dataset}/`

4. **Update publish workflow**
   - Modify `publish_larvae_to_obis.qmd` to read from DuckDB
   - Add bottle data to EMoF extension

### Phase 3: Working DuckLake
