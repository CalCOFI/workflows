...
| `variable.csv` | One row per **crosswalk key** `measurement_type.variable` points at — a label, description, units, category and NERC P01 held once for the key rather than repeated on every member series (D3, "Measurements catalog" plan 2026-09-10). **Read with `calcofi4db::read_variable()`, appended with `register_variables()` (never a bare `write_csv`), checked against `measurement_type.csv` with `check_variable_registry()`.** A `variable` unifies two series only where the NERC P01 concept is identical (or differs only by a method qualifier for the same quantity), the units agree or convert, the series come from the same kind of sample, and the two are not plausibly the same physical samples (`libs/measure_variable_crosswalk.R` is the reproducible query); `register_variables()` refuses a row whose `nerc_p01` disagrees with a member series' own value, so criterion (i) cannot be violated by a typo. |
