...
| WS7 bridge | ✅ phase 1 (DuckDB `postgres` ext) in calcofi4r + loader; ✅ phase 2: `calcofi-postgis:18-3.6-duckdb` image (postgis + pg_duckdb 1.1.1) **live**, `duckdb.postgres_role=calcofi_reader`, `release.{cruise,ship,dataset}` views over the public Parquet (`postgis/init/50_release_views.sql`; version pinned — re-run per release). ⏳ nightly accepted-flags → GCS → ingest round trip not built yet | — |
| WS8 CTD schema | ✅ `postgis/init/40_ctd.sql` applied: immutable `ctd.file`/`ctd.scan` (82 verbatim columns), `ctd.scan_column` dictionary, IODE `ctd.qual_code` + `ctd.source_qual_code`, `ctd.flag` (RLS: anyone proposes, curators review; audit), `ctd.cast` matview, `v_scan_best`/`v_scan_qc`/`v_scan_clean`, `refresh_derived()`; rules verified in a rolled-back test. Loader: `workflows/libs/pg_ctd.R` + `load_pg_ctd.qmd`; parquet build of the 409 db-CSV files (4.5 GB, ~7.5 M scans) → `gs://calcofi-db/pg/ctd_scan_raw/` → server-side load (in progress at time of writing; see session notes) | — |
