...
librarian::shelf(
  DT, glue, janitor, mapview, purrr, readr, readxl, 
...
# rename columns
cols_orig       <- names(d_cast)
cols            <- make_clean_names(cols_orig)
names(d_cast) <- cols
d_cols <- tibble(
  table       = "ctdcast",
  column_orig = cols_orig,
  column      = cols)
...
# rename columns
cols_orig       <- names(d_DIC)
cols            <- make_clean_names(cols_orig)
cols[which(cols_orig == "Cast_Index")]   <- "cst_cnt"
cols[which(cols_orig == "Bottle_Index")] <- "btl_cnt"
names(d_DIC) <- cols
d_cols <- bind_rows(
  d_cols,
  tibble(
...
```{r columns_renamed}
# write table to database
dbWriteTable(con, "columns_renamed", d_cols, overwrite=T)
