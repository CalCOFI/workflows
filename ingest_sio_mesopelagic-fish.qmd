...
calcofi:
  target_name: ingest_sio_mesopelagic_fish
  workflow_type: ingest
  dependency:
    - ingest_swfsc_ichthyo
  output: data/parquet/sio_mesopelagic-fish/manifest.json
  provider: sio
  dataset: mesopelagic-fish
  workflow_url: https://calcofi.io/workflows/ingest_sio_mesopelagic-fish.html
  questions_file: metadata/sio/mesopelagic-fish/questions.csv
  dataset_meta:
    ...
    dataset_name_short: Mesopelagic Fish
    category: Mesopelagic Fish
    color: "#5c7cfa"
    description: >
    ...
    # source: the DOI already embedded in citation_main above; confirmed
    # https://doi.org/10.6075/J0BZ64DH resolves (302 -> the Library object).
    # The Library object page itself (bb9217084g) is behind a Cloudflare/Anubis
    # bot check that blocks automated fetches, so no further metadata (e.g. a
    # second, object-level DOI) could be independently verified. checked 2026-09-03
    doi: 10.6075/J0BZ64DH
    # normalized to SPDX id (was "CC BY 4.0")
    license: CC-BY-4.0
  ...
  tables_owned:
    ...
    - {table: obs, shared: true, note: "core occurrence headline (bio, per-tow tally)"}
    - {table: taxon, shared: true, note: "shared taxa reference"}
    - {table: dataset_taxon, shared: true, note: "scientific name -> taxon_key crosswalk"}
...
Provider `sio` (SIO curates the dataset; CalCOFI is the sampling
...
`mesopelagic_fish_taxon`, which `resolve_dataset_taxon()` then crosswalks into the
...
(`design_env-bio-consolidation.md`) — a
...
# calcofi4db >= 3.29.0). `ds_taxa_code` IS the verbatim spreadsheet header (the
# source has no local code), so `obs` joins dataset_taxon on the name it stores
# on the measurement row -- which is exactly why the " sp." suffix must NOT be
# cleaned out of the code: rewriting it would orphan every observation of those
# six taxa. The lookup name is cleaned instead, inside the xref name fallback.
d_vocab <- dbGetQuery(con, "
  SELECT scientific_name           AS ds_taxa_code,
         scientific_name           AS ds_scientific_name,
         CAST(worms_id AS INTEGER) AS worms_id
  FROM mesopelagic_fish_taxon ORDER BY taxon_id")
...
# the ingest asserts its own crosswalk (D6): every name the measurements use is
# in the vocabulary and every staged row keys an authority id. Halts on any
# finding.
obs_codes <- dbGetQuery(con,
  "SELECT DISTINCT scientific_name AS c FROM mesopelagic_fish_measurement
   WHERE scientific_name IS NOT NULL")$c
chk_taxon <- check_dataset_taxon(con, ds_key, codes = obs_codes)
cat(glue("vocabulary: {n_staged} species staged; taxon {n_taxon}, ",
...
# falls back to a dataset-local key via resolve_dataset_taxon()).
