...
calcofi:
  ...
  dataset_meta:
    ...
    # left empty; see metadata/cce-lter/zooscan/questions.csv Q06 (proposed to
    # Ohman/Frants). checked 2026-09-03
    citation_main: ""
    ...
    link_others: []
    # NOT CC-BY-4.0: the portal's "Data Use Policy" section is bespoke, non-CC
    # text: "intended for scholarly use... Use or reproduction of any materials
    # herein for any commercial purpose is prohibited without prior written
    # permission from Dr. Mark D. Ohman."
    # source: https://oceaninformatics.ucsd.edu/zooscandb/ (Data Use Policy), checked 2026-09-03
    license: custom
    license_url: https://oceaninformatics.ucsd.edu/zooscandb/
    ...
    # additional citation.
    # source: https://oceaninformatics.ucsd.edu/zooscandb/ (Data Acknowledgement Policy), checked 2026-09-03
...
# cruise_key: the reference cruise whose date span holds the station date, else
# the source's own year/month designation, keys not in `cruise` left NULL
# (calcofi4db::resolve_cruise_key)
dbWriteTable(con, "zk", d_sample0 |>
  transmute(sample_key, ship_key, datetime = station_date,
            cruise_ym = sprintf("%04d%02d", year, month)), overwrite = TRUE)
resolve_cruise_key(con, "zk", datetime_col = "datetime", cruise_ym_col = "cruise_ym",
                   require_in_cruise = TRUE) |>
  datatable(caption = "cruise_key resolution method")
d_sample <- d_sample0 |>
  left_join(d_ship, by = "ship_key") |>
  left_join(dbGetQuery(con, "SELECT sample_key, cruise_key FROM zk"), by = "sample_key") |>
...
# the authorities. There is no zooscan arm inside calcofi4db any more -- a column
...
d_vocab <- dbGetQuery(con, "
  SELECT CAST(taxon_id AS VARCHAR) AS ds_taxa_code,
         ...
         taxon_zooscan AS ds_common_name,
         CAST(aphia_id AS INTEGER) AS worms_id,
         rank
  FROM zooscan_taxon ORDER BY taxon_id")
...
# the ingest asserts its own crosswalk (D6). ZooScan's bioclasses are
# OPERATIONAL, not taxonomic: four of the 23 name a life stage or an image
# artefact rather than an organism, so they key dataset-locally by declaration --
# one key at a time, so a genuinely unresolved taxon cannot hide among them (Q03).
local_allow <- tibble::tribble(
  ~ds_taxa_code, ~why,
  "13", "eggs - a life stage, not a taxon",
  "15", "multiples - more than one organism in the vignette",
  "16", "nauplii - a crustacean larval stage across taxa",
  "18", "others - the residual bioclass")
obs_codes <- dbGetQuery(con,
  "SELECT DISTINCT CAST(taxon_id AS VARCHAR) AS c FROM zooscan_measurement
   WHERE taxon_id IS NOT NULL")$c
chk_taxon <- check_dataset_taxon(
  con, ds_key, allow = paste0(ds_key, ":", local_allow$ds_taxa_code),
  codes = obs_codes)
cat(glue("vocabulary: {n_staged} bioclasses staged; {nrow(local_allow)} keyed ",
