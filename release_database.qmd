...
# review (currently dfw_dungeness-crab) is staged without leaking into a release.
...
core_shard_tables <- c(
  "sample", "obs", "obs_attribute", "sample_measurement", "obs_ctd_full",
...
# cce-lter_picoplankton-bacteria and ucsd_sio_mesopelagic-fish, which orphaned
...
the core via `calcofi4db::emit_core_tables()` — the single authoritative
projection — and writes its slice as parquet. `assemble_core()` unions those
...
dbExecute(con_wdl, glue(
  "CREATE OR REPLACE TABLE measurement_type AS
   ...
   FROM read_csv_auto('{here('metadata/measurement_type.csv')}')"))

# --- assemble the core from the per-dataset shards --------------------------
# Each ingest emits its own slice via calcofi4db::emit_core_tables(), which is
# the single authoritative projection. This step only concatenates: it UNIONs the
...
core_n <- assemble_core(con_wdl, root = here(), supplemental = build_obs_ctd_full)
