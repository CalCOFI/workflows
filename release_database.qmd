...
database from the Working DuckLake. This is the "caboose" notebook that
always runs last, after all ingest notebooks complete.
...
```{mermaid}
%%| label: fig-workflow
%%| fig-cap: "Pipeline: ingest notebooks build Working DuckLake → release caboose creates frozen release"
...
flowchart LR
    ...
    subgraph wdl["Working DuckLake"]
        w["~21 tables<br/>with provenance"]
    end
    ...
    style wdl fill:#fff3e0,stroke:#ef6c00
...
if (all(c("casts", "grid") %in% tbls)) {
  if ("grid_key" %in% DBI::dbListFields(con_wdl, "casts") &&
      "grid_key" %in% DBI::dbListFields(con_wdl, "grid")) {
    grid_orphans <- dbGetQuery(con_wdl,
...
if ("cruise" %in% tbls) {
  cruise_dups <- dbGetQuery(con_wdl,
...
if ("casts" %in% tbls) {
  bridge_stats <- dbGetQuery(con_wdl,
    "SELECT
...
ichthyo_tbls <- intersect(c("cruise", "ship", "site", "tow", "net"), schema_tbls)
species_tbls <- intersect(c("ichthyo", "species", "lookup", "taxon", "taxa_rank"), schema_tbls)
spatial_tbls <- intersect(c("grid", "segment"), schema_tbls)
bottle_tbls  <- intersect(
  c("casts", "bottle", "bottle_measurement", "cast_condition", "measurement_type"),
  schema_tbls)
ctd_tbls <- intersect(
  c("ctd_cast", "ctd_measurement", "ctd_summary"), schema_tbls)

if (length(ichthyo_tbls) > 0) d <- d |> dm::dm_set_colors(lightblue    = !!ichthyo_tbls)
if (length(species_tbls) > 0) d <- d |> dm::dm_set_colors(lightyellow   = !!species_tbls)
if (length(spatial_tbls) > 0) d <- d |> dm::dm_set_colors(lightgreen    = !!spatial_tbls)
if (length(bottle_tbls)  > 0) d <- d |> dm::dm_set_colors(pink          = !!bottle_tbls)
if (length(ctd_tbls)     > 0) d <- d |> dm::dm_set_colors(lavender      = !!ctd_tbls)
...
release_notes <- paste0(
  ...
  "- **Total Size**: ",
    round(sum(freeze_stats$file_size) / 1024 / 1024, 1), " MB\n\n",
  "## Data Sources\n\n",
  ...
  "    'https://storage.googleapis.com/calcofi-db/ducklake/releases/",
    release_version, "/parquet/ichthyo.parquet')\n",
  "  LIMIT 10\")\n",
  "```\n\n",
  "Or use calcofi4r:\n\n",
  "```r\n",
