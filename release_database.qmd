...
if (file.exists(meta_json_path)) {
  ...
  meas_var_path <- here("metadata/variable.csv")
  meas_var <- if (file.exists(meas_var_path))
    readr::read_csv(meas_var_path, na = "", show_col_types = FALSE) else NULL
  if (is.null(meas_var))
    cat("metadata/variable.csv absent: every measurement falls back to its canonical series' ",
        "description and carries the `no_label` flag\n", sep = "")
