...
erd <- cc_erd(con_wdl, layoutdir = "TB")
plot(erd)

erd <- cc_erd(
  con_wdl,
  rels_path = file.path(dir_frozen, "relationships.json"),
  colors = list(
...
# lns <- cat(erd) |> separate("\n")

# erd <- readLines("diagrams/erd.mmd") |> paste(collapse = "\n")
plot(erd)
...
geom_tbls_schema <- DBI::dbGetQuery(
  con_wdl,
...
if (is.null(d)) {
  ...
  rels_paths <- c(
    here("data/parquet/swfsc_ichthyo/relationships.json"), # TODO: add
    here("data/parquet/calcofi_bottle/relationships.json"), # TODO: add
  ...
  if (length(rels_paths) > 0) {
    ...
    d <- tryCatch(dm::dm_add_fk(d, casts, ship_key, ship), error = function(e) {
      d
    })
    d <- tryCatch(dm::dm_add_fk(d, casts, grid_key, grid), error = function(e) {
      d
    })
