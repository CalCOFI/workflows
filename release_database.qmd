...
TAXON_LOCAL_ALLOW <- c(
  ...
  # phytoplankton: "other" buckets + codes absent from the source Definitions
  # sheet (Q05 in calcofi/phytoplankton/questions.csv)
  paste0("calcofi_phytoplankton:",
         c("4", "40", "59", "218", "229", "231", "232", "300", "337",
           "454", "532", "540", "596", "597")))
...
d_cov <- check_cruise_coverage(con_wdl, max_orphan_cruises = ORPHAN_CRUISES_MAX)
...
tighten <- d_cov$dataset_key[
  d_cov$dataset_key %in% names(ORPHAN_CRUISES_MAX) &
    d_cov$cruises_no_obs < ORPHAN_CRUISES_MAX[d_cov$dataset_key]]
if (length(tighten))
  cat(glue(
    "ORPHAN_CRUISES_MAX can be tightened: ",
    "{paste(sprintf('%s -> %d', tighten,
                    d_cov$cruises_no_obs[match(tighten, d_cov$dataset_key)]),
            collapse = ', ')}\n"))
```
