...
for (i in seq_len(nrow(geom_cols))) {
  tb <- geom_cols$table_name[i]; cl <- geom_cols$column_name[i]
  # CREATE OR REPLACE ... SELECT * REPLACE rather than UPDATE: mutating a
  # CRS-tagged GEOMETRY column in place hits the DuckDB checkpoint bug that
  # CLAUDE.md documents (GeoColumnData::GetChildStats, through >= v1.5.1).
  DBI::dbExecute(con_wdl, glue(
    "CREATE OR REPLACE TABLE {tb} AS
     SELECT * REPLACE (ST_SetCRS({cl}, 'EPSG:4326') AS {cl}) FROM {tb}"))
}
cat(glue("normalized {nrow(geom_cols)} geometry column(s) to EPSG:4326: ",
         "{paste(sprintf('%s.%s', geom_cols$table_name, geom_cols$column_name), collapse=', ')}"), "\n")
