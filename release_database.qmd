...
# default `obs` carries CTD via the thinned ctd_thin (~5.5M). Flip off for a fast
# render; the full pipeline builds it.
build_obs_ctd_full <- TRUE
...
if (build_obs_ctd_full && exists("n_full"))
  stopifnot("obs_ctd_full parity" =
    ...
    q("SELECT COUNT(*) n FROM read_parquet('{ctd_meas_glob}', hive_partitioning=true) m JOIN ctd_cast cc ON m.ctd_cast_uuid=cc.ctd_cast_uuid WHERE cc.grid_key IS NOT NULL"))
...
# 2. upload derived tables from local parquet (cruise, measurement_type)
