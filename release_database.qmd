...
stopifnot("GEBCO tif not found; set CALCOFI_GEBCO_TIF" = file.exists(gebco_tif))
...
cat(glue("sample_root {n_root} · obs_bio {n_bio} · obs_env {n_env} rows\n"))
...
unlink(c(grid_geojson_path, spatial_geojson_path))
...
# sample_spatial is a core relationship table (ERD + default table list); the three browser objects
# are supplemental — hosted and catalogued, hidden from cc_get_db() by default like obs_ctd_full
core_keep         <- c(core_keep, "sample_spatial")
supplemental_keep <- c(supplemental_keep, "sample_root", "obs_bio", "obs_env")
```
...
core_spec <- dplyr::bind_rows(
  tibble(
    ...
    partitioned = c(FALSE,    TRUE,  FALSE,           FALSE,
                    FALSE,    FALSE,           FALSE)),
  # browser-shaped objects built in [browser_objects]; obs_env is one object per variable
  tibble(table       = c("sample_spatial", "sample_root", "obs_bio", "obs_env"),
         partitioned = c(FALSE,            FALSE,         FALSE,     TRUE)),
...
# in memory only for a staging run (whose date has no section on disk)
...
for (f in c(coverage_path, coverage_stations_path, grid_geojson_path, spatial_geojson_path))
