...
# where con_wdl spills, and how much, from the environment when set. An in-memory
# connection spills under tempdir(), so a killed render orphans its spill files
# (103 GB on 2026-09-24), and DuckDB's default spill cap (90 % of free disk) then
# shrinks every retry. A known temp_directory can be cleaned by whoever stops a
# run; an explicit max_temp_directory_size fails fast instead of filling the disk.
wdl_config <- list(
  temp_directory          = Sys.getenv("CALCOFI_DUCKDB_TEMP_DIR", ""),
  max_temp_directory_size = Sys.getenv("CALCOFI_DUCKDB_MAX_TEMP", ""))
wdl_config <- wdl_config[nzchar(unlist(wdl_config))]
if (!is.null(wdl_config$temp_directory))
  dir.create(wdl_config$temp_directory, recursive = TRUE, showWarnings = FALSE)
con_wdl <- get_duckdb_con(":memory:", config = wdl_config)
wdl_set <- function(s) dbGetQuery(con_wdl, glue("SELECT current_setting('{s}') AS v"))$v
cat(glue("con_wdl: memory_limit {wdl_set('memory_limit')}, threads {wdl_set('threads')}, ",
         "temp_directory {wdl_set('temp_directory')}, ",
         "max_temp_directory_size {wdl_set('max_temp_directory_size')}"), "\n")
...
dbWriteTable(con_wdl, "_measurement_type_reg", as.data.frame(d_meas_type_reg),
             overwrite = TRUE)
# the label registry for the crosswalk KEY (metadata/variable.csv, measurements catalog plan § D3),
# read once here and used twice: build_coverage() puts `label` onto coverage.json's variables[] (the
# explorer's hook, in place of its hard-coded UNIFIED labels) and step 3b'' puts it on every
# measurement of measurements.json. read_variable() skips the file's leading `#` comment line and
# validates, which a bare read_csv() would not. Absent, every measurement carries `no_label` and
# nothing is invented.
meas_var <- if (file.exists(here("metadata/variable.csv")))
  read_variable(here("metadata/variable.csv")) else NULL
cat(glue("metadata/variable.csv: {if (is.null(meas_var)) 'absent — labels fall back to the ",
         "canonical series description (no_label)' else paste(nrow(meas_var), 'labelled key(s)')}\n"))
...
TAXON_LOCAL_ALLOW <- c(
  ...
  # picoplankton: the flow cytometer counts four populations, and two of them are
  # operational classes rather than taxa (measurement-faces plan D9, 2026-09-12).
  # Synechococcus and Prochlorococcus key worms:160572 / worms:345515; these two
  # cannot, and keying them to Eukaryota / Bacteria would be a lazy lookup that
  # says something false about what was counted.
  "cce-lter_picoplankton-bacteria:picoeukaryotes",  # a <2-3 um SIZE fraction
  "cce-lter_picoplankton-bacteria:het_bacteria",    # the non-cyanobacterial fraction
...
the `pipeline-targets` skill, § *The CTD team's PostgreSQL database*) and records how many it
...
# site_key is one spelling per station (calcofi4db 4.8.0: append_sample() normalises, this gates).
# v2026.09.06 shipped 28 CTD casts with the source's own Sta_ID forms ("93.3    26.4"); harmless for a
# map (grid_key is the join), fatal for anything keyed on the station — the climatology and the
# sections now are.
sk_res <- check_site_key_format(con_wdl)
datatable(sk_res, caption = "site_key format per dataset (n_bad must be 0; NULL is a sample with no station)")
...
dataset). See the `cruise-key` skill, § "Provider UUIDs are columns; the cruise key is checked
...
d_cki <- check_cruise_key_integrity(
  ...
  ratchets = list(
    ...
    key_null_max      = c(    # root samples with NULL cruise_key, per dataset; only ever down
      ...
      # 216 examined ones only (WS-C), 97 of whose log entries resolved no
      # cruise; 97 -> 1 once the ingest keys them by station occupation + span
      # (#109, calcofi4db 4.17.0). The one left is Q15 (2006-04-23, two ships).
      "cdfw_dungeness-crab" = 1L,
...
# station (sample.site_key since 4.8.0 — the inshore grid cells hold 2–4 stations, so the cell blended them;
# grid_key rides along as the station's modal cell) x calendar month x 10 m depth_bin x measurement type over
# 1993-2013, >= 5 cruises per cell (3 until v2026.09.06), the window
...
# min_cruises = 5, not the function's default 3: Rasmus Swalethorp (2026-09-09) — a three-cruise cell
# is too thin a baseline for an anomaly a reader will act on; the consumers show n_cruises per cell.
# Measured on v2026.09.06: 5 keeps 577,113 of 768,880 cells (75 %; 87 % of CTD temperature cells).
n_clim <- build_climatology(con_wdl, qual_ok_sql = qual_sql, min_cruises = 5L)
cat(glue("sample_root {n_root} · obs_bio {n_bio} · obs_env {n_env} · climatology {n_clim} rows\n"))
datatable(dbGetQuery(con_wdl, "
  SELECT dataset_key, count(*) AS n_cells, count(DISTINCT measurement_type) AS n_types, count(DISTINCT site_key) AS n_stations, count(DISTINCT grid_key) AS n_cells_grid,
  ...
  FROM climatology GROUP BY 1 ORDER BY 1"), caption = "climatology — cells per dataset (each ≥ 5 cruises; the window is on every row)")
...
cov <- build_coverage(con_wdl, release_version, variable = meas_var)
...
jsonlite::write_json(
  build_spatial_layers(
    ...
    # the reference rows' counts (the land mask, the gazetteer labels; plan 2026-09-09 D52, calcofi4db >= 4.8.0)
    reference_json = here("data/parquet/spatial/reference_layers.json")),
...
`calcofi4db::check_release_relationships()` (>= 4.7.0) walks every declared key on the
assembled tables — each primary key unique and non-NULL, each non-NULL foreign key resolving in
its referenced column — and writes `integrity.json` beside `catalog.json`, so the schema browser
and the docs can show *measured* beside every key instead of trusting the diagram. Until
2026-09-08 only the core primary keys (`check_core_pk_unique()`, above) and the `cruise_key` edges
(`check_cruise_key_integrity()`) were gated; the other ~50 declared edges were true by construction.
A NULL foreign key is a nullable edge and is permitted; an orphan stops the release. This runs here,
after `browser_objects`, because it is the first point at which every released table exists.

```{r}
#| label: relationship_integrity

# every released table declares its primary key. The release-built ones (obs_bio, obs_env,
# sample_root, sample_spatial, climatology, spatial, …) come from core_relationships(), which the
# ingest sidecars merged in [combined_schema] never carry; they are filled in here.
released_tbls <- setdiff(DBI::dbListTables(con_wdl),
  c("_meta", "_sp_update", "casts_derived", "ctd_cast_derived", "_measurement_taxon"))
rels_merged <- jsonlite::fromJSON(rels_merged_path, simplifyVector = FALSE)
rel_pks <- core_relationships(released_tbls)$primary_keys
for (tb in names(rel_pks))
  if (is.null(rels_merged$primary_keys[[tb]])) rels_merged$primary_keys[[tb]] <- rel_pks[[tb]]
rels_merged$primary_keys <- rels_merged$primary_keys[
  intersect(names(rels_merged$primary_keys), released_tbls)]

# an FK edge ships only when both ends do. relationships_cross.csv still records how each ingest
# resolved its keys on the per-dataset tables the release retired in 2026-07 (casts, ctd_cast,
# zooplankton_tow, …); those rows stay in the CSV, and in relationships_all.csv as released = FALSE,
# and leave relationships.json so the ERD, db-schema and this check see the released graph alone.
fk_released <- vapply(rels_merged$foreign_keys, function(fk)
  fk$table %in% released_tbls && fk$ref_table %in% released_tbls, logical(1))
n_dropped <- sum(!fk_released)
rels_merged$foreign_keys <- rels_merged$foreign_keys[fk_released]
# one edge once: the ingest sidecars and relationships_cross.csv both declare the core edges
# (obs.sample_key -> sample, …), and [combined_schema] appended the cross rows without deduplicating
fk_id <- vapply(rels_merged$foreign_keys, function(fk)
  paste(fk$table, fk$column, fk$ref_table, fk$ref_column), character(1))
rels_merged$foreign_keys <- rels_merged$foreign_keys[!duplicated(fk_id)]
jsonlite::write_json(rels_merged, rels_merged_path, auto_unbox = TRUE, pretty = TRUE, null = "null")
rels_all_path <- file.path(dir_frozen, "relationships_all.csv")
if (file.exists(rels_all_path)) {
  rels_all <- readr::read_csv(rels_all_path, show_col_types = FALSE)
  rels_all$released <- rels_all$from_table %in% released_tbls & rels_all$to_table %in% released_tbls
  readr::write_csv(rels_all, rels_all_path)
}
cat(glue("relationships.json: {length(rels_merged$primary_keys)} primary keys over ",
         "{length(released_tbls)} released tables; {length(rels_merged$foreign_keys)} foreign keys ",
         "({n_dropped} ingest-only edges left in relationships_all.csv)\n"))

integrity_path <- file.path(dir_frozen, "integrity.json")
d_integrity <- check_release_relationships(
  con_wdl, rels_merged_path, path = integrity_path, version = release_version, halt = TRUE)
cat(glue("integrity: {sum(d_integrity$primary_keys$status == 'ok')} primary keys and ",
         "{sum(d_integrity$foreign_keys$status == 'ok')} foreign keys measured ok; ",
         "{d_integrity$n_skipped} skipped\n"))
datatable(d_integrity$foreign_keys,
  caption = "foreign keys measured on the assembled release (n_null is permitted; an orphan fails)")
```

```{r}
#| label: release_notes_narrative

# RELEASES.md is the database's NEWS file (`release-run` skill, § "RELEASES.md is not
...
# upload relationships.json and the measured keys beside it
...
integrity_json <- file.path(dir_frozen, "integrity.json")
if (file.exists(integrity_json))
  put_gcs_file(integrity_json,
    glue("gs://{gcs_bucket}/{gcs_release}/integrity.json"))
...
if (file.exists(meta_json_path)) {
  ...
  # this release's own STAC tree is uploaded in §3d, AFTER this check: a dataset in its first
  # release has no collection there yet, so a dead URL under that root is `url_pending` here and
  # is re-probed right after the upload (calcofi4db >= 4.17.1; calcofi_ctd-derived, 2026-09-24)
  d_catalog_chk <- check_dataset_catalog(datasets_rec, cat_registries, network = net_ok,
                                         pending_base = release_stac_base(release_prefix))
  ...
  # 3b'. taxa.json — the species catalog record (plan 2026-09-09 § D6/D7; calcofi4db >= 4.9.0) ----
  # One entry per row of THIS release's `taxon` table with an observation at or below it — the
  # observed taxa plus their ancestors (2,410 of 2,614 rows on v2026.09.06; the 204 vocabulary-only
  # rows get no page and are listed under the dataset that declares them) — with its lineage, ids and
  # groups, the direct counts and the rollup over its descendants, and one block per dataset that
  # observed it carrying that dataset's own name for the taxon from `dataset_taxon`, flagged where
  # the authority says otherwise (synonym · sp_to_genus · rekeyed · no_name). calcofi.io/species/
  # generates one page per entry from this file; nothing on those pages is authored.
  # It is built AFTER datasets.json because it reads that record's datasets[] for the short names,
  # colours and categories the species pages draw their dataset dots with — so a dot cannot disagree
  # between /datasets/ and /species/ — and from con_wdl, which holds the assembled taxon,
  # dataset_taxon, taxon_group and obs_bio. check_taxa_catalog() is a release gate like
  # check_dataset_catalog(): the counts are re-measured against obs_bio, every parent_taxon_key must
  # resolve inside taxa[], the roots' rollups must sum to obs_bio's keyed row count, slugs must be
  # unique and every dataset must be in the record. The record is GENERATED, so a failing row is a
  # bug in the builder or a break in the release's own tables — never something a question excuses.
  taxa_rec  <- build_taxa_catalog(con_wdl, datasets_rec, release_version,
                                  as.character(catalog$release_date))
  taxa_path <- write_taxa_catalog(taxa_rec, dir_frozen)
  validate_taxa_catalog(taxa_path)
  d_taxa_chk <- check_taxa_catalog(taxa_rec, con_wdl, datasets_rec)
  assert_taxa_catalog(d_taxa_chk)
  d_taxa_chk |>
    datatable(caption = "taxa catalog check: every row must be ok (the record is generated from the release, so a failure is a bug, not a data gap)")
  put_gcs_file(taxa_path, glue("gs://{gcs_bucket}/{gcs_release}/{basename(taxa_path)}"))
  cat(glue("taxa.json: {taxa_rec$counts$pages} pages ({taxa_rec$counts$taxa_observed} observed taxa, ",
           "{taxa_rec$counts$species_observed} at species rank, of {taxa_rec$counts$taxon_rows} taxon rows) · ",
           "{taxa_rec$counts$datasets} datasets · {round(file.size(taxa_path) / 1e6, 2)} MB\n"))

  # 3b''. measurements.json — the measurements catalog record (plan 2026-09-10 § D2/D4,
  # Appendix A; calcofi4db >= 4.12.0) ----
  # taxa.json's twin for the environment half of the catalog: one entry per MEASUREMENT KEY the
  # release's obs_env carries — the registry's `variable` where one is set (the bottle's
  # `temperature` and the CTD's `temperature_ave` are both `temperature`), else the
  # measurement_type — with one `series[]` per measurement_type x dataset carrying that dataset's
  # own column and flag column, its values by year, calendar month, depth band and quality code,
  # the observed quantiles, the registry's declared bounds and the NERC ids. 79 keys over 84 series
  # on v2026.09.06. calcofi.io/measurements/ generates one page per entry; nothing on those pages
  # is authored — including the label, which comes from metadata/variable.csv and, absent a row,
  # falls back to the canonical series' registry description with a `no_label` flag.
  ...
  # sensor_vs_mean · same_casts): P01 identity says two series name the same quantity, never that
  # they may be pooled — the CTD files' own bottle table is plausibly the same physical bottles as
  # the bottle dataset.
  # `observed{}` is computed WITHIN each series' declared bounds and what falls outside is counted
  # in `out_of_bounds{n, min, max}`, so a page's range is the range a reader would use while the
  # breach stays visible; the raw counts (n_values, years, months, depth bands, qual) are untouched,
  # which is why the arithmetic gate still equals obs_env's row count. `full_rows` counts obs_env
  # plus the full-resolution supplementals from con_wdl, which carries them.
  # It is built AFTER datasets.json for the same reason taxa.json is: the dataset dots come from
  # that record. check_measurements_catalog() is a release gate — the sum of series[].n_values over
  # every key must be obs_env's row count (a series counted twice is exactly what that catches),
  # every years{} must sum to at most its series, every measurement_type must be registered and
  # every dataset_key must be in the record. The record is GENERATED, so a failing row is a bug in
  # the builder or a break in obs_env, never something a question excuses.
  # `meas_var` is metadata/variable.csv, read once with read_variable() where the registries
  # are loaded and already on coverage.json's variables[]
  # schema 1.1 (plan 2026-09-11 "Measurement faces", § D6/D8, Appendix A): the record also
  # carries, per key, the five registries CalCOFI authors under metadata/ —
  # measurement_{face,chem,method,scale,why}.csv, the page's WHAT / HOW / WHY — and `anomaly`,
  # the yearly departure from the release's own `climatology` per depth band (obs_env joined on
  # dataset_key, measurement_type, site_key, calendar month and 10 m bin; qual_ok only, because a
  # provider's flag outranks a bound in anything derived; the mean per cruise, then per year; a
  # unified key merges its series first). `observed{}` is now computed inside qual_ok as well as
  # inside the declared bounds and `n_flagged` (n_values - qual_ok_n) rides beside it, so the
  # measurement page's flagged heads-up READS the record instead of recomputing it.
  # Every 1.1 field is additive and absent when its registry is: a metadata/ directory with none
  # of the five files writes exactly the 1.0 record.
  # `registries` takes the directory OR a named list of data frames, so when WS-MF1's readers land
  # this becomes `registries = list(chem = read_measurement_chem(here("metadata/measurement_chem.csv")),
  # method = …, scale = …, why = …, face = …)` — the validators then run at read time rather than
  # the builder taking the files on trust.
  meas_rec  <- build_measurements_catalog(
    con_wdl, datasets_rec,
    measurement_type = cat_registries$measurement_type,
    variable         = meas_var,
    category         = cat_registries$category,
    release_version  = release_version,
    release_date     = as.character(catalog$release_date),
    registries       = list(
      chem   = read_measurement_chem(here("metadata/measurement_chem.csv")),
      method = read_measurement_method(here("metadata/measurement_method.csv")),
      scale  = read_measurement_scale(here("metadata/measurement_scale.csv")),
      why    = read_measurement_why(here("metadata/measurement_why.csv")),
      face   = read_measurement_face(here("metadata/measurement_face.csv"))))
  meas_path <- write_measurements_catalog(meas_rec, dir_frozen)
  validate_measurements_catalog(meas_path)
  d_meas_chk <- check_measurements_catalog(
    meas_rec, con_wdl, datasets_rec, cat_registries$measurement_type)
  assert_measurements_catalog(d_meas_chk)
  d_meas_chk |>
    datatable(caption = "measurements catalog check: every row must be ok (the record is generated from the release, so a failure is a bug, not a data gap)")
  put_gcs_file(meas_path, glue("gs://{gcs_bucket}/{gcs_release}/{basename(meas_path)}"))
  cat(glue("measurements.json: {meas_rec$counts$measurements} measurements over ",
           "{meas_rec$counts$series} series in {meas_rec$counts$datasets} datasets · ",
           "{format(meas_rec$counts$obs_env_rows, big.mark = ',')} obs_env rows · ",
           "{round(file.size(meas_path) / 1e3)} KB\n"))
...
stopifnot("the STAC root the catalog check deferred to must be the one this chunk uploads" =
            identical(as.character(stac_base), release_stac_base(release_prefix, gcs_bucket)))
...
# the catalog URLs the check left pending on this upload must answer now, before anything is
# promoted: a new dataset's collection that the upload did not write is a dead link (a gate)
if (exists("d_catalog_chk") && net_ok) {
  d_pending <- recheck_pending_urls(d_catalog_chk)
  assert_dataset_catalog(d_pending)
  cat(glue("catalog URLs pending on this upload: {nrow(d_pending)} re-probed, ",
           "{sum(d_pending$finding == 'ok')} answer\n"))
}
...
sidecar_urls <- c(
  ...
  glue("gs://{gcs_bucket}/{gcs_release}/taxa.json"),
  glue("gs://{gcs_bucket}/{gcs_release}/measurements.json"),
