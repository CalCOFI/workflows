...
```{r}
#| label: fig-workflow
#| fig-cap: "Pipeline dependency graph, auto-discovered from `_targets.R` (every workflow added to this folder appears here). This notebook — `release_database` — is the highlighted caboose: it runs last, after all ingests, to assemble the frozen release."
#| message: false
#| warning: false
librarian::shelf(targets, visNetwork, quiet = TRUE)
...
# skips the up-to-date check, so there's no "Outdated / Regular target" legend.
invisible(suppressMessages(capture.output(
  net <- tar_network(targets_only = TRUE, outdated = FALSE, callr_function = NULL))))

nodes <- data.frame(
  id    = net$vertices$name,
  label = net$vertices$name,
  group = ifelse(net$vertices$name == "release_database", "release", "workflow"),
  stringsAsFactors = FALSE)
edges <- data.frame(
  from = net$edges$from, to = net$edges$to, stringsAsFactors = FALSE)

visNetwork(nodes, edges, height = "520px", width = "100%") |>
  visGroups(
    groupname = "workflow", shape = "dot", size = 16,
    color = list(background = "#7fb3c8", border = "#5a8ea3",
                 highlight = "#a9d0de")) |>
  visGroups(
    groupname = "release", shape = "dot", size = 34, shadow = TRUE,
    color = list(background = "#ef6c00", border = "#b35100",
                 highlight = "#ff9a3d"),
    font  = list(size = 22, color = "#b35100", face = "bold")) |>
  visNodes(font = list(size = 16)) |>
  visEdges(arrows = "to", color = list(color = "#b0b0b0", highlight = "#666666")) |>
  visHierarchicalLayout(
    direction = "LR", sortMethod = "directed", levelSeparation = 230) |>
