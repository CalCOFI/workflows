...
for (d in derived_dirs) {
  tbl <- basename(d)
  dst <- glue("gs://{gcs_bucket}/{gcs_release}/parquet/{tbl}")
  # rsync, not cp: obs_ctd_full is ~10 GB across ~96 partitions and uploads at a
  # few MiB/s, so this leg takes over an hour — and `cp` restarts it from zero
  # every time. A single transient failure near the end therefore cost the whole
  # upload (observed: 88 of 96 partitions landed, then the run died and a retry
  # would have re-sent all 88). rsync skips what already matches, so a retry costs
