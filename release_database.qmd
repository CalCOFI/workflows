...
reference table keyed by `dataset_id = provider_dataset`, and unified
`v_obs_env` / `v_obs_bio` (+ `v_obs`) VIEWs that project every dataset's
long-format observations into a common shape stamped with `dataset_id`. These
...
# dataset reference: dataset_id = provider_dataset (from the registry)
dbExecute(con_wdl, glue(
  "CREATE OR REPLACE TABLE dataset AS
   SELECT provider || '_' || dataset AS dataset_id, provider, dataset, dataset_name,
...
env_arms <- list(
  bottle = if (has("bottle_measurement","bottle","casts")) "
    SELECT 'calcofi_bottle' AS dataset_id, c.grid_key, c.cruise_key,
...
bio_arms <- list(
  ichthyo = if (has("ichthyo","net","tow","site")) "
    SELECT 'swfsc_ichthyo' AS dataset_id, s.grid_key, s.cruise_key,
...
dbExecute(con_wdl,
  "CREATE OR REPLACE VIEW v_obs AS
   SELECT realm, dataset_id, grid_key, cruise_key, latitude, longitude, datetime,
...
dbGetQuery(con_wdl,
  "SELECT realm, dataset_id, count(*) n_obs, count(DISTINCT grid_key) n_stations
   FROM v_obs GROUP BY 1,2 ORDER BY 1,2") |>
  datatable(caption = "v_obs: unified observation views (dataset_id-stamped)")
```
