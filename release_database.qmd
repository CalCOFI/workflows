...
if (file.exists(meta_json_path)) {
  ...
  rel_https  <- glue("https://storage.googleapis.com/{gcs_bucket}/{release_prefix}")
  prev_versions <- tryCatch(jsonlite::fromJSON(glue("{rel_https}/versions.json"), simplifyVector = FALSE),
                            error = function(e) NULL)
  since <- if (!is.null(prev_versions) && net_ok) dataset_since_versions(prev_versions, base = rel_https) else character()
  ...
  datasets_rec <- build_dataset_catalog(
    ...
    spatial_layers = spatial_layers_path, bathymetry = bathy)
