...
pivot_sql <- function(ck) {
  cols <- unlist(lapply(types, function(tp) c(
    glue("MAX(measurement_value) FILTER (WHERE measurement_type = '{tp}') AS \"{tp}\""),
    glue("MAX(measurement_qual)  FILTER (WHERE measurement_type = '{tp}') AS \"q_{tp}\""))))
  glue("
...
for (tp in c("sigma_theta_ave", "spiciness0"))
  append_obs(con, glue("
    ...
    WHERE o.measurement_type = 'temperature_ave' AND b.{tp} IS NOT NULL"))
