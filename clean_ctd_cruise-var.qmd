...
```{mermaid}
%%| label: fig-flow
%%| fig-cap: "The QA/QC loop. Originals (navy) are loaded once and never edited; automated rules propose rows into the flag ledger (yellow); curators accept or reject; the QC views apply only accepted verdicts, so every product (right) can be regenerated — and accepted flags flow nightly into the public database release."
flowchart LR
  A["calcofi.org cast-file archive<br/>409 db-CSV files, 1993 → present"]
  A -->|"load_pg_ctd.qmd<br/>verbatim, one-time"| B

  subgraph PG["PostgreSQL calcofi — schema ctd"]
    direction TB
    B[("ctd.file / ctd.scan / ctd.scan_issue<br/>ORIGINALS — immutable by trigger<br/>10.8M scans x 82 columns")]
    F[("ctd.flag — the ledger<br/>proposed → accepted / rejected<br/>every change audited")]
    V["ctd.v_scan_qc / ctd.v_scan_clean<br/>originals + ACCEPTED flags only"]
    B --- V
    F -->|"curators accept / reject<br/>pgAdmin or one UPDATE (Sec. 6)"| V
  end

  B -->|"automated QC rules (Sec. 4)<br/>cc_qc_spike / sensor_pair / range"| P["flag candidates<br/>evidence + triage (Sec. 5)"]
  P -->|"cc_propose_flags (Sec. 6)<br/>idempotent"| F
  V -->|"cc_bin_1m (Sec. 7)"| W[("work.clean_1m_*<br/>regenerable products")]
  F -->|"nightly export"| G["gs://calcofi-db/qc/ctd/<br/>flag_accepted.parquet"]
  G -.->|"applied as measurement_qual"| R["public DB release<br/>(ingest_calcofi_ctd-cast)"]
