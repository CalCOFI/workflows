...
d_clean <- d_raw |>
  mutate(
    ...
    # sta_key matching calcofi convention: "090.0 062.0"
    sta_key = Station_ID
  )
...
to casts using `sta_key` + `datetime_utc` (±3 day window).
...
n_dic_sta <- dbGetQuery(
  con, "SELECT COUNT(DISTINCT sta_key) FROM dic_sample")[[1]]
n_matched_sta <- dbGetQuery(
  con,
  "SELECT COUNT(DISTINCT d.sta_key)
   FROM dic_sample d
   INNER JOIN casts c ON d.sta_key = c.sta_key")[[1]]
cat(glue(
...
# match DIC samples to casts by sta_key + date (±3 day window)
...
d_unmatched <- dbGetQuery(
  con,
  "SELECT DISTINCT sta_key, datetime_utc::DATE AS date, ship_name, expocode
...
sql_parts <- map_chr(meas_map, \(m) glue(
  "SELECT expocode, sta_key, datetime_utc, depth_m,
...
dbExecute(con, glue(
  "CREATE TABLE dic_measurement AS
   SELECT ROW_NUMBER() OVER (
     ORDER BY sta_key, datetime_utc, depth_m, measurement_type
...
dbExecute(
  ...
  "CREATE OR REPLACE TABLE dic_measurement_summary AS
   ...
   GROUP BY sta_key, datetime_utc, depth_m,
...
n_dup_sample <- dbGetQuery(
  con,
  "SELECT COUNT(*) FROM (
     SELECT expocode, sta_key, depth_m, datetime_utc, COUNT(*) AS n
     FROM dic_sample
     GROUP BY expocode, sta_key, depth_m, datetime_utc
     HAVING COUNT(*) > 1)")[[1]]
...
n_dup_summ <- dbGetQuery(
  con,
  "SELECT COUNT(*) FROM (
     SELECT sta_key, datetime_utc, depth_m, measurement_type, COUNT(*) AS n
     FROM dic_measurement_summary
...
# define PK/FK relationships for this dataset
add_dic_keys <- function(dm) {
  dm |>
    dm_add_pk(dic_measurement, dic_measurement_id) |>
    dm_add_pk(measurement_type, measurement_type) |>
    dm_add_fk(dic_measurement, measurement_type, measurement_type) |>
    dm_add_fk(dic_measurement_summary, measurement_type, measurement_type)
}

# build dm from DIC-specific tables only (exclude casts/bottle loaded for FK matching)
dic_tables <- c(
  "dic_sample", "dic_measurement", "dic_measurement_summary",
  "measurement_type", "dataset")
dm_dic <- dm_from_con(con, table_names = dic_tables, learn_keys = FALSE) |>
  add_dic_keys() |>
  dm_set_colors(
...
dm_draw(dm_dic, rankdir = "LR", view_type = "all")
```
...
dbGetQuery(con, "SELECT * FROM dic_sample LIMIT 100") |>
  datatable(
    caption = glue(
