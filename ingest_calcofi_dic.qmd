...
# shard is loaded under a distinct name: dic builds its OWN `sample` later in
# emit_core_tables(), and these views must keep resolving after that happens.
load_prior_tables(
  con         = con,
  tables      = c("sample"),
...
compat <- create_compat_views(con, "calcofi_bottle", sample_tbl = "_bottle_sample")
cat(glue("bottle event tables rebuilt from core: {paste(compat, collapse = ', ')}"), "\n")
...
double-counting the bottle.

```{r}
#| label: emit_core

core <- emit_core_tables(con, "calcofi_dic")
cat(glue(
...
stopifnot(
  "obs must be one row per grid-matched DIC measurement" =
    n_obs == dbGetQuery(con, "SELECT COUNT(*) FROM dic_measurement dm JOIN casts c USING (cast_id)
