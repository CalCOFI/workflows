...
contract <- tibble::tribble(
  ...
  "consumer: match/download bio (ichthyo abundance x species x effort)", "nonzero", glue(
    "SELECT count(*) n FROM {rp('obs')} o
       JOIN {rp('species')} sp ON CAST(o.taxon_id AS INTEGER) = sp.species_id
...
if (nzchar(gh_bin)) {
  disp2 <- system2(gh_bin,
    c("workflow", "run", "refresh.yml", "--repo", "CalCOFI/2026-ucsb-station-data-portal"),
    stdout = TRUE, stderr = TRUE)
  if (!identical(attr(disp2, "status") %||% 0L, 0L))
    warning(glue("station-portal refresh dispatch failed: {paste(disp2, collapse = '; ')}"))
  else
    message("dispatched CalCOFI/2026-ucsb-station-data-portal refresh workflow")
}
```
