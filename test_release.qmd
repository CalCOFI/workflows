...
release on GCS, and reported below — followed by the consumer-contract
queries, the client packages' README examples (calcofi4r, calcofi4py) and
the catalog records. If every check passes, `latest.txt` is written;
otherwise the target fails and consumer apps keep pointing at the
...
contract <- tibble::tribble(
  ...
  # no 500 m ceiling since calcofi4db 4.15.0: build_climatology()'s depth_max_m defaults to NULL
  # and the >= n-cruise rule alone stops the table where the data thins (510-720 m on
  # v2026.09.24). The bound is the pipeline's depth ceiling for any coordinate.
  "integrity: climatology depth_bin is a 10 m floor bin within 0-CC_DEPTH_MAX_M", "zero", glue(
    "SELECT count(*) n FROM {rp('climatology')}
     WHERE depth_bin % 10 <> 0 OR depth_bin < 0 OR depth_bin > {calcofi4db::CC_DEPTH_MAX_M}"),
...
# the measured keys travel with the release: integrity.json (calcofi4db >= 4.7.0, written by
# release_database.qmd's relationship_integrity chunk) must exist and report every key ok
integrity_path <- file.path(releases_dir, release_version, "integrity.json")
integ <- if (file.exists(integrity_path)) jsonlite::fromJSON(integrity_path) else NULL
integ_row <- tibble::tibble(
  query  = "sidecar: integrity.json present and every declared key measured ok",
  label  = "consumer-contract",
  status = if (!is.null(integ) && isTRUE(integ$ok)) "pass" else "fail",
  reason = NA_character_,
  rows   = if (is.null(integ)) NA_integer_ else as.integer(integ$n_foreign_keys),
  ms     = NA_real_,
  error  = if (is.null(integ)) "integrity.json missing" else
           if (!isTRUE(integ$ok)) "integrity.json reports a failing key" else NA_character_)
contract_res <- dplyr::bind_rows(contract_res, integ_row)
...
## Package examples — calcofi4r and calcofi4py are consumers too

The two client packages are the first thing a reader copies. On 2026-09-09 a researcher ran the
opening example of the calcofi4py README against v2026.09.06 and it failed on `sample.datetime_utc`
(the column is `datetime`); the calcofi4r README had drifted further — to tables the core
consolidation retired — because its chunks were never executed. The contract queries above pin the
*shapes* consumers depend on; this section runs the packages' own **README examples** against the
just-uploaded release, as a reader would:

- `../calcofi4r/tests/testthat/test-readme.R` knits `README.Rmd` for real (every chunk executes);
- `../calcofi4py/tests/test_readme.py` executes every `python` block of `README.md`.

Both packages resolve `"latest"` through `CALCOFI_RELEASE_VERSION` and the releases prefix through
`CALCOFI_RELEASE_PREFIX` (calcofi4r ≥ 1.24.0, calcofi4py ≥ 0.9.1), so they read **this** release
under **this** prefix — on a staging run, the staging bucket — never the promoted one. One row per
package joins the promote gate. The sibling checkouts are required, as `../db-query` is: the gate
tests the packages as they are on disk, so pull them first. `CALCOFI_PY` names the Python to use
(default: the sibling's `.venv/bin/python`, else `python3`).

```{r}
#| label: package_examples

pkg_env <- c(
  CALCOFI_RELEASE_VERSION = release_version,
  CALCOFI_RELEASE_PREFIX  = release_prefix,
  # the package tests skip their PostgreSQL blocks without this; the gate is the release
  CALCOFI_PG_TEST = "",
  # a bare Rscript has no NOT_CRAN, and testthat's skip_if_offline() then skips "On CRAN" —
  # which this gate counts as a failure (the first dry run, 2026-09-09, failed exactly so)
  NOT_CRAN = "true")
pkg_row <- function(name, ok, ms, err) tibble::tibble(
  query = name, label = "package-examples", status = if (isTRUE(ok)) "pass" else "fail",
  reason = NA_character_, rows = NA_integer_, ms = ms,
  error = if (isTRUE(ok)) NA_character_ else as.character(err))
run_logged <- function(cmd, args, dir) {
  t0  <- Sys.time()
  out <- withr::with_envvar(pkg_env, withr::with_dir(dir,
    system2(cmd, args, stdout = TRUE, stderr = TRUE)))
  list(status = attr(out, "status") %||% 0L, out = out,
       ms = as.numeric(difftime(Sys.time(), t0, units = "secs")) * 1000)
}
tail_txt <- function(x, n = 25) paste(utils::tail(x, n), collapse = "\n")

# calcofi4r: the README knit test, on the sibling checkout (its DESCRIPTION version is what runs)
r_dir <- here("../calcofi4r")
stopifnot("clone CalCOFI/calcofi4r as a sibling of this repo" = dir.exists(r_dir))
r_ver <- read.dcf(file.path(r_dir, "DESCRIPTION"), fields = "Version")[[1]]
r_run <- run_logged(file.path(R.home("bin"), "Rscript"), c("-e", shQuote(
  "res <- testthat::test_file('tests/testthat/test-readme.R', reporter = 'summary', package = 'calcofi4r', load_package = 'source');
   df <- as.data.frame(res); cat('\nreadme: failed', sum(df$failed), 'error', sum(df$error), 'skipped', sum(df$skipped), '\n');
   quit(status = as.integer(sum(df$failed) + sum(df$error) + sum(df$skipped) > 0))")), r_dir)
# a SKIP is a failure here: offline or a missing README.Rmd means the release was not tested
r_ok <- identical(r_run$status, 0L)
if (!r_ok) cat(tail_txt(r_run$out), "\n")

# calcofi4py: every README block, through the sibling's venv
py_dir <- here("../calcofi4py")
stopifnot("clone CalCOFI/calcofi4py as a sibling of this repo" = dir.exists(py_dir))
py_bin <- Sys.getenv("CALCOFI_PY", "")
if (!nzchar(py_bin)) py_bin <- if (file.exists(file.path(py_dir, ".venv/bin/python")))
  file.path(py_dir, ".venv/bin/python") else Sys.which("python3")
py_ver <- trimws(system2(py_bin, c("-c", shQuote("import calcofi4py; print(calcofi4py.__version__)")),
                         stdout = TRUE, stderr = FALSE))
# -rs prints the skip reasons; a skipped release block (offline) fails the gate below
py_run <- run_logged(py_bin, c("-m", "pytest", "-q", "-rs", "tests/test_readme.py"), py_dir)
py_summary <- grep("passed|failed|error", py_run$out, value = TRUE)
py_skipped_release <- any(grepl("SKIPPED .*no network", py_run$out))
py_ok <- identical(py_run$status, 0L) && !py_skipped_release
if (!py_ok) cat(tail_txt(py_run$out), "\n")

pkg_res <- dplyr::bind_rows(
  pkg_row(glue("package: calcofi4r {r_ver} README.Rmd knits against {release_version}"),
          r_ok, r_run$ms, tail_txt(r_run$out, 12)),
  pkg_row(glue("package: calcofi4py {py_ver} README.md blocks run against {release_version} [{paste(py_summary, collapse = '; ')}]"),
          py_ok, py_run$ms, tail_txt(py_run$out, 12)))
results <- dplyr::bind_rows(results, pkg_res)
pkg_res |>
  mutate(status_html = status_badge(status)) |>
  select(status_html, query, ms, error) |>
  DT::datatable(escape = FALSE, rownames = FALSE, colnames = c("", "package example run", "ms", "error"))
```

## Dataset catalog record

`datasets.json` (calcofi4db >= 4.1.0; plan 2026-09-05 § D-1) is the one dataset list every product
reads — the landing page's dataset grid and pages, `cc_datasets()`, the sitemap, `data.json`, STAC.
Four rows join the promote gate: the file validates against `datasets.schema.json`, it declares the
**schema version this release was built with** (`1.1` from calcofi4db 4.5.0 — a record that says
`1.0` means the release ran an older builder than the checks assume), it carries every dataset
`metadata.json` carries, and `check_dataset_catalog()` reports no blocking finding (a dead endpoint
on a page is worse than absence; the network half follows `CALCOFI_SKIP_LINK_CHECK`).

Schema 1.1's three new findings are reported here with the rest and only one of them blocks:
`grain_without_description` is an **error** — the builder should never emit an ERDDAP grain nobody
has described, and a new grain means a new sentence in `erddap_grain_description()` — while
`registration_without_id` and `bbox_implausible` are warnings a human reads. `bbox_implausible` is
expected to fire for `swfsc_ichthyo` (its asserted extent reads 0–54° N × 180–77° W from bad upstream
coordinates while its sampled positions sit in the California Current; question Q16 to SWFSC), so its
count is printed rather than gated.

```{r}
#| label: dataset_catalog
...
`taxa.json` (calcofi4db >= 4.9.0; plan 2026-09-09 § D6/D7) is what calcofi.io/species/ generates
every species page from — one entry per taxon of this release's `taxon` table with an observation at
or below it, its lineage and rollups, and the name each dataset uses for it. Six rows join the
promote gate: the file validates against `taxa.schema.json`, it declares the schema version this
release was built with (`1.0`), its `counts.taxa_observed` equals the distinct `taxon_key` of **this
release's own `obs_bio`**, every `parent_taxon_key` resolves inside `taxa[]` (a page with a dangling
breadcrumb is a 404 on the site), the roots' `rollup.n_obs` sums to `obs_bio`'s keyed row count, and
every `dataset_key` it names is in `datasets.json`.

The record is **generated**, so unlike the dataset catalog there is nothing here a provider question
can excuse: a failing row is a bug in `build_taxa_catalog()` or a break in the release's own
`taxon` / `obs_bio`, and it stops the promotion. `taxon` and `obs_bio` are read back through this
release's catalog, exactly as a consumer reads them, not from the connection that built the record.

```{r}
#| label: taxa_catalog

taxa_url <- glue("https://storage.googleapis.com/calcofi-db/{rel_prefix}/{release_version}/taxa.json")
taxa_txt <- tryCatch(paste(readLines(taxa_url, warn = FALSE, encoding = "UTF-8"), collapse = "\n"),
                     error = function(e) NULL)
tx_row <- function(name, ok, err) tibble::tibble(
  query = name, label = "taxa-catalog", status = if (isTRUE(ok)) "pass" else "fail",
  reason = NA_character_, rows = NA_integer_, ms = NA_real_,
  error = if (isTRUE(ok)) NA_character_ else as.character(err))

taxa_res <- if (is.null(taxa_txt)) {
  tx_row("taxa: taxa.json present", FALSE, glue("{taxa_url} not readable"))
} else {
  taxa_rec  <- jsonlite::fromJSON(taxa_txt, simplifyVector = FALSE)
  schema_ok <- tryCatch(validate_taxa_catalog(taxa_txt), error = function(e) conditionMessage(e))
  # the release's own tables, resolved through its catalog like every consumer
  dbExecute(con_test, glue("CREATE OR REPLACE VIEW taxon   AS SELECT * FROM {rp('taxon')}"))
  dbExecute(con_test, glue("CREATE OR REPLACE VIEW obs_bio AS SELECT * FROM {rp('obs_bio')}"))
  tx_chk <- check_taxa_catalog(taxa_rec, con_test, if (exists("ds_rec")) ds_rec else NULL)
  chk <- function(nm) {
    r <- tx_chk[tx_chk$check == nm, , drop = FALSE]
    if (nrow(r)) as.list(r[1, ]) else
      list(ok = FALSE, expected = NA_character_, observed = NA_character_, detail = "check not reported")
  }
  a <- chk("taxa_observed"); pr <- chk("parents_resolve")
  rt <- chk("rollup_total");  dk <- chk("datasets_known")
  n_bad <- sum(!tx_chk$ok)
  dplyr::bind_rows(
    tx_row("taxa: taxa.json validates against taxa.schema.json", isTRUE(schema_ok), schema_ok),
    # the version the package that BUILT the record declares, never a literal: a schema bump
    # (1.1 added n_present, 2026-09-11) otherwise fails the release on a test that was not updated
    tx_row(glue("taxa: schema_version is {calcofi4db:::CC_TAXA_SCHEMA_VERSION} ",
                "[{taxa_rec$schema_version %||% 'absent'}]"),
           identical(taxa_rec$schema_version, calcofi4db:::CC_TAXA_SCHEMA_VERSION),
           "taxa.json was built by a different calcofi4db than the one this test loads"),
    tx_row(glue("taxa: counts.taxa_observed == distinct obs_bio.taxon_key ",
                "[{a$observed} vs {a$expected}]"), isTRUE(a$ok), a$detail),
    tx_row(glue("taxa: every parent_taxon_key resolves within taxa[] ",
                "[{taxa_rec$counts$pages} pages]"), isTRUE(pr$ok), pr$detail),
    tx_row(glue("taxa: the roots' rollup.n_obs sums to obs_bio's keyed rows [{rt$observed}]"),
           isTRUE(rt$ok), rt$detail),
    tx_row(glue("taxa: every dataset_key is in datasets.json [{taxa_rec$counts$datasets} datasets]"),
           isTRUE(dk$ok), dk$detail),
    tx_row(glue("taxa: check_taxa_catalog: {nrow(tx_chk) - n_bad}/{nrow(tx_chk)} checks pass"),
           n_bad == 0, paste(tx_chk$check[!tx_chk$ok], collapse = "; ")))
}
results <- dplyr::bind_rows(results, taxa_res)
taxa_res |>
  mutate(status_html = status_badge(status)) |>
  select(status_html, query, error) |>
  DT::datatable(escape = FALSE, rownames = FALSE, colnames = c("", "species catalog check", "error"))
```

## Measurements catalog record

`measurements.json` (calcofi4db >= 4.12.0; plan 2026-09-10 § D2/D4) is `taxa.json`'s twin for the
environment half of the catalog, and what calcofi.io/measurements/ generates every measurement page
from — one entry per measurement **key** this release's `obs_env` carries (the registry's `variable`
where one is set, else the `measurement_type`), with one `series[]` per `measurement_type` x dataset.
Six rows join the promote gate: the file validates against `measurements.schema.json`, it declares
the schema version this release was built with (`1.0`), its `counts.obs_env_rows` equals **this
release's own `obs_env`** row count, the sum of `series[].n_values` over every key equals that same
number (a series counted twice — the risk a wrong `variable` merge carries — is exactly what this
catches), every `series[].measurement_type` is in `metadata/measurement_type.csv`, and every
`dataset_key` it names is in `datasets.json`.

The record is **generated**: a failing row is a bug in `build_measurements_catalog()` or a break in
`obs_env`, never something a provider question can excuse, and it stops the promotion. `obs_env` is
read back through this release's catalog, exactly as a consumer reads it, not from the connection
that built the record.

```{r}
#| label: measurements_catalog

meas_url <- glue("https://storage.googleapis.com/calcofi-db/{rel_prefix}/{release_version}/measurements.json")
meas_txt <- tryCatch(paste(readLines(meas_url, warn = FALSE, encoding = "UTF-8"), collapse = "\n"),
                     error = function(e) NULL)
mm_row <- function(name, ok, err) tibble::tibble(
  query = name, label = "measurements-catalog", status = if (isTRUE(ok)) "pass" else "fail",
  reason = NA_character_, rows = NA_integer_, ms = NA_real_,
  error = if (isTRUE(ok)) NA_character_ else as.character(err))

meas_res <- if (is.null(meas_txt)) {
  mm_row("measurements: measurements.json present", FALSE, glue("{meas_url} not readable"))
} else {
  meas_rec  <- jsonlite::fromJSON(meas_txt, simplifyVector = FALSE)
  schema_ok <- tryCatch(validate_measurements_catalog(meas_txt), error = function(e) conditionMessage(e))
  dbExecute(con_test, glue("CREATE OR REPLACE VIEW obs_env AS SELECT * FROM {rp('obs_env')}"))
  mm_reg <- read_measurement_type(here("metadata/measurement_type.csv"))
  mm_chk <- check_measurements_catalog(
    meas_rec, con_test, if (exists("ds_rec")) ds_rec else NULL, mm_reg)
  chk <- function(nm) {
    r <- mm_chk[mm_chk$check == nm, , drop = FALSE]
    if (nrow(r)) as.list(r[1, ]) else
      list(ok = FALSE, expected = NA_character_, observed = NA_character_, detail = "check not reported")
  }
  ro <- chk("obs_env_rows"); st <- chk("series_total")
  tk <- chk("types_known");  dk <- chk("datasets_known")
  n_bad <- sum(!mm_chk$ok)
  # the mocked page's own arithmetic: the temperature entry's totals are its two series
  tmp <- Filter(function(m) identical(m$key, "temperature"), meas_rec$measurements)
  tmp_ok <- length(tmp) == 1 &&
    isTRUE(as.numeric(tmp[[1]]$totals$n_values) ==
             sum(vapply(tmp[[1]]$series, function(s) as.numeric(s$n_values), 0)))
  dplyr::bind_rows(
    mm_row("measurements: measurements.json validates against measurements.schema.json",
           isTRUE(schema_ok), schema_ok),
    mm_row(glue("measurements: schema_version is {calcofi4db:::CC_MEASUREMENTS_SCHEMA_VERSION} ",
                "[{meas_rec$schema_version %||% 'absent'}]"),
           identical(meas_rec$schema_version, calcofi4db:::CC_MEASUREMENTS_SCHEMA_VERSION),
           "measurements.json was built by a different calcofi4db than the one this test loads"),
    mm_row(glue("measurements: counts.obs_env_rows == obs_env rows [{ro$observed} vs {ro$expected}]"),
           isTRUE(ro$ok), ro$detail),
    mm_row(glue("measurements: sum of series[].n_values == obs_env rows over ",
                "{meas_rec$counts$measurements} keys / {meas_rec$counts$series} series [{st$observed}]"),
           isTRUE(st$ok), st$detail),
    mm_row(glue("measurements: every series[].measurement_type is in measurement_type.csv ",
                "[{meas_rec$counts$series} series]"), isTRUE(tk$ok), tk$detail),
    mm_row(glue("measurements: every dataset_key is in datasets.json ",
                "[{meas_rec$counts$datasets} datasets]"), isTRUE(dk$ok), dk$detail),
    mm_row(glue("measurements: the temperature entry's totals.n_values is its series' sum ",
                "[{if (length(tmp) == 1) tmp[[1]]$totals$n_values else 'no temperature key'}]"),
           tmp_ok, "totals.n_values != sum(series[].n_values) for `temperature`"),
    mm_row(glue("measurements: check_measurements_catalog: {nrow(mm_chk) - n_bad}/{nrow(mm_chk)} checks pass"),
           n_bad == 0, paste(mm_chk$check[!mm_chk$ok], collapse = "; ")))
}
results <- dplyr::bind_rows(results, meas_res)
meas_res |>
  mutate(status_html = status_badge(status)) |>
  select(status_html, query, error) |>
  DT::datatable(escape = FALSE, rownames = FALSE, colnames = c("", "measurements catalog check", "error"))
```

## Static STAC catalog

The STAC catalog `release_database.qmd` wrote to `gs://calcofi-db/stac/` (plan 2026-09-05 § D-5.3;
calcofi4db >= 4.3.0) is checked **as published**, not as built: the root, every child collection and
this release's item are fetched back from the bucket into a local mirror and run through
`check_stac()` — the structural half always, `stac-validator` (pip) over every document when it is
installed. That is what `stac-browser` at calcofi.io/stac/ will read.
...
# a staging run must not touch the public index or the hosted consumers: both read the REAL
# latest.txt, so a staging pass would rebuild every consumer against the unchanged promoted
# release (measured 2026-09-10 while preparing v2026.09.10: nothing here was staging-aware).
idx_script <- here::here("scripts", "build_release_index.R")
if (staging) {
  message("STAGING run: release index and consumer dispatches skipped")
} else if (file.exists(idx_script)) {
...
gh_dispatch <- tribble(
  ...
  # the client packages: re-run their README/vignette examples against the promoted latest and
  # republish their sites (pkgdown executes the vignettes; the pytest job executes the README)
  "CalCOFI/calcofi4r",        "pkgdown.yaml",                "calcofi4r site + vignettes against latest",
  "CalCOFI/calcofi4py",       "test.yml",                    "calcofi4py README examples against latest")

gh_bin <- Sys.which("gh")
if (staging) {
  # skipped above: nothing to nudge until a real promotion
} else if (!nzchar(gh_bin)) {
  message(glue(
