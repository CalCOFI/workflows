...
# so the staging prefix resolves the same way.
...
tbl_sql <- function(version, table, col = NULL, val = NULL) {
  ...
  src <- calcofi4r::cc_release_sources(.tbl_cats[[version]], table)
  if (!is.null(col)) {
...
contract <- tibble::tribble(
  ...
  # assert the second.
  "consumer: seabird/mammal taxa reachable by worms_id (db-viz-hex get_sp)", "nonzero", glue(
    "SELECT count(*) n FROM {rp('obs')} o
       JOIN {rp('taxon')} t ON t.taxon_key = o.taxon_key
  ...
  "integrity: no farallon obs left unreachable by worms_id", "zero", glue(
    "SELECT count(*) n FROM {rp('obs')} o
...
tryCatch(
  publish_release_notes(
    release_version, here::here("RELEASES.md"), here::here("data/releases"),
    bucket = gcs_bucket,
    pkg_versions = c(
