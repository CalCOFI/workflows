...
calcofi:
  ...
  dataset_meta:
    ...
    link_calcofi_org: https://calcofi.org/data/biology/ichthyoplankton/
    coverage_temporal: 1951 to present
...
core <- emit_core_tables(
  con, "swfsc_ichthyo", measurement_taxon = mt_taxon, overrides = tx_over)
cat(glue(
...
stopifnot(
  ...
  "obs must carry only the abundance headline" =
    ...
    dbGetQuery(con, "SELECT COUNT(*) FROM ichthyo i JOIN net n USING (net_uuid)
                     ...
                     WHERE i.measurement_type IS NULL AND s.grid_key IS NOT NULL")[[1]],
...
cat(glue("sample: {format(n_site, big.mark=',')} sites / {format(n_tow, big.mark=',')} tows / ",
         "{format(n_net, big.mark=',')} nets"), "\n")

# serve the retired per-dataset names as VIEWs over the core (exact for every
# column the core models; see ?create_compat_views)
compat <- create_compat_views(con, "swfsc_ichthyo")
