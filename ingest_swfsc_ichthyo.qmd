...
# define paths
dataset_name <- "NOAA CalCOFI Ichthyoplankton Database"
provider <- "swfsc"
dataset <- "ichthyo"
...
segment <- tbl(con, sql(
  "SELECT cruise_key, orderocc, site_uuid, longitude AS lon, latitude AS lat
   FROM site")) |>
  left_join(
    tbl(con, "tow") |>
...
if (!file_exists(map_segment_png)) {
  ...
  seg_cols <- dbGetQuery(con,
    "SELECT column_name FROM information_schema.columns
     WHERE table_name = 'segment' AND data_type != 'GEOMETRY'")$column_name
  seg_sql  <- paste(
    "SELECT", paste(seg_cols, collapse = ", "),
...
# exclude tables with GEOMETRY columns (unsupported by dm_from_con via duckdb R driver)
all_tbls <- DBI::dbListTables(con)
geom_tbls <- DBI::dbGetQuery(con,
  "SELECT DISTINCT table_name FROM information_schema.columns
   WHERE data_type = 'GEOMETRY'")$table_name
dm_tbls <- setdiff(all_tbls, geom_tbls)

tryCatch({
  dm_final <- dm_from_con(con, table_names = dm_tbls, learn_keys = FALSE) |>
    add_cc_keys()
  dm_draw(dm_final, view_type = "all")
}, error = function(e) {
  message(glue("dm diagram skipped: {e$message}"))
  # list tables with row counts instead
  tbl_stats <- purrr::map_dfr(all_tbls, function(t) {
    n <- DBI::dbGetQuery(con, glue("SELECT COUNT(*) AS n FROM {t}"))$n
    tibble(table = t, rows = n, has_geom = t %in% geom_tbls)
  })
  tbl_stats |> datatable(caption = "Tables in wrangling database")
})
```
...
metadata_path <- build_metadata_json(
  ...
  dataset = dataset,
  workflow_url = glue(
    "https://calcofi.io/workflows/ingest_swfsc_ichthyo.html"
  )
)

# write relationships.json sidecar with PKs/FKs
# note: tables with GEOMETRY columns are excluded from dm_from_con
#   (duckdb R driver limitation); add_cc_keys may fail if referenced tables
#   are excluded — wrap in tryCatch to gracefully handle
geom_tbls_meta <- DBI::dbGetQuery(con,
  "SELECT DISTINCT table_name FROM information_schema.columns
   WHERE data_type = 'GEOMETRY'")$table_name
dm_tbls_meta <- setdiff(dbListTables(con), geom_tbls_meta)

tryCatch({
  dm_all <- dm_from_con(
    con,
    table_names = dm_tbls_meta,
    learn_keys = FALSE
  ) |>
    add_cc_keys()
...
## Re-export Parquet with Taxonomy Tables

Update parquet outputs to include the new taxon and taxa_rank tables, and
the updated species table (now with worms_id, itis_id, gbif_id).

```{r}
#| label: rewrite_parquet

# rewrite parquet for modified/new tables
modified_tables <- c("species", "taxon", "taxa_rank")
for (tbl in modified_tables) {
  pqt_path <- file.path(dir_parquet, paste0(tbl, ".parquet"))
  cols <- DBI::dbGetQuery(
    con,
    glue(
      "SELECT column_name FROM information_schema.columns
  ...
  DBI::dbExecute(
    con,
    glue(
      "COPY (SELECT {paste(export_cols, collapse = ', ')} FROM {tbl})
     TO '{pqt_path}' (FORMAT PARQUET, COMPRESSION 'snappy')"
    )
  )
  message(glue("Re-exported {tbl}.parquet"))
}
...
if (file.exists(manifest_path)) {
  ...
  new_tables <- unique(c(manifest$tables, modified_tables))
  ...
  message(glue("Updated manifest: {length(new_tables)} tables"))
}
```
