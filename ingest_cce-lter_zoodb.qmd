...
Collection). Per-taxon CSVs were exported from the portal's `save.php` endpoint
(public access) and consolidated to `zoodb_holoplankton.csv`
(see `by_taxon/_PROVENANCE.md`).
...
The consolidated long table (33 per-taxon × 2-unit portal exports merged on the
shared tow key) lives at `{dir_data}/cce-lter/ZooDB/zoodb_holoplankton.csv`.
Pooled rows carry `N/A` for the per-station fields, read as `NA`.

```{r}
#| label: read-source

zoo_dir <- path_expand(glue("{dir_data}/cce-lter/ZooDB"))
zoo_csv <- path(zoo_dir, "zoodb_holoplankton.csv")
