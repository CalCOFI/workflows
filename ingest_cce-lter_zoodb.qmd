...
calcofi:
  ...
  dataset_meta:
    ...
    dataset_name_short: ZooDB (Holoplankton Community)
    ...
    # no formal author/year/title citation is stated anywhere on the portal —
    # left empty; see metadata/cce-lter/zoodb/questions.csv Q10 (proposed to
    # Ohman/Sala). checked 2026-09-03
    citation_main: ""
    ...
    link_others: []
    # NOT CC-BY-4.0: the portal's "Data Use Policy" panel (JS-toggled content on
    # the same page, no separate anchor URL) is bespoke, non-CC text: "intended
    # for scholarly use... Use or reproduction of any materials herein for any
    # commercial purpose is prohibited without prior written permission from
    # Dr. Mark D. Ohman."
    # source: https://oceaninformatics.ucsd.edu/zoodb/ (Data Use Policy panel), checked 2026-09-03
    license: custom
    license_url: https://oceaninformatics.ucsd.edu/zoodb/
    # moved from citation_others (2026-09-03): this is credit prose the portal's
    # "Data Acknowledgement Policy" requires be included in any use, not an
    # additional citation.
    # source: https://oceaninformatics.ucsd.edu/zoodb/ (Data Acknowledgement Policy panel), checked 2026-09-03
  ...
  tables_owned:
    ...
    - {table: dataset_taxon, shared: true, note: "zoodb taxon_id -> taxon_key crosswalk"}
...
# ship path: the reference cruise whose date span holds the sample, else its
# month, keys not in `cruise` left NULL (calcofi4db::resolve_cruise_key) — the
# old month rule put a sample in the first days of a month on the NEXT cruise
dbWriteTable(con, "zk", d_sample |> select(sample_key, ship_key, datetime = date), overwrite = TRUE)
resolve_cruise_key(con, "zk", datetime_col = "datetime", require_in_cruise = TRUE) |>
  datatable(caption = "cruise_key resolution method (ship path)")
d_sample <- d_sample |>
  left_join(dbGetQuery(con, "SELECT sample_key, cruise_key AS cruise_key_ship FROM zk"),
            by = "sample_key") |>
...
(`design_env-bio-consolidation.md`, `RUNBOOK.md` §3b). These core tables **are**
this ingest's output: `release_database.qmd` concatenates the per-dataset shards
rather than re-deriving the core, so there is exactly one projection to keep
correct.

**This projection lives here, in the notebook that owns the dataset**, rather than
in a `switch(dataset_key, …)` arm inside `calcofi4db`. The reusable *shapes* stay in
the package — `sample_arm_self()` for a one-event-table dataset,
`compat_measurement_sql()` for the compat VIEW — so declaring a dataset's projection
is a handful of parameters, not copied SQL.
...
# per local taxon_id with `taxon_key` empty and the AphiaID the SOURCE supplied
# carried along as `ds_source_json`; resolve_dataset_taxon() fills the key from
# the authorities. There is no zoodb arm inside calcofi4db any more -- a column
# renamed or dropped here now fails at this call rather than becoming NA at
# release. The source's denormalized lineage columns are NOT staged: the
# classification the release publishes comes from ensure_taxon_lineage(), the
# authority, not from the export's copy of it.
d_vocab <- dbGetQuery(con, "
  SELECT CAST(taxon_id AS VARCHAR) AS ds_taxa_code,
         scientific_name           AS ds_scientific_name,
         taxon_zoodb AS ds_common_name,
         CAST(aphia_id AS INTEGER) AS worms_id,
         rank
  FROM zoodb_taxon ORDER BY taxon_id")
...
# the ingest asserts its own crosswalk (D6): every taxon_id the measurements use
# is in the vocabulary and every staged row keys an authority id. Halts on any
# finding.
obs_codes <- dbGetQuery(con,
  "SELECT DISTINCT CAST(taxon_id AS VARCHAR) AS c FROM zoodb_measurement
   WHERE taxon_id IS NOT NULL")$c
chk_taxon <- check_dataset_taxon(con, ds_key, codes = obs_codes)
cat(glue("vocabulary: {n_staged} taxa staged; taxon {n_taxon}, ",
