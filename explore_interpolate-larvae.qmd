...
Borrowing from rules described by Andrew Thompson: CalCOFI Analysis for CINMS/MBNMS infographics ([calcofi.io/larvae-cinms](https://calcofi.io/larvae-cinms)).
...
librarian::shelf(
  calcofi/calcofi4r,
  dplyr, DT, dygraphs, glue, here, mapview, purrr, sf, tidyr,
  quiet = T)
...
d <- tbl(con, "net2cruise") |> # colnames() |> paste(collapse=", ") 
  ...
  filter(
    ...
    # DEBUG
    cruise_ymd >= "20010101",
    cruise_ymd <= "20100101") |>  # select only starboard samples from the bongo nets
  # TODO: filter by `linestation_core`?
    left_join(
...
# d_g |>
#   mutate(larvaecount_nonzero = larvaecount != 0) |>
#   pull(larvaecount_nonzero) |> 
#   table()
# larvaecount_nonzero:
#    FALSE   TRUE 
#   20,322  7,399
...
# d_g |> 
#   mutate(
#     ym = as.integer(format(starttime, "%Y%m"))) |>
#   group_by(ym) |> 
#   summarize(
#     larvaecount = sum(larvaecount)) |> 
#   arrange(desc(larvaecount))
# # A tibble: 319 × 2
#        ym larvaecount
#     <int>       <int>
#  1 198104       40321
...
```{r}
d_m <- d_g |> # 99 × 7 <- 27,721 × 7
  filter(
    # format(starttime, "%Y-%m") == "2022-04")
    format(starttime, "%Y-%m") == "2005-04")
# table(d_m$larvaecount != 0)
# FALSE  TRUE 
#    88   106
# hist(log(d_m$larvaecount))
# hist(log10(d_m$larvaecount))

p_m <- d_m |> 
  select(-data) |>
  mutate(
    # larvaecount_log   = log(larvaecount + 0.1),
    larvaecount_log10 = log10(larvaecount + 0.1)) |>
    # larvaecount_log   = log(larvaecount),
    # larvaecount_log10 = log10(larvaecount)) |>
  sf::st_as_sf(
    coords = c("longitude", "latitude"), 
    crs    = 4326,
    remove = F)
# table(p_m$larvaecount)
# table(p_m$larvaecount_log)
# table(p_m$larvaecount_log10)

mapView(
  p_m, 
  zcol        = "larvaecount_log10", 
...
The [WMTS `GetCapabilities`](https://wmts.marine.copernicus.eu/teroWmts/GLOBAL_MULTIYEAR_PHY_001_030?request=GetCapabilities) gets parsed to collect available variables, depths (`elevation`) and dates (`time`) for the given `product_id/dataset_id/variable` (`r glue("{product_id}/{dataset_id}")`).
...
variable   = "thetao"
# date       = as.Date("2022-04-01") |> as.POSIXct()
...
productId = "cmems_mod_nws_phy_anfc_0.027deg-3D_P1D-m"
...
cmd <- glue("{cm} --help subset") 
cmd <- glue("{cm} -h subset") 
cmd <- glue("{cm} --version") 
# copernicusmarine, version 1.1.1
...
# dataset_part    <- "timeseries" # or "geoseries"
...
r_idw <- terra::interpIDW(
  ...
  mask(r_cm)
names(r_idw)     <- "larvaecount_log10_idw"
...
p_mi <- r_idw |> 
  terra::extract(p_m, method = "simple", bind = T) |> 
  st_as_sf() |> 
  mutate(
    dif_idw = larvaecount_log10 - larvaecount_log10_idw)

# p_mi$dif_idw |> summary()
#     Min.  1st Qu.   Median     Mean  3rd Qu.     Max.     NA's 
# -2.24324 -0.28024  0.00000  0.01276  0.38674  1.76034        6 

d_mi <- p_mi |> 
  st_drop_geometry() |>
  filter(!is.na(larvaecount_log10_idw))

rmse_idw <- with(
  d_mi,
  sqrt( 
    sum((larvaecount_log10_idw - larvaecount_log10)^2) /
      nrow(d_mi) ) )
# [1] 0.7824212

mapView(
  r_idw, 
  zcol        = "larvaecount_log10_idw", 
  col.regions = rev(RColorBrewer::brewer.pal(11,"Spectral"))) + 
mapView(
  p_mi, 
  zcol        = "larvaecount_log10", 
...
Root Mean Square Error: `r mse_idw`
...
# fit GAM
mdl_xy <- mgcv::gam(
  larvaecount_log10 ~ s(longitude, latitude, k = 60),
  data   = p_mi |> st_drop_geometry() |> 
    select(larvaecount_log10, longitude, latitude), 
...
# range(p_idw$larvaecount_log10_idw)
# [1] -1.000000  2.413467

# predict with GAM
p_idw$larvaecount_log10_xy <- predict.gam(
  mdl_xy, 
  newdata = p_idw |> 
    st_drop_geometry() |> 
    select(longitude, latitude), 
  type = "response") # link") # "response")
range(p_idw$larvaecount_log10_xy)
...
r_xy <- rasterize(
  p_idw |> 
    ...
    as.matrix(),
  r_idw, 
  values = p_idw$larvaecount_log10_xy)
range(values(r_xy, na.rm=T))
# plot(r_xy)
names(r_xy)     <- "larvaecount_log10_xy"

p_mix <- r_xy |> 
  terra::extract(p_mi, method = "simple", bind = T) |> 
  st_as_sf() |> 
  mutate(
    dif_xy = larvaecount_log10 - larvaecount_log10_xy)

# p_mix$dif_xy |> summary()
#      Min.   1st Qu.    Median      Mean   3rd Qu.      Max.      NA's 
...
rmse_xy <- with(
  d_mix,
  sqrt( 
    sum((larvaecount_log10_xy - larvaecount_log10)^2) /
      nrow(d_mix) ) )
# [1] 0.8445961

mapView(
  r_xy, 
  zcol        = "larvaecount_log10_xy", 
  col.regions = rev(RColorBrewer::brewer.pal(11,"Spectral"))) + 
mapView(
  p_mix, 
  zcol        = "larvaecount_log10", 
...
p_mixt <- r_cm |> 
  terra::extract(p_mix, method = "simple", bind = T) |> 
  st_as_sf()

# fit GAM
mdl_xyt <- mgcv::gam(
  larvaecount_log10 ~ s(longitude, latitude, k = 60) + s(thetao),
  data   = p_mixt |> st_drop_geometry() |> 
    select(larvaecount_log10, longitude, latitude, thetao), 
  method = "GCV.Cp", # method = "REML",
  family = gaussian())
summary(mdl_xyt)
# Family: gaussian 
...
# predict with GAM
p_cm$larvaecount_log10_xyt <- predict.gam(
  mdl_xyt, 
  newdata = p_cm |> 
    st_drop_geometry() |> 
    select(longitude, latitude, thetao), 
  type = "response") # link") # "response")
range(p_cm$larvaecount_log10_xyt)
...
r_xyt <- rasterize(
  p_cm |> 
    ...
    as.matrix(),
  r_idw, 
  values = p_cm$larvaecount_log10_xyt)
range(values(r_xyt, na.rm=T))
# plot(r_xy)
names(r_xyt)     <- "larvaecount_log10_xyt"

p_mixt <- r_xyt |> 
  terra::extract(p_mixt, method = "simple", bind = T) |> 
  st_as_sf() |> 
  mutate(
    dif_xyt = larvaecount_log10 - larvaecount_log10_xyt)



p_mixt$dif_xyt |> summary()
#      Min.   1st Qu.    Median      Mean   3rd Qu.      Max.      NA's 
# -2.403050 -0.347458  0.003876  0.001680  0.394274  2.457923         6 
...
rmse_xyt <- with(
  d_mixt,
  sqrt( 
    sum((larvaecount_log10_xyt - larvaecount_log10)^2) /
      nrow(d_mixt) ) )
# rmse_xyt
# [1] 0.8054934
