...
calcofi:
  ...
  dataset_meta:
    ...
    # source: https://cite.edirepository.org/cite/knb-lter-cce.313.1?style=ESIP, checked 2026-09-03
    # (revision 1 is the newest: DataONE has no object at .../313/2, /3, /4)
    citation_main: "Ohman, M.D. 2022. California Current Ecosystem Euphausiid data, Brinton and Townsend Euphausiid Database (BTEDB) ver 1. Environmental Data Initiative. https://doi.org/10.6073/pasta/4a92a0044bcd1523a4f994ece874a57d"
    # source: https://cite.edirepository.org/cite/knb-lter-cce.313.1?style=ESIP, checked 2026-09-03
    ...
    link_others: []
    # NOT CC-BY-4.0: the EML <intellectualRights> (DataONE mirror, since the EDI
    # portal blocks automated fetches with a Cloudflare bot check) is bespoke,
    # non-CC text requiring the acknowledgement below and deposit of reprints
    # with the Pelagic Invertebrate Collection and the CCE-LTER program office.
    # source: https://cn.dataone.org/cn/v2/object/https%3A%2F%2Fpasta.lternet.edu%2Fpackage%2Fmetadata%2Feml%2Fknb-lter-cce%2F313%2F1, checked 2026-09-03
    license: custom
    license_url: https://portal.edirepository.org/nis/metadataviewer?packageid=knb-lter-cce.313.1
    # source: EML <intellectualRights> (same object as license, above), checked 2026-09-03 —
    # the required acknowledgement text verbatim
...
`euphausiids_taxon`. *Emit Core Tables* then stages that column with
`calcofi4db::append_dataset_taxon()` and lets `resolve_dataset_taxon()` fill
`taxon_key`, which is what lets `obs.taxon_key` be populated for euphausiids.
...
# keys not in `cruise` left NULL (calcofi4db::resolve_cruise_key) — the old
# month rule put a tow in the first days of a month on the NEXT cruise
dbWriteTable(con, "zk", d_keys |> select(tow_id, ship_key, datetime = date), overwrite = TRUE)
resolve_cruise_key(con, "zk", datetime_col = "datetime", require_in_cruise = TRUE) |>
  datatable(caption = "cruise_key resolution method")
d_keys <- d_keys |>
  left_join(dbGetQuery(con, "SELECT tow_id, cruise_key FROM zk"), by = "tow_id")
...
# per BTEDB species with `taxon_key` empty and the AphiaID standardize_species()
# resolved above carried along as `ds_source_json`; resolve_dataset_taxon() fills
# the key from the authorities. There is no euphausiids arm inside calcofi4db any
# more -- a column renamed or dropped here now fails at this call rather than
# becoming NA at release.
d_vocab <- dbGetQuery(con, "
  ...
  FROM euphausiids_taxon ORDER BY taxon_id")
...
obs_codes <- dbGetQuery(con,
  "SELECT DISTINCT CAST(taxon_id AS VARCHAR) AS c FROM euphausiids_measurement
