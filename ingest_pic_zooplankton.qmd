...
# this source carries tow effort/position only — no measurements — so it
# contributes `sample` and nothing else. Assert that, rather than let a silent
# zero-row obs look like a projection bug.
stopifnot(
  "pic_zooplankton contributes sample rows" = (core$sample %||% 0) > 0,
  "pic_zooplankton has no measurements to project" = is.null(core$obs))
```
...
metadata_path <- build_metadata_json(
  ...
  metadata_derived_csv = c(here("metadata/core_dictionary.csv"),
                           here("metadata/pic/zooplankton/metadata_derived.csv")),
