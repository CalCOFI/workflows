...
infer_segments <- function(d) {
  ...
  tibble(
    lat_start = lat_start %>% unlist(),
    lon_start = lon_start %>% unlist(),
    lat_stop  = lat_stop %>% unlist(),
    lon_stop  = lon_stop %>% unlist(),
    effort    = FALSE)
}
...
update_lines <- function(d_cruise_date){ # data split by CRUISE & DATE
  ...
  d_cruise_date %>% 
    infer_segments() %>% 
    ungroup() %>% 
    bind_rows(d_cruise_date %>% select(lat_start, lon_start, lat_stop, lon_stop, effort))
}
...
get_linestring <- function(d){
  ...
  st_sfc(lines_sf, crs = "+proj=longlat +datum=WGS84") %>% 
    st_as_sf() %>% mutate(effort = d$effort)
}
```
...
```{r}
# messy with all segments. better approach would be filtering beforehand or retaining attribute information for use in filtering afterward
all_segments <- map(d_list, update_lines) %>% 
  bind_rows() %>% 
  get_linestring() %>% 
  st_as_sf() 
mapview(all_segments, add=T, zcol="effort")
```
