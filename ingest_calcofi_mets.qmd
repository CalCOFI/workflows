...
calcofi:
  ...
  # the full ~1-minute mets_measurement ships alongside as supplemental.
  tables_owned:
    ...
    - {table: mets_measurement, supplemental: true, note: "full ~1-min series, not in the core"}
...
Project METS into the shared consolidated core model via
`calcofi4db::emit_core_tables()`. The `underway` sample grain already exists —
`swfsc_cufes` uses it — and `obs` is fed by **`mets_thin`**, the same pattern
`calcofi_ctd-cast` follows (its `obs` carries `ctd_thin`, not the full scan
set). Thinning is what makes this proportionate: the full ~1-minute series is
20.5M rows and stays a supplemental output, while the thinned track lands in the
database like any other dataset. `sample` carries only the samples `mets_thin`
references, so the event dimension does not fill with rows that have no `obs`.

```{r}
#| label: emit_core

core <- emit_core_tables(con, "calcofi_mets", taxa = FALSE)
cat(glue(
...
n_exp <- dbGetQuery(con,
  "SELECT COUNT(*) FROM mets_thin t JOIN mets_sample s USING (mets_sample_uuid)
   WHERE s.grid_key IS NOT NULL")[[1]]
stopifnot(
  "obs must be one row per grid-matched mets_thin measurement" = n_obs == n_exp,
...
# of mets_thin, and a regression here would silently 10x the core
stopifnot(
...
tbls_out <- core_output_tables(
  con, extra = c("mets_measurement", "measurement_type", "dataset"))
parquet_stats <- write_parquet_outputs(
  ...
  supplemental     = c("mets_measurement"))
