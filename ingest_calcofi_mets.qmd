...
if (build_obs_mets_full) {
  ...
  n_exp_full <- dbGetQuery(con, "
    ...
    JOIN mets_sample s USING (mets_sample_uuid)
    WHERE s.latitude IS NOT NULL AND s.longitude IS NOT NULL")[[1]]
  stopifnot(
    "obs_mets_full must cover every grid-matched measurement" = n_full == n_exp_full,
...
parquet_stats <- write_parquet_outputs(
  ...
  sort_by      = list(
    ...
    obs_mets_full = c("grid_key", "measurement_type")),
