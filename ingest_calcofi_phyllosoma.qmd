...
calcofi:
  ...
  dataset_meta:
    ...
    dataset_name_short: Phyllosoma (Lobster Larvae)
    ...
    # source: https://cite.edirepository.org/cite/knb-lter-cce.188.4?style=ESIP, checked 2026-09-03
    citation_main: "CalCOFI - Scripps Institution of Oceanography and T. Koslow. 2017. Data pertaining to lobster phyllosoma, Panulirus interruptus, collection methods, locations, identification and staging (1951-2008, months of July and August) ver 4. Environmental Data Initiative. https://doi.org/10.6073/pasta/9e38121ebb26f1b59b7b39b2eff844fa"
    # source: https://cite.edirepository.org/cite/knb-lter-cce.188.4?style=ESIP, checked 2026-09-03
    ...
    # non-CC text: "intended for scholarly use by the research community...
    # Use or reproduction of any material herein for any commercial purpose is
    # prohibited without prior written permission from the responsible party."
    # source: https://cn.dataone.org/cn/v2/object/https%3A%2F%2Fpasta.lternet.edu%2Fpackage%2Fmetadata%2Feml%2Fknb-lter-cce%2F188%2F4, checked 2026-09-03
    license: custom
    license_url: https://portal.edirepository.org/nis/metadataviewer?packageid=knb-lter-cce.188.4
    pi_names: J. Anthony Koslow
  # publishes the consolidated core: the tow/measurement source shape is wrangled
  # in the notebook and projected into sample / obs / obs_attribute + taxa refs.
  tables_owned:
    - {table: sample, shared: true, note: "core event dimension (tow grain)"}
    - {table: obs, shared: true, note: "core occurrence headline (total phyllosoma per tow)"}
    - {table: obs_attribute, shared: true, note: "stage distribution within each occurrence"}
    - {table: taxon, shared: true, note: "shared taxa reference"}
    - {table: dataset_taxon, shared: true, note: "stage-type -> taxon_key crosswalk"}
...
dbWriteTable(con, "zk", zt |> select(tow_id, ship_key, datetime = date), overwrite = TRUE)
resolve_cruise_key(con, "zk", datetime_col = "datetime", require_in_cruise = TRUE) |>
  datatable(caption = "cruise_key resolution method")
zt <- zt |> left_join(dbGetQuery(con, "SELECT tow_id, cruise_key FROM zk"), by = "tow_id")
...
The source is one column per stage plus a total. Only `total_phyllosoma` is an
occurrence headline; the eleven `phyllosoma_stage_N` columns are a stage
distribution *within* that occurrence, so they become `obs_attribute` rows
(`measurement_type = 'stage'`, `bin_value = N`) rather than eleven separate
observations. `metadata/measurement_taxon.csv` supplies the split and the
`taxon_key` for *Panulirus interruptus*.
