...
core <- emit_core_tables(
  con, "calcofi_phyllosoma", measurement_taxon = mt_taxon, overrides = tx_over)
cat(glue(
...
# the headline must be the total only — one row per grid-matched tow — and the
...
n_exp <- dbGetQuery(con, "
  ...
  WHERE tw.grid_key IS NOT NULL AND m.measurement_type = 'total_phyllosoma'")[[1]]
n_stage_in_obs <- dbGetQuery(con,
  "SELECT COUNT(*) FROM obs WHERE measurement_type LIKE 'phyllosoma_stage%'")[[1]]
stopifnot(
  "obs must carry the total only, one row per grid-matched tow" = n_obs == n_exp,
...
cat(glue("obs parity: {format(n_obs, big.mark = ',')} headline rows; ",
         "{format(core$obs_attribute %||% 0, big.mark = ',')} stage bins"), "\n")
...
compat <- create_compat_views(con, "calcofi_phyllosoma")
