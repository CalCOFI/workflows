...
2. **Split serves subsets fast; lumped re-reads everything (Fig 2).** A per-cruise
   ...
   **long** view live.
3. **NetCDF, the native path, is per-cruise = one dataset.** CalCOFI has **~5.55 M
   casts**, so one file *per cast* (à la Trinidad Head) is infeasible; bundling **by
   cruise = 96 files**, served as **one aggregated ERDDAP dataset** (one form), loads
   small and serves cruise-scoped queries fastest.
