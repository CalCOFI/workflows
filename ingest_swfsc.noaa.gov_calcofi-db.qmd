...
librarian::shelf(
  ...
  jsonlite, knitr, litedown, lubridate, purrr, readr, rlang, sf, stringr, tibble, tidyr,
  units, uuid, webshot2,
  quiet = T)
...
# TODO: force species_id to integer from dbl in species_id, ichthyo
```
...
tbls <- DBI::dbListTables(con) |>
  sort() # |>
  # setdiff(c("grid", "segment"))
  # TODO: add grid, segment to later workflow
...
dbExecute(con, "ALTER TABLE site ADD COLUMN IF NOT EXISTS geom GEOMETRY")
dbExecute(con, "UPDATE site SET geom = ST_Point(longitude, latitude)")
message("Added geometry column to site table")
```
...
dbExecute(con, "ALTER TABLE site ADD COLUMN IF NOT EXISTS grid_key TEXT")

# spatial join (may be slower than PostGIS without spatial indexes)
dbExecute(con, "
  UPDATE site SET grid_key = (
    SELECT g.grid_key FROM grid g
    WHERE ST_Intersects(site.geom, g.geom) LIMIT 1)")

# check results
grid_stats <- dbGetQuery(con, "
  SELECT
...
# note: parquet outputs are in data/parquet/swfsc.noaa.gov/calcofi-db/
...
- [ ] Upload parquet to gs://calcofi-db/ingest/{provider}_{dataset}/
