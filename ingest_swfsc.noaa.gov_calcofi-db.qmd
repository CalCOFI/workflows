...
librarian::shelf(
  DBI, dm, DT, fs, glue, gargle, googledrive, here, janitor, jsonlite, 
...
workflow      <- glue("ingest_{provider}_{dataset}")
workflow_qmd  <- glue("{workflow}.qmd")
...
qmd_path  <- ifelse(
  ...
  path_real()
stopifnot(str_detect(qmd_path, glue("ingest_{provider}_{dataset}\\.qmd")))

# dataset with external data folder
dir_data <- "/Users/bbest/My Drive/projects/calcofi/data"
# noaa-calcofi-db - Google Drive
gdir_data <- "https://drive.google.com/drive/u/0/folders/13A9yDJCdV0M-lrMLqurzIb3G3ozknr9O" 

dir_csv    <- glue("{dir_data}/{provider}/{dataset}")
dir_ingest <- here(glue("ingest/{provider}/{dataset}/tbls_raw.csv"))
# input tables (version controlled)
tbls_in_csv <- here(glue("ingest/{provider}/{dataset}/tbls_raw.csv"))
flds_in_csv <- here(glue("ingest/{provider}/{dataset}/flds_raw.csv"))
# rename tables (version controlled)
tbls_rn_csv <- here(glue("ingest/{provider}/{dataset}/tbls_redefine.csv"))
flds_rn_csv <- here(glue("ingest/{provider}/{dataset}/flds_redefine.csv"))
```
...
googledrive::drive_auth(
  ...
  scopes = "drive")
# OAuth access credentials cached in ~/Library/Caches/gargle
