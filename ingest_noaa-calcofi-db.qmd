...
site_seg <- tbl(con_dev, "site") |> 
  select(cruise_uuid, site_uuid, lon = longitude, lat = latitude) |> 
  left_join(
    tbl(con_dev, "tow") |> 
      select(site_uuid, time_start),
    by = "site_uuid") |> 
  group_by(
    site_uuid, cruise_uuid, lon, lat) |> 
  summarize(
    time_beg = min(time_start),
    time_end = max(time_start),
    .groups = "drop") |> 
  arrange(cruise_uuid, time_beg) |> 
  collect() |> 
...
d_cruise <- tbl(con_dev, "cruise") |> 
  collect() |> 
  mutate(
    ym_ship = glue("{format(date_ym, '%Y-%m')}:{ship_key}"))
  
site_seg |> 
  left_join(
    d_cruise,
    by = "cruise_uuid") |> 
  mapView(zcol = "ym_ship")
```
