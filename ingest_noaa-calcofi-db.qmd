...
Database documentation scheme. Google Drive \*.csv files get ingested,
and metadata CSV files are setup to rename and describe tables and
fields (tbls_rename.csv , flds_rename.csv). Then after injecting the
data for each of the tables, extra metadata is added to the COMMENTs of
each table as with JSON elements (optionally with links as markdown),
including at the table level:

-   **description**: general description describing contents and how
    each row is unique
-   **source**: CSV (linked to Google Drive source as markdown)
-   **source_created**: datetime stamp of when source was created on
    GoogleDrive
...
-   **units**: using the International System of Units (SI) as much as
    possible These comments are then exposed by the API
    [db_tables](https://api.calcofi.io/db_tables) endpoint, which can be
    consumed and rendered into a tabular searchable catalog with
    [calcofi4r::cc_db_catalog](https://calcofi.io/calcofi4r/reference/cc_db_catalog.html)

```{r}
#| label: setup

librarian::shelf(
  DBI, dm, DT, fs, glue, googledrive, here, janitor, jsonlite, knitr, lubridate, 
  purrr, readr, rlang, tibble, tidyr, uuid,
  quiet = T)
...
# get path of this quarto document with knitr
qmd_path  <- ifelse(
  rstudioapi::isAvailable(), 
  rstudioapi::getActiveDocumentContext()$path,
  knitr::current_input(dir = TRUE)) |> 
  path_real()
workflow_qmd  <- str_replace(qmd_path, glue("{here()}/"), "")
workflow      <- path_ext_remove(workflow_qmd)
workflow_url  <- glue("https://calcofi.io/workflows/{workflow}.html")
# dataset with external data folder
dir_data <- "/Users/bbest/My Drive/projects/calcofi/data"
gdir_data <- "https://drive.google.com/drive/u/0/folders/13A9yDJCdV0M-lrMLqurzIb3G3ozknr9O" # noaa-calcofi-db - Google Drive
dataset  <- "noaa-calcofi-db"
dir_csv  <- glue("{dir_data}/{dataset}")
# input tables (version controlled)
tbls_in_csv <- here(glue("data/ingest/{dataset}/tbls_raw.csv"))
flds_in_csv <- here(glue("data/ingest/{dataset}/flds_raw.csv"))
# rename tables (version controlled)
tbls_rn_csv <- here(glue("data/ingest/{dataset}/tbls_redefine.csv"))
flds_rn_csv <- here(glue("data/ingest/{dataset}/flds_redefine.csv"))
```

```{r}
#| label: google-drive_csv-list

# Get the Google Drive folder;  cache OAuth access credentials in ~/Library/Caches/gargle
d_gdir_data <- drive_ls(gdir_data) |> 
  mutate(
    web_view_link = map_chr(drive_resource, \(x) x$webViewLink),
    created_time  = map_chr(drive_resource, \(x) x$createdTime) |> 
      as_datetime()) |> 
  filter(
    str_detect(name, "\\.csv$"))
...
d_gdir_data |>
  mutate(
    csv = glue("<a href='{web_view_link}' target='_blank'>{name}</a>") ) |>
  select(csv, created_time) |>
  datatable(escape = F, caption = "CSV files to ingest.") |> 
  formatDate(~created_time, "toLocaleString")
```
...
tbl_to_db <- function(tbl){ # tbl = "egg" # tbl = "ship"
  ...
  if (!tbl_exists | overwrite) {
    ...
    # comment to table ----
    #   with JSON elements (in markdown) of dtime ingested, source, and workflow
    tbl_description <- d |> 
      filter(tbl_new == !!tbl) |>
      pull(tbl_description)
    csv <- d |> 
      filter(tbl_new == !!tbl) |> 
      pull(csv) |> 
      basename()
    ...
    dbExecute(
      ...
      glue(
        'COMMENT ON TABLE {schema}.{tbl} IS \'{{
          ...
          "source":            "[{csv}]({csv_url})",
          "source_created":    "{csv_created}",
          "workflow":          "[{workflow}]({workflow_url})"},
          "workflow_ingested": "{Sys.time()}" }}\'' ))
    
    # get comments on table
    # devtools::install_local(here("../calcofi4r"))
    # calcofi4r::cc_db_catalog(tables = "larva")
    
    # tbl(con_dev, "pg_description")
    # dbGetQuery(con, "SELECT * FROM pg_description WHERE objsubid = 0")
